...
def load_variables(root: Path) -> Dict:
    """Load the single-source variables file, overlaying version from VERSION."""
    ...
    with open(var_file, "r", encoding="utf-8") as f:
        v = json.load(f)
    # Override version and project_name from the authoritative VERSION file
    # generated by CMake so they never drift from CMakeLists.txt.
    version_file = root / "VERSION.txt"
    if version_file.exists():
        lines = version_file.read_text().strip().splitlines()
        if len(lines) >= 1:
            v["version"] = lines[0].strip()
        if len(lines) >= 2:
            v["project_name"] = lines[1].strip()
    return v
