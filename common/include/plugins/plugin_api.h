...
typedef struct PolyglotHostServices {
  // Logging
  void (*log)(const PolyglotHostContext *ctx, PolyglotLogLevel level, const char *message);

  // Emit a diagnostic visible in the IDE's problems view
  void (*emit_diagnostic)(const PolyglotHostContext *ctx, const PolyglotDiagnostic *diag);

  // Read a plugin-scoped setting.  Returns NULL if not set.
  const char *(*get_setting)(const PolyglotHostContext *ctx, const char *key);

  // Write a plugin-scoped setting (persisted across sessions).
  void (*set_setting)(const PolyglotHostContext *ctx, const char *key, const char *value);

  // Request the host to open a file in the editor (UI plugins only).
  void (*open_file)(const PolyglotHostContext *ctx, const char *path, uint32_t line);

  // Register a file extension with a language name.
  void (*register_file_type)(const PolyglotHostContext *ctx, const char *extension,
                             const char *language);
  ...
  void (*subscribe_event)(const PolyglotHostContext *ctx, PolyglotEventType event_type);

  // Unsubscribe from a previously subscribed event type.
  void (*unsubscribe_event)(const PolyglotHostContext *ctx, PolyglotEventType event_type);

  // Register a menu item contribution in the host IDE.
  void (*register_menu_item)(const PolyglotHostContext *ctx, const PolyglotMenuContribution *item);

  // Unregister a previously registered menu item by action_id.
  void (*unregister_menu_item)(const PolyglotHostContext *ctx, const char *action_id);
...
typedef struct PolyglotLanguageProvider {
  ...
  uint32_t (*tokenize)(const char *source, size_t len, PolyglotToken *out_tokens,
                       uint32_t max_tokens);

  // Analyse `source` and emit diagnostics via `host->emit_diagnostic`.
  void (*analyze)(const PolyglotHostContext *ctx, const PolyglotHostServices *host,
  ...
  char *(*compile_to_ir)(const PolyglotHostContext *ctx, const PolyglotHostServices *host,
                         const char *source, size_t len, const char *filename);
...
typedef struct PolyglotOptimizerPass {
  ...
  char *(*run)(const PolyglotHostContext *ctx, const PolyglotHostServices *host, char *ir,
               size_t *ir_len);
...
typedef struct PolyglotCodeAction {
  ...
  char *(*apply)(const PolyglotHostContext *ctx, const PolyglotHostServices *host,
                 const char *source, size_t len, const char *filename, uint32_t sel_start_line,
                 uint32_t sel_start_col, uint32_t sel_end_line, uint32_t sel_end_col);
} PolyglotCodeAction;
...
typedef struct PolyglotFormatter {
  ...
  char *(*format)(const PolyglotHostContext *ctx, const PolyglotHostServices *host,
                  const char *source, size_t len, const char *filename);
} PolyglotFormatter;
...
typedef struct PolyglotLinter {
  ...
  void (*lint)(const PolyglotHostContext *ctx, const PolyglotHostServices *host, const char *source,
...
typedef struct PolyglotCompletionProvider {
  ...
  uint32_t (*complete)(const PolyglotHostContext *ctx, const PolyglotHostServices *host,
                       const char *source, size_t len, const char *filename, uint32_t line,
                       uint32_t column, PolyglotCompletionItem *out_items, uint32_t max_items);
...
typedef struct PolyglotDiagnosticProvider {
  ...
  void (*diagnose)(const PolyglotHostContext *ctx, const PolyglotHostServices *host,
                   const char *source, size_t len, const char *filename);
...
typedef struct PolyglotTemplateProvider {
  ...
  uint32_t (*get_templates)(const PolyglotHostContext *ctx, PolyglotTemplate *out, uint32_t max);
...
typedef struct PolyglotTopologyProcessor {
  ...
  char *(*process)(const PolyglotHostContext *ctx, const PolyglotHostServices *host,
...
typedef PolyglotPlugin *(*PFN_polyglot_plugin_create)(const PolyglotHostContext *ctx,
...
// CAP_LANGUAGE
typedef const PolyglotLanguageProvider *(*PFN_polyglot_plugin_get_language)(PolyglotPlugin *plugin);

// CAP_OPTIMIZER — return a NULL-terminated array of passes
typedef const PolyglotOptimizerPass **(*PFN_polyglot_plugin_get_passes)(PolyglotPlugin *plugin,
                                                                        uint32_t *out_count);

// CAP_CODE_ACTION — return a NULL-terminated array of actions
typedef const PolyglotCodeAction **(*PFN_polyglot_plugin_get_code_actions)(PolyglotPlugin *plugin,
                                                                           uint32_t *out_count);

// CAP_FORMATTER
typedef const PolyglotFormatter *(*PFN_polyglot_plugin_get_formatter)(PolyglotPlugin *plugin);

// CAP_LINTER
typedef const PolyglotLinter *(*PFN_polyglot_plugin_get_linter)(PolyglotPlugin *plugin);
...
// CAP_TEMPLATE
typedef const PolyglotTemplateProvider *(*PFN_polyglot_plugin_get_template)(PolyglotPlugin *plugin);
...
typedef void (*PFN_polyglot_plugin_on_event)(PolyglotPlugin *plugin, const PolyglotEvent *event);
...
#define POLYGLOT_PLUGIN_ENTRY_POINTS                                                               \
  ...
  POLYGLOT_EXPORT PolyglotPlugin *polyglot_plugin_create(const PolyglotHostContext *ctx,           \
                                                         const PolyglotHostServices *host);        \
