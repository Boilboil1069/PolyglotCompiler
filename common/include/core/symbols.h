...
public:
  ...
  const Symbol *Declare(const Symbol &symbol) {
    ...
    // Functions may be overloaded; keep a vector but also preserve first-declared for simple
    // lookup.
  ...
  const Symbol *DeclareInScope(int scope_id, const Symbol &symbol) {
    if (scope_id < 0 || scope_id >= static_cast<int>(scopes_.size()))
      return nullptr;
  ...
  std::optional<ResolveResult> Lookup(const std::string &name) const {
    ...
    for (auto it = scope_stack_.rbegin(); it != scope_stack_.rend(); ++it, ++distance) {
      ...
      if (found_scope == symbols_by_scope_.end())
        continue;
  ...
  std::optional<ResolveResult> ResolveFunction(const std::string &name,
    ...
    auto conv_score = [&](const Type &from, const Type &to) -> int {
      ...
      auto is_object = [](const Type &t) {
        return t.kind == TypeKind::kClass || t.kind == TypeKind::kStruct ||
               t.kind == TypeKind::kGenericInstance;
      };
      if (is_object(from) && is_object(to) && (from.language == to.language))
        return 10;
    ...
    for (auto it = scope_stack_.rbegin(); it != scope_stack_.rend(); ++it, ++distance) {
      ...
      if (scope_it == functions_by_scope_.end())
        continue;
      auto fn_it = scope_it->second.find(name);
      if (fn_it == scope_it->second.end())
        continue;
      const Symbol *best = nullptr;
      int best_score = std::numeric_limits<int>::max();
      for (const Symbol *cand : fn_it->second) {
        if (!cand || cand->type.type_args.size() < 1)
          continue;
      ...
      if (best)
        return ResolveResult{best, distance};
    }
    return std::nullopt;
  }
...
private:
  ...
  bool IsAccessible(const Symbol *sym, int declaring_scope) const {
    if (!sym)
      return false;
    if (sym->access.empty() || sym->access == "public")
      return true;
  ...
  bool IsDerivedFrom(const std::string &type, const std::string &base) const {
    ...
    if (it == type_bases_.end())
      return false;
    for (const auto &b : it->second) {
      if (b == base)
        return true;
      if (IsDerivedFrom(b, base))
        return true;
    }
    return false;
  }
  ...
  std::unordered_map<int, std::unordered_map<std::string, std::vector<const Symbol *>>>
      functions_by_scope_{};
