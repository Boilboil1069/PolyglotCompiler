...
struct SourceLocation {
  ...
  SourceLocation(const std::string &f, uint32_t l, uint32_t c = 0) : file(f), line(l), column(c) {}
};
...
public:
  ...
  void SetCompileUnit(const std::string &filename, const std::string &directory,
                      const std::string &producer, dwarf::Language language);

  // Add type information
  DIE *AddBaseType(const std::string &name, uint32_t byte_size, dwarf::BaseTypeEncoding encoding);
  ...
  DIE *AddMember(DIE *struct_type, const std::string &name, DIE *type, uint32_t offset);
  ...
  DIE *AddTemplateValueParameter(DIE *parent, const std::string &name, DIE *type,
  ...
  DIE *AddSubprogram(const std::string &name, DIE *return_type, uint64_t low_pc, uint64_t high_pc,
                     const SourceLocation &loc);
  void AddParameter(DIE *subprogram, const std::string &name, DIE *type,
                    const LocationExpr &location);
  void AddLocalVariable(DIE *subprogram, const std::string &name, DIE *type,
...
public:
  ...
  void RegisterType(const std::string &name, uint32_t size, dwarf::BaseTypeEncoding encoding);

  // Function debugging
  void BeginFunction(const std::string &name, const std::string &return_type,
                     uint64_t start_address, const SourceLocation &loc);
  void AddFunctionParameter(const std::string &name, const std::string &type, int frame_offset);
  void AddLocalVariable(const std::string &name, const std::string &type, int frame_offset,
                        const SourceLocation &loc);
