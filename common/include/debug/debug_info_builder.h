...
public:
  ...
  enum class Encoding { Signed, Unsigned, Float, Bool, Char };
...
public:
  /** @brief Kind enumeration. */
  enum class Kind { Local, Parameter, Global, Member };

  DIVariable(Kind kind, const std::string &name, DIType *type, const SourceLocation &location) :
...
class DIFunction {
public:
  DIFunction(const std::string &name, const SourceLocation &location, DIType *return_type) :
  ...
  const std::vector<DIVariable *> &GetLocalVariables() const { return local_variables_; }
  ...
  void AddInlineInstance(const InlineInstance &instance) { inline_instances_.push_back(instance); }
  const std::vector<InlineInstance> &GetInlineInstances() const { return inline_instances_; }
...
public:
  ...
  // 创建函数
  DIFunction *CreateFunction(const std::string &name, const SourceLocation &location,
                             DIType *return_type);
  ...
  // 优化代码调试支持
  void TrackValueLocation(DIVariable *var, uint64_t pc, const DIVariable::Location &location);

  // 内联调试信息
  void RecordInlineInstance(DIFunction *inlined_func, const std::string &caller,
                            const SourceLocation &call_site, uint64_t low_pc, uint64_t high_pc);
...
public:
  explicit DebugInfoValidator(const DebugInfoBuilder &builder) : builder_(builder) {}
...
public:
  explicit DebugInfoPrinter(const DebugInfoBuilder &builder) : builder_(builder) {}
