...
void DwarfBuilder::SetCompileUnit(const std::string& filename,
                                  const std::string& directory,
    ...
    compile_unit_->SetAttribute(dwarf::Attribute::kLanguage,
                               dwarf::Form::kData2,
...
DIE* DwarfBuilder::AddBaseType(const std::string& name,
                               uint32_t byte_size,
...
DIE* DwarfBuilder::AddPointerType(DIE* pointee_type) {
    ...
    if (pointee_type) {
        die->SetAttribute(dwarf::Attribute::kType, dwarf::Form::kRef4,
                         pointee_type->GetOffset());
    }
...
DIE* DwarfBuilder::AddTemplateValueParameter(DIE* parent, const std::string& name, 
...
DIE* DwarfBuilder::AddMember(DIE* struct_type, const std::string& name,
                             DIE* type, uint32_t offset) {
    auto die = std::make_unique<DIE>(dwarf::Tag::kMember);
    die->SetAttribute(dwarf::Attribute::kName, name);
    die->SetAttribute(dwarf::Attribute::kType, dwarf::Form::kRef4, type->GetOffset());
    die->SetAttribute(dwarf::Attribute::kDataMemberLocation,
                     dwarf::Form::kData4, offset);
...
DIE* DwarfBuilder::AddSubprogram(const std::string& name,
                                DIE* return_type,
                                uint64_t low_pc,
                                uint64_t high_pc,
...
void DwarfBuilder::AddParameter(DIE* subprogram,
...
void DwarfBuilder::AddLocalVariable(DIE* subprogram,
                                   const std::string& name,
                                   DIE* type,
                                   const LocationExpr& location,
                                   const SourceLocation& loc) {
...
std::vector<uint8_t> DwarfBuilder::EncodeDebugAbbrev() {
    ...
    result.push_back(0); result.push_back(0);  // End of attributes
...
void DebugInfoGenerator::AddFunctionParameter(const std::string& name,
...
void DebugInfoGenerator::AddLocalVariable(const std::string& name,
                                         const std::string& type,
                                         int frame_offset,
                                         const SourceLocation& loc) {
    if (!current_function_) return;
