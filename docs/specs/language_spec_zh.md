...
| `__ploy_python_new` | `runtime/include/libs/python_rt.h` | 实例化 Python 类 |
| `__ploy_java_call` | `runtime/include/libs/java_rt.h` | 通过 JNI 调用 Java 方法 |
| `__ploy_java_init` | `runtime/include/libs/java_rt.h` | 初始化 JVM |
| `__ploy_dotnet_call` | `runtime/include/libs/dotnet_rt.h` | 通过 CoreCLR 调用 .NET 方法 |
| `__ploy_dotnet_init` | `runtime/include/libs/dotnet_rt.h` | 初始化 .NET 运行时 |
