...
对类型为 `OPTION<T>` 的值施加 `?`，求值后：

* 若为 `Some(v)`，整个表达式取 `v`（类型为 `T`）；
* 若为 `None`，外层函数立即返回其零值 / `None`，与 Rust 中的 `?`
  在 `Option` 流水线上的早返语义一致。

`?` 是**后缀**运算符，绑定优先级高于所有二元运算符，因此
`obj.method()?.field` 会先求值 `obj.method()`，再解包，最后访问
`.field`。

静态检查（语义分析，自 v1.19.0 起）：

* 操作数类型必须为 `OPTION<T>`，否则报告 `OPTION<T> operand` 硬错误；
* 外层函数返回类型必须为 `OPTION<U>` 且 `U` 与 `T` 赋值兼容，否则报告
  `OPTION<U>` 不匹配错误；
* `?` 不允许出现在模块作用域。

未来错误传播形式（在 `Result<T, E>` / `Error` 类调用上复用同一 `?`
token）通过**操作数类型**与本节区分：操作数为 `OPTION` 时按上文规则
解析；操作数为 `Error` 形态（后续版本引入）时进入错误传播分支。
