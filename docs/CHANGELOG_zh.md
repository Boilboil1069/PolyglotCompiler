...
- POSIX 平台上 `polyld` 现在为产出的可执行文件、共享库与 Mach-O
  ...
  `0644` 默认掩码，导致产出的二进制无法直接通过 `execve(2)` 启动，
  必须先手工 `chmod +x`。静态库与可重定位对象保持默认权限，因为
  它们只是后续阶段的输入。
- `polyld` 内部「`.exe` 后缀自动路由 PE32+ 写出器」的逻辑现在仅
  在 Windows 宿主上生效。在 macOS / Linux 上，将输出命名为
  `foo.exe` 的构建脚本不再被静默改判为 PE32+；改为按宿主原生容器
  （Mach-O / ELF）写出，使产物能被宿主加载器直接装载。
- Windows 宿主上的不变量「`polyld foo.o -o foo.exe` 产出 PE32+
  镜像」保持不变。
