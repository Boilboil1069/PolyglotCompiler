...
## 9. PE-7 — 真实后端 `polyrt_println` 闭环

**状态：** 已发布（v1.9.1，2026-04-29）。

PE-7 是把 B4（`BuildPrintlnSequencePE` 直接合成手写 PE 输出预算好的
消息向量）与 B7（脚手架要求*真实*的后端流水线为任意用户 PRINTLN
字节产出该 PE）之间的鸿沟收掉的一档。在 PE-7 之前，`polyld`
只能从手工构造的对象集合中恢复消息字节：把真实的
`polyc --emit-obj=…` 灌进去会得到空的恢复序列，链接器静默兜底
到 `BuildExitZeroPE`，于是程序 stdout 为空。PE-7 让恢复路径对
任何由生产降级链路产出的、含 PRINTLN 的程序都无条件成立。

### 9.1  COFF 重定位记录的盘上步长

每条盘上的 COFF 重定位记录恰好 **10 字节**
（`u32 VirtualAddress + u32 SymbolTableIndex + u16 Type`），但
`tools/polyld/src/linker.cpp` 中的加载器之前是按 C++ 结构体的
`sizeof` 读取的，由于尾部 `u16` 的自然对齐而被填到 **12 字节**。
第一条之后的每条记录都偏移 2 字节，重定位表从下标 1 开始几乎
全是脏数据。加载器现在通过 `std::memcpy` 从固定的 `buf[10]`
逐字段读取，与盘上布局逐字节一致。

### 9.2  COFF 符号表中的 aux 记录对齐

COFF 在每个段符号后会放一条辅助（aux）记录，承载段长与重定位
计数。加载器之前在盘上正确跳过了每条 aux，但**没有**在
`obj.symbols` 里为其保留槽位，于是内存里的向量是*压缩*过的，而
盘上的重定位记录仍然引用*未压缩*的盘上下标。重定位回填环节
（`r.symbol = obj.symbols[r.symbol_index].name`）就读到了错位的
条目，把所有重定位都赋上了错误的符号名。加载器现在为每条
吞掉的 aux 记录追加一个占位 `Symbol`（空名、`is_defined == false`），
让盘上下标空间与 `obj.symbols` 位置一一对齐。占位条目永远不会
命中下游查找，因为所有恢复 / 解析路径都按非空 `name` 作键并
只考虑 `is_defined == true` 的候选。
...
- 同时接受 `println.msg<N>`（历史）与 `str<N>`（当前）两种前缀，
  并要求前缀后必须紧跟数字以保证收口严格、不会把 `stride`、
  `string_table` 之类无关用户全局误纳入。
- 解析重定位符号时会剥掉可选的 `.ptr` GEP 别名后缀，再到任一
  对象的 `.rdata` 里定位裸数据符号。
- 对 COFF 中每符号尺寸记为 0 的情况做容错：从同段内下一个
  已定义同级符号的偏移反推消息长度，若不可得则扫描 NUL
  终止符，最后兜底到段尾作为上界。
...
配套的样本脚手架门闸 `--require-min-ok N` /
`-RequireMinOk N`（添加在 `scripts/build_all_samples.{ps1,sh}`）为
CI 提供了一个发布钩子：一旦后端可以闭环全部样本矩阵中的
PRINTLN 字面量，把下限提升到 `33`（样本数）即可让降级或恢复
pass 上的任何回归立即让构建挂掉。
