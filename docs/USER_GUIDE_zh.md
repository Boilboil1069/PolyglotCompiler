...
| 功能 | 说明 |
|------|------|
| **力导向布局** | 使用 Fruchterman–Reingold 风格的力模拟自动布局，节点间互斥、边沿引力，含模拟退火平滑收敛。也可通过布局选择器切换网格布局（从上到下、从左到右）。 |
| **交互式边创建** | 从输出端口拖拽到输入端口（或反向）创建新边。自环和重复边会被拒绝并显示诊断信息。 |
| **交互式边删除** | 右键点击边即可从图中删除。 |
| **文件实时重载** | 通过 `QFileSystemWatcher` 监控已加载的 `.ploy` 文件；文件在磁盘上变更后，经 500 毫秒防抖后自动重建拓扑。 |
| **调试断点高亮** | 调试器暂停在某个源码位置时，对应的拓扑节点以黄色发光边框高亮显示，视图自动滚动居中。调试会话结束时高亮清除。 |
| **端口悬停提示** | 将鼠标悬停在任意输入或输出端口上，可查看包含端口名称、类型、方向和父节点名称的丰富提示。悬停时端口视觉放大。 |
...
2. 使用布局选择器在 *力导向*、*从上到下网格* 和 *从左到右网格* 之间切换。
