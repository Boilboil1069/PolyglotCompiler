...
class Threading {
public:
  template <typename Fn> std::thread Run(Fn &&fn) { return std::thread(std::forward<Fn>(fn)); }

  template <typename Fn> void RunDetached(Fn &&fn) { std::thread(std::forward<Fn>(fn)).detach(); }
...
public:
  ...
  template <typename F> void ParallelFor(size_t start, size_t end, F &&body);

  template <typename T, typename F> T ParallelReduce(size_t start, size_t end, T init, F &&reducer);
...
public:
  template <typename F> std::shared_ptr<Coroutine> Create(F &&func);
...
public:
  ...
  template <typename F> void Then(F &&callback);
...
enum class MemoryOrder { kRelaxed, kAcquire, kRelease, kAcqRel, kSeqCst };
...
public:
  ...
  bool CompareExchange(T &expected, T desired, MemoryOrder success = MemoryOrder::kSeqCst,
...
template <typename F> auto ThreadPool::Submit(F &&task) -> std::future<decltype(task())> {
...
template <typename F> void WorkStealingScheduler::ParallelFor(size_t start, size_t end, F &&body) {
  size_t workers =
      num_workers_ == 0 ? std::max<size_t>(1, std::thread::hardware_concurrency()) : num_workers_;
...
template <typename T> void LockFreeQueue<T>::Push(const T &value) {
...
template <typename T> bool LockFreeQueue<T>::Pop(T &value) {
  while (true) {
...
template <typename T> bool LockFreeQueue<T>::Empty() const {
  Node *head = head_.load(std::memory_order_acquire);
  return head->next.load(std::memory_order_acquire) == nullptr;
}

template <typename T> LockFreeStack<T>::LockFreeStack() : head_(nullptr) {}
...
template <typename T> void LockFreeStack<T>::Push(const T &value) {
  Node *node = new Node(value);
...
template <typename T> bool LockFreeStack<T>::Pop(T &value) {
...
template <typename T> bool LockFreeStack<T>::Empty() const {
  return head_.load(std::memory_order_acquire) == nullptr;
}

template <typename F> std::shared_ptr<Coroutine> CoroutineScheduler::Create(F &&func) {
...
template <typename T> ThreadLocal<T>::ThreadLocal(T value) : initial_(value) {}

template <typename T> T &ThreadLocal<T>::Get() {
  std::lock_guard<std::mutex> lock(mutex_);
...
template <typename T> void ThreadLocal<T>::Set(const T &value) {
  std::lock_guard<std::mutex> lock(mutex_);
  storage_[std::this_thread::get_id()] = value;
}

template <typename T> Future<T>::Future() : state_(std::make_shared<State>()) {}
...
template <typename T> bool Future<T>::IsReady() const {
  std::lock_guard<std::mutex> lock(state_->mu);
  return state_->ready;
}

template <typename T> template <typename F> void Future<T>::Then(F &&callback) {
  std::lock_guard<std::mutex> lock(state_->mu);
...
template <typename T> void Promise<T>::SetValue(const T &value) {
...
template <typename T> void Promise<T>::SetException(std::exception_ptr ex) {
  {
    std::lock_guard<std::mutex> lock(state_->mu);
...
template <typename T> Future<T> Promise<T>::GetFuture() {
  return Future<T>(state_);
}

template <typename T> T Atomic<T>::Load(MemoryOrder order) const {
  return value_.load(ToStdOrder(order));
}

template <typename T> void Atomic<T>::Store(T value, MemoryOrder order) {
  value_.store(value, ToStdOrder(order));
}

template <typename T> T Atomic<T>::Exchange(T value, MemoryOrder order) {
...
bool Atomic<T>::CompareExchange(T &expected, T desired, MemoryOrder success, MemoryOrder failure) {
  return value_.compare_exchange_strong(expected, desired, ToStdOrder(success),
                                        ToStdOrder(failure));
}

template <typename T> T Atomic<T>::FetchAdd(T arg, MemoryOrder order) {
  return value_.fetch_add(arg, ToStdOrder(order));
}

template <typename T> T Atomic<T>::FetchSub(T arg, MemoryOrder order) {
