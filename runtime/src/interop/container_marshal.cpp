...
static inline SlotState *SlotStatePtr(const RuntimeDict *dict, std::size_t idx) {
  return reinterpret_cast<SlotState *>(static_cast<char *>(dict->slots) + SlotOffset(dict, idx));
}
  ...
  if (dict->slots) {
    for (std::size_t i = 0; i < dict->capacity; ++i) {
      ...
      for (std::size_t step = 0; step < new_cap; ++step) {
        ...
        SlotState *nst =
            reinterpret_cast<SlotState *>(static_cast<char *>(new_slots) + pos * stride);
        if (*nst == SlotState::kEmpty) {
          *nst = SlotState::kOccupied;
          std::memcpy(static_cast<char *>(new_slots) + pos * stride + dict->key_offset, old_key,
                      dict->key_size);
          std::memcpy(static_cast<char *>(new_slots) + pos * stride + dict->value_offset, old_val,
                      dict->value_size);
          break;
        }
      }
...
void *__ploy_rt_dict_lookup(void *raw, const void *key) {
  ...
  for (std::size_t step = 0; step < dict->capacity; ++step) {
    ...
    if (*st == SlotState::kEmpty)
      return nullptr; // probe chain broken
    if (*st == SlotState::kTombstone)
      continue; // skip deleted slot
    if (std::memcmp(SlotKeyPtr(dict, idx), key, dict->key_size) == 0) {
...
static bool EnsurePythonLoaded() {
  if (py_init_attempted_)
    return py_lib_ != nullptr;
  py_init_attempted_ = true;

#ifdef _WIN32
  py_lib_ = PY_LOAD("python3.dll");
  if (!py_lib_)
    py_lib_ = PY_LOAD("python310.dll");
  if (!py_lib_)
    py_lib_ = PY_LOAD("python311.dll");
  if (!py_lib_)
    py_lib_ = PY_LOAD("python312.dll");
#elif defined(__APPLE__)
  py_lib_ = PY_LOAD("libpython3.dylib");
#else
  py_lib_ = PY_LOAD("libpython3.so");
  if (!py_lib_)
    py_lib_ = PY_LOAD("libpython3.10.so");
  if (!py_lib_)
    py_lib_ = PY_LOAD("libpython3.11.so");
#endif
...
void *__ploy_rt_convert_list_to_pylist(void *list) {
  ...
  for (std::size_t i = 0; i < rl->count; ++i) {
    ...
    if (rl->elem_size <= 8 && py_long_from_) {
      ...
      py_obj =
          py_bytes_from_(static_cast<const char *>(elem_ptr), static_cast<long>(rl->elem_size));
    }
...
void *__ploy_rt_convert_dict_to_pydict(void *dict) {
  ...
  for (std::size_t i = 0; i < rd->capacity; ++i) {
    ...
    if (rd->key_size <= 8 && py_long_from_) {
      ...
      py_key = py_bytes_from_(static_cast<const char *>(kptr), static_cast<long>(rd->key_size));
    }
    ...
    if (rd->value_size <= 8 && py_long_from_) {
      ...
      py_val = py_bytes_from_(static_cast<const char *>(vptr), static_cast<long>(rd->value_size));
    }
...
void *__ploy_rt_convert_pydict_to_dict(void *pydict, std::size_t key_size, std::size_t value_size) {
  ...
  if (!keys || !py_list_size_ || !py_list_getitem_)
    return dict;
