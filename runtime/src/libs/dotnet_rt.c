...
typedef int (*coreclr_initialize_fn)(const char *exePath, const char *appDomainFriendlyName,
...
typedef int (*coreclr_create_delegate_fn)(void *hostHandle, unsigned int domainId,
                                          const char *assemblyName, const char *typeName,
...
int polyglot_dotnet_init(int version_hint) {
  ...
  if (dotnet_root && dotnet_root[0]) {
    ...
    int versions_to_try[] = {
        version_hint ? version_hint : 8, 9, 8, 7, 6, 0 // Sentinel
    };
    for (int i = 0; versions_to_try[i] != 0; ++i) {
      ...
      snprintf(clr_path, sizeof(clr_path),
               "%s/shared/Microsoft.NETCore.App/%d.0.0/libcoreclr.dylib", dotnet_root, ver);
#else
  ...
  if (!clr_lib_) {
    fprintf(stderr,
            "[polyglot-dotnet] error: CoreCLR library not found at '%s'. "
  ...
  if (runtime_dir[0]) {
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
      do {
        char full_path[1200];
        snprintf(full_path, sizeof(full_path), "%s\\%s", runtime_dir, find_data.cFileName);
  ...
  int rc = clr_init_fn_("polyglot", "PolyglotCompiler", 1, property_keys, property_values,
                        &clr_host_, &clr_domain_);
  if (rc != 0) {
    fprintf(stderr,
            "[polyglot-dotnet] error: coreclr_initialize failed with "
...
void *polyglot_dotnet_call_static(const char *assembly_name, const char *type_name,
  ...
  int rc = clr_delegate_fn_(clr_host_, clr_domain_, assembly_name, type_name, method_name,
                            (void **)&method);
  if (rc != 0 || !method)
    return NULL;
...
void *polyglot_dotnet_new_object(const char *assembly_name, const char *type_name,
  ...
  int rc = clr_delegate_fn_(clr_host_, clr_domain_, assembly_name, type_name, "__PolyglotFactory",
                            (void **)&factory);
  if (rc != 0 || !factory)
    return NULL;
...
void *polyglot_dotnet_call_method(void *object, const char *method_name, const void *const *args,
  ...
  const void **ext_args =
      (const void **)polyglot_raw_malloc((size_t)(arg_count + 1) * sizeof(void *));
  if (!ext_args)
    return NULL;
  ext_args[0] = object;
  ...
  int rc = clr_delegate_fn_(clr_host_, clr_domain_, "PolyglotInterop", "PolyglotInterop.Dispatch",
...
void polyglot_dotnet_dispose(void *object) {
  if (!clr_host_ || !clr_delegate_fn_ || !object)
    return;
...
void *__ploy_dotnet_call_static(const char *assembly_name, const char *type_name,
                                const char *method_name, const void *const *args, int arg_count) {
  return polyglot_dotnet_call_static(assembly_name, type_name, method_name, args, arg_count);
}

void *__ploy_dotnet_new_object(const char *assembly_name, const char *type_name,
                               const void *const *args, int arg_count) {
  return polyglot_dotnet_new_object(assembly_name, type_name, args, arg_count);
}

void *__ploy_dotnet_call_method(void *object, const char *method_name, const void *const *args,
                                int arg_count) {
...
void __ploy_dotnet_set_property(void *object, const char *property_name, const void *value) {
