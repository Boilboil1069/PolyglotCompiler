 ...
 *
 *             1. **Node.js embedding** â€” looked up via `napi_create_string_utf8`
 *                & friends from libnode.so / node.dll.  This is the primary
 *                supported path and gives a full ECMAScript runtime.
 *
 *             2. **Standalone fallback** â€” when no engine library can be
...
// ----------------------------------------------------------------------------
// Value descriptor â€” a tagged union so the standalone fallback can still
