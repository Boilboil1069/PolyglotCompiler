...
int polyglot_java_init(int version_hint) {
  ...
  if (!jvm_lib_) {
    fprintf(stderr,
            "[polyglot-java] error: JVM library not found at '%s'. "
  ...
  JNI_CreateJavaVM_t create_vm = (JNI_CreateJavaVM_t)JVM_SYM(jvm_lib_, "JNI_CreateJavaVM");
  if (!create_vm) {
  ...
  // Build JNI init args.
  // JavaVMInitArgs is { jint version; jint nOptions; JavaVMOption *options; jboolean
  // ignoreUnrecognized; } We only need the version field for basic initialisation.
  struct {
    jint version;
    jint nOptions;
    void *options;
  ...
  case 8:
    ...
    break; // JNI_VERSION_1_8
  case 17:
    vm_args.version = 0x00130000;
    break; // JNI_VERSION_19 (17 maps to 19)
  case 21:
    vm_args.version = 0x00150000;
    break; // JNI_VERSION_21
  case 23:
    vm_args.version = 0x00150000;
    break; // Use 21 as baseline
  default:
    vm_args.version = 0x00010008;
    break; // Default to 1.8
  }
  ...
  if (rc != 0) {
    fprintf(stderr,
            "[polyglot-java] error: JNI_CreateJavaVM failed with "
...
void *polyglot_java_call_static(const char *class_name, const char *method_name,
  ...
  if (!jni_env_ || !class_name || !method_name || !signature)
    return NULL;
  ...
  if (result) {
    ...
    if (new_ref)
      result = new_ref(jni_env_, result);
  }
...
void *polyglot_java_new_object(const char *class_name, const char *ctor_signature,
  ...
  if (!jni_env_ || !class_name || !ctor_signature)
    return NULL;
...
void *polyglot_java_call_method(void *object, const char *method_name, const char *signature,
                                const void *const *args, int arg_count) {
  if (!jni_env_ || !object || !method_name || !signature)
    return NULL;
...
void *polyglot_java_get_field(void *object, const char *field_name, const char *field_type) {
  if (!jni_env_ || !object || !field_name || !field_type)
    return NULL;
...
void *__ploy_java_call_method(void *object, const char *method_name, const char *signature,
                              const void *const *args, int arg_count) {
  return polyglot_java_call_method(object, method_name, signature, args, arg_count);
}

void *__ploy_java_get_field(void *object, const char *field_name, const char *field_type) {
  return polyglot_java_get_field(object, field_name, field_type);
}

void __ploy_java_set_field(void *object, const char *field_name, const char *field_type,
                           const void *value) {
