...
ThreadPool::ThreadPool(size_t num_threads) {
  num_threads_ =
      num_threads == 0 ? std::max<size_t>(1, std::thread::hardware_concurrency()) : num_threads;
...
void ThreadPool::Wait() {
  std::unique_lock<std::mutex> lock(mutex_);
  cv_.wait(lock,
           [this]() { return tasks_.empty() && active_.load(std::memory_order_relaxed) == 0; });
}
