...
struct TokenInfo {
  ...
  size_t length{0};
  std::string
      kind; // "keyword", "identifier", "number", "string", "comment", "operator", "type", "builtin"
  std::string lexeme;
};
...
public:
  ...
  std::vector<TokenInfo> Tokenize(const std::string &source, const std::string &language) const;

  // Analyze source code and return diagnostics (errors + warnings)
  std::vector<DiagnosticInfo> Analyze(const std::string &source, const std::string &language,
...
private:
  // Internal helpers
  std::vector<DiagnosticInfo> ConvertDiagnostics(const frontends::Diagnostics &diags) const;
