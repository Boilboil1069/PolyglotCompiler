...
public:
  ...
  TopoPortItem(uint64_t port_id, Direction direction, const QString &name, const QString &type_name,
...
public:
  TopoNodeItem(uint64_t node_id, const QString &name, const QString &language, const QString &kind,
  ...
  QPointF AddOutputPort(uint64_t port_id, const QString &name, const QString &type_name);
...
public:
  TopoEdgeItem(uint64_t edge_id, const QPointF &start, const QPointF &end, const QString &status,
               QGraphicsItem *parent = nullptr);
  ...
  void SetEndpointIds(uint64_t src_node, uint64_t src_port, uint64_t tgt_node, uint64_t tgt_port);
...
public:
   ...
   */
  explicit DrillDownWindow(uint64_t container_node_id, const QString &container_name,
