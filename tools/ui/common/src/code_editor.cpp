...
CodeEditor::CodeEditor(QWidget *parent) : QPlainTextEdit(parent) {
  ...
  connect(minimap_, &MinimapWidget::ScrollRequested, this, [this](int line) {
  ...
  connect(this, &CodeEditor::textChanged, minimap_, &MinimapWidget::UpdateContent);
  connect(verticalScrollBar(), &QScrollBar::valueChanged, minimap_,
          QOverload<>::of(&MinimapWidget::update));
...
void CodeEditor::resizeEvent(QResizeEvent *event) {
  ...
  if (minimap_visible_) {
    minimap_->setGeometry(
        QRect(cr.right() - MinimapWidget::kWidth, cr.top(), MinimapWidget::kWidth, cr.height()));
    minimap_->show();
  } else {
    minimap_->hide();
  }
...
void CodeEditor::ApplyTheme() {
  const auto &tc = ThemeManager::Instance().Active();
  setStyleSheet(QString("QPlainTextEdit { background: %1; border: none; "
...
void CodeEditor::keyPressEvent(QKeyEvent *event) {
  ...
  if (completion_popup_->isVisible()) {
    ...
    if (event->key() == Qt::Key_Down) {
      ...
      if (row + 1 < count) {
        completion_popup_->setCurrentIndex(completion_popup_->Model()->index(row + 1, 0));
      }
      return;
    }
  ...
  if (auto_indent_enabled_ && (event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter)) {
    QTextCursor cursor = textCursor();
    ...
    if (trimmed.endsWith('{') || trimmed.endsWith(':')) {
      whitespace +=
          insert_spaces_ ? QString(tab_width_spaces_, QLatin1Char(' ')) : QStringLiteral("\t");
    }
...
void CodeEditor::TriggerCompletion() {
  ...
  if (compiler_service_) {
    ...
    auto compiler_items =
        compiler_service_->Complete(toPlainText().toStdString(), language_, line, col);

    // Filter by prefix (case-insensitive)
    std::string pfx = prefix.toStdString();
    std::string lower_pfx = pfx;
    std::transform(lower_pfx.begin(), lower_pfx.end(), lower_pfx.begin(), ::tolower);
  ...
  while (block.isValid() && top <= event->rect().bottom()) {
    if (block.isVisible() && bottom >= event->rect().top()) {
      ...
      for (const auto &diag : diagnostics_) {
        if (static_cast<int>(diag.line) != line)
          continue;
        ...
        QRectF wave_rect(x_start + contentOffset().x(), static_cast<double>(top), x_end - x_start,
                         fm.height());
        ...
        QString hint = QString::fromStdString(" \u2190 " + diag.message);
        if (hint.length() > 60) {
          hint = hint.left(57) + "...";
        }
        ...
        painter.drawText(
            QPointF(text_end_x + contentOffset().x(), static_cast<double>(top) + fm.ascent()),
            hint);
...
void CodeEditor::mousePressEvent(QMouseEvent *event) {
  if (goto_def_enabled_ && event->button() == Qt::LeftButton &&
...
CompletionPopup::CompletionPopup(QWidget *parent) : QListView(parent) {
  setModel(&model_);
  ...
  // Styling
  setStyleSheet("QListView { background: #252526; color: #cccccc; border: 1px solid #454545; "
                "font-family: monospace; font-size: 10pt; }"
                "QListView::item:selected { background: #094771; }");
}

void CompletionPopup::ShowCompletions(const std::vector<CompletionItem> &items, const QPoint &pos) {
...
MinimapWidget::MinimapWidget(CodeEditor *editor) : QWidget(editor), editor_(editor) {
...
void MinimapWidget::paintEvent(QPaintEvent * /*event*/) {
  ...
  while (last_block.isValid() && last_block.isVisible()) {
    QRectF geom = editor_->BlockBoundingGeometry(last_block).translated(editor_->ContentOffset());
    if (geom.top() > editor_->viewport()->height())
      break;
  ...
  if (cache_dirty_) {
    ...
    while (block.isValid()) {
      ...
      if (!text.trimmed().isEmpty()) {
        ...
        int bar_len =
            qMin(static_cast<int>(static_cast<double>(visible_chars) / 120.0 * kWidth), kWidth);
        // Indent-aware: skip leading whitespace in minimap
        int leading = 0;
        while (leading < text.length() && text[leading].isSpace())
          ++leading;
        int x_start = static_cast<int>(static_cast<double>(leading) / 120.0 * kWidth);
