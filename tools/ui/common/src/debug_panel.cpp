...
void DebugPanel::SetupBreakpointsView() {
  ...
  connect(breakpoint_tree_, &QTreeWidget::itemChanged, this, &DebugPanel::OnBreakpointItemChanged);
  connect(breakpoint_tree_, &QTreeWidget::itemDoubleClicked, this,
          &DebugPanel::OnBreakpointDoubleClicked);
  connect(breakpoint_tree_, &QTreeWidget::customContextMenuRequested, this,
          &DebugPanel::OnBreakpointContextMenu);
...
void DebugPanel::SetupCallStackView() {
  ...
  connect(callstack_tree_, &QTreeWidget::itemClicked, this, &DebugPanel::OnStackFrameClicked);
...
void DebugPanel::SetupVariablesView() {
  ...
  connect(variables_tree_, &QTreeWidget::itemExpanded, this, &DebugPanel::OnVariableExpanded);
...
void DebugPanel::SetupWatchView() {
  ...
  connect(watch_tree_, &QTreeWidget::customContextMenuRequested, this, [this](const QPoint &pos) {
    auto *item = watch_tree_->itemAt(pos);
    if (!item)
      return;

    QMenu menu(this);
    menu.setStyleSheet("QMenu { background: #252526; color: #cccccc; border: 1px solid #454545; }"
  ...
  add_watch_button_->setFixedWidth(28);
  {
    ...
    add_watch_button_->setStyleSheet(
        QString("QPushButton { background: %1; color: %2; border: none; "
            ...
            .arg(tc.accent.name(), tc.button_primary_text.name(), tc.accent_hover.name()));
  }
  ...
  console_input_->setPlaceholderText("Debug command...");
  {
    ...
    console_input_->setStyleSheet(
        QString("QLineEdit { background: %1; color: %2; border: none; "
            ...
            .arg(tc.input_background.name(), tc.input_text.name(), tc.input_border.name()));
  }
...
void DebugPanel::ToggleBreakpoint(const QString &file, int line) {
  ...
  if (IsDebugging() && IsPaused()) {
    SendDebuggerCommand(QString("breakpoint set --file %1 --line %2").arg(file).arg(line));
  }
...
void DebugPanel::SetBreakpointCondition(const QString &file, int line, const QString &condition) {
  for (auto &bp : breakpoints_) {
    if (bp.file == file && bp.line == line) {
      bp.condition = condition;
      if (IsDebugging()) {
        SendDebuggerCommand(QString("breakpoint modify -c '%1' %2").arg(condition).arg(bp.id));
      }
      RefreshBreakpointTree();
      emit BreakpointsChanged();
      return;
...
void DebugPanel::StartDebug() {
  ...
  connect(debugger_process_, &QProcess::readyReadStandardOutput, this,
          &DebugPanel::OnDebuggerReadyRead);
  connect(debugger_process_, &QProcess::errorOccurred, this, &DebugPanel::OnDebuggerError);
  connect(debugger_process_, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
          &DebugPanel::OnDebuggerFinished);
  ...
  console_output_->appendPlainText(">>> " + debugger_path_ + " " + args.join(" ") + "\n");
...
void DebugPanel::RunToCursor(const QString &file, int line) {
  ...
  if (is_lldb) {
    SendDebuggerCommand(
        QString("breakpoint set --one-shot true --file %1 --line %2").arg(file).arg(line));
...
void DebugPanel::OnDebuggerError() {
  if (!debugger_process_)
    return;
  console_output_->appendPlainText("[Error] " + debugger_process_->errorString() + "\n");
}

void DebugPanel::OnDebuggerFinished(int exit_code, QProcess::ExitStatus /*status*/) {
  console_output_->appendPlainText(QString("\n--- Debugger exited (code %1) ---\n").arg(exit_code));
...
void DebugPanel::ParseDebuggerOutput(const QString &output) {
  ...
  if (is_lldb) {
    ...
    static const QRegularExpression stop_re(R"(stop reason = (?:breakpoint|step|signal))");
    static const QRegularExpression frame_re(R"(at\s+(.+?):(\d+))");
    ...
    while (bt_iter.hasNext()) {
      ...
      auto *item = new QTreeWidgetItem(
          {QString::number(sf.index), sf.function_name, sf.file, QString::number(sf.line)});
    ...
    static const QRegularExpression var_re(R"(\(([^)]+)\)\s+(\w+)\s+=\s+(.+))");
    ...
    static const QRegularExpression expr_result_re(R"(\(([^)]+)\)\s+\$\d+\s+=\s+(.+))");
    ...
    static const QRegularExpression gdb_file_re(R"RE(fullname="(.+?)",line="(\d+)")RE");
    ...
    if (gdb_stop_re.match(output).hasMatch()) {
      ...
      if (file_match.hasMatch()) {
        emit DebugLocationChanged(file_match.captured(1), file_match.captured(2).toInt());
      }
    ...
    static const QRegularExpression gdb_eval_re(R"RE(\^done,value="(.+?)")RE");
...
void DebugPanel::SetBreakpointsInDebugger() {
  ...
  for (auto &bp : breakpoints_) {
    ...
    if (is_lldb) {
      QString cmd = QString("breakpoint set --file %1 --line %2").arg(bp.file).arg(bp.line);
      ...
      SendDebuggerCommand(cmd);
      if (!bp.condition.isEmpty()) {
        SendDebuggerCommand(QString("-break-condition %1 %2").arg(bp.id).arg(bp.condition));
      }
    }
  }
}
...
void DebugPanel::OnBreakpointItemChanged(QTreeWidgetItem *item, int column) {
  if (column != 0)
    return; // only care about checkbox
  ...
  for (auto &bp : breakpoints_) {
    if (bp.id == bp_id) {
      ...
      if (IsDebugging()) {
        ...
        if (is_lldb) {
          SendDebuggerCommand(
              QString("breakpoint %1 %2").arg(enabled ? "enable" : "disable").arg(bp.id));
        } else {
          SendDebuggerCommand(
              QString(enabled ? "-break-enable %1" : "-break-disable %1").arg(bp.id));
        }
      }
      break;
    }
...
void DebugPanel::OnBreakpointContextMenu(const QPoint &pos) {
  ...
  QMenu menu(this);
  {
    ...
    menu.setStyleSheet(QString("QMenu { background: %1; color: %2; border: 1px solid %3; }"
                           ...
                           .arg(tc.menu_background.name(), tc.menu_text.name(), tc.border.name(),
                                tc.menu_hover.name()));
  }
  ...
  menu.addAction("Edit Condition...", [this, bp_id]() {
    for (auto &bp : breakpoints_) {
      if (bp.id == bp_id) {
        bool ok = false;
        QString cond = QInputDialog::getText(this, "Breakpoint Condition",
                                             "Stop when condition is true:", QLineEdit::Normal,
                                             bp.condition, &ok);
        if (ok) {
  ...
  for (int i = 0; i < static_cast<int>(watch_expressions_.size()); ++i) {
    ...
    if (we.value == "<evaluating...>" || we.value == "<not evaluated>") {
      ...
      if (i < watch_tree_->topLevelItemCount()) {
        ...
        if (value.startsWith("0x") || value.contains("nil") || value.contains("null") ||
            value.contains("NULL")) {
