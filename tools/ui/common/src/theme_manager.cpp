...
QString ThemeManager::MenuBarStylesheet() const {
  const auto &t = Active();
  return QString("QMenuBar { background: %1; color: %2; padding: 2px; }"
      ...
      .arg(t.surface.name(), t.text.name(), t.button_hover.name(), t.menu_background.name(),
           t.menu_text.name(), t.border.name(), t.menu_hover.name(), t.menu_separator.name());
}

QString ThemeManager::ToolBarStylesheet() const {
  const auto &t = Active();
  return QString("QToolBar { background: %1; border: none; spacing: 4px; padding: 2px; }"
      ...
      .arg(t.surface.name(), t.text.name(), t.button_hover.name(), t.accent_pressed.name(),
           t.text_disabled.name());
}

QString ThemeManager::StatusBarStylesheet() const {
  const auto &t = Active();
  return QString("QStatusBar { background: %1; color: %2; font-size: 12px; }"
                 "QStatusBar::item { border: none; }"
                 "QLabel { color: %3; padding: 0px 8px; }")
      .arg(t.statusbar_background.name(), t.statusbar_text.name(), t.statusbar_text.name());
}
...
QString ThemeManager::TabWidgetStylesheet(bool bottom_tabs) const {
  ...
  return QString("QTabWidget::pane { border: none; background: %1; }"
      ...
      .arg(min_width)
      .arg(font_sz)
      .arg(t.tab_active.name(), t.tab_active_text.name(), border_side,
           t.tab_active_indicator.name(), t.tab_hover.name());
}
...
QString ThemeManager::TreeWidgetStylesheet() const {
  ...
  return QString("QTreeWidget { background: %1; color: %2; border: none; }"
      ...
      .arg(t.background.name(), t.text.name(), t.selection.name(), t.surface.name(), t.text.name());
}
...
QString ThemeManager::PushButtonPrimaryStylesheet() const {
  ...
  return QString("QPushButton { background: %1; color: %2; border: none; "
      ...
      .arg(t.button_primary.name(), t.button_primary_text.name(), t.button_primary_hover.name(),
           t.accent_pressed.name());
}

QString ThemeManager::PlainTextEditStylesheet() const {
  const auto &t = Active();
  return QString("QPlainTextEdit { background: %1; color: %2; border: none; "
...
QString ThemeManager::EditorStylesheet() const {
  ...
  return QString("QPlainTextEdit { background: %1; color: %2; border: none; "
      ...
      .arg(t.editor_background.name(), t.editor_text.name(), t.editor_selection.name());
}

QString ThemeManager::GroupBoxStylesheet() const {
  const auto &t = Active();
  return QString("QGroupBox { color: %1; font-weight: bold; border: 1px solid %2; "
...
QString ThemeManager::CheckBoxStylesheet() const {
  const auto &t = Active();
  return QString("QCheckBox { color: %1; spacing: 6px; }"
...
QString ThemeManager::SpinBoxStylesheet() const {
  const auto &t = Active();
  return QString("QSpinBox { background: %1; color: %2; border: 1px solid %3; "
...
QString ThemeManager::ProgressBarStylesheet() const {
  const auto &t = Active();
  return QString("QProgressBar { background: %1; border: none; }"
...
QString ThemeManager::ListWidgetStylesheet() const {
  const auto &t = Active();
  return QString("QListWidget { background: %1; color: %2; border: none; "
                 "font-size: 13px; outline: none; }"
                 "QListWidget::item { padding: 10px 15px; }"
                 "QListWidget::item:selected { background: %3; color: %4; }"
                 "QListWidget::item:hover { background: %5; }")
      .arg(t.surface_alt.name(), t.text.name(), t.selection.name(), t.selection_text.name(),
           t.tab_hover.name());
}
