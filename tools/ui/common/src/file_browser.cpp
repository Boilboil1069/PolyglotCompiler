...
void FileBrowser::SetupUi() {
  ...
  title_label_ = new QLabel("  EXPLORER", this);
  {
    ...
    title_label_->setStyleSheet(QString("QLabel { color: %1; font-size: 11px; font-weight: bold; "
                                        "padding: 4px 0px; }")
                                    .arg(tc.text_secondary.name()));
  }
  ...
  connect(filter_edit_, &QLineEdit::textChanged, this, &FileBrowser::OnFilterTextChanged);
  ...
  {
    const auto &tc = ThemeManager::Instance().Active();
    tree_view_->setStyleSheet(QString("QTreeView { background: %1; color: %2; border: none; "
                                      "font-size: 13px; }"
                                      "QTreeView::item { padding: 2px 0px; }"
                                      "QTreeView::item:hover { background: %3; }"
                                      "QTreeView::item:selected { background: %4; color: %5; }")
                                  .arg(tc.background.name(), tc.text.name(), tc.surface_alt.name(),
                                       tc.selection.name(), tc.selection_text.name()));
  }

  // Enable custom context menu
  tree_view_->setContextMenuPolicy(Qt::CustomContextMenu);
  connect(tree_view_, &QTreeView::customContextMenuRequested, this,
          &FileBrowser::OnContextMenuRequested);

  layout_->addWidget(tree_view_);

  connect(tree_view_, &QTreeView::doubleClicked, this, &FileBrowser::OnItemDoubleClicked);
}
...
void FileBrowser::SetupContextMenu() {
  ...
  connect(action_open_, &QAction::triggered, this,
          [this]() { OnContextMenuRequested(tree_view_->mapFromGlobal(QCursor::pos())); });
...
void FileBrowser::OnContextMenuRequested(const QPoint &pos) {
  ...
  connect(action_new_file_, &QAction::triggered, this, [this, index]() { ContextNewFile(index); });
  connect(action_new_folder_, &QAction::triggered, this,
          [this, index]() { ContextNewFolder(index); });
  connect(action_rename_, &QAction::triggered, this, [this, index]() { ContextRename(index); });
  connect(action_delete_, &QAction::triggered, this, [this, index]() { ContextDelete(index); });
  connect(action_copy_path_, &QAction::triggered, this,
          [this, index]() { ContextCopyPath(index); });
  connect(action_copy_relative_path_, &QAction::triggered, this,
          [this, index]() { ContextCopyRelativePath(index); });
  connect(action_reveal_explorer_, &QAction::triggered, this,
          [this, index]() { ContextRevealInExplorer(index); });
  connect(action_open_terminal_, &QAction::triggered, this,
          [this, index]() { ContextOpenTerminal(index); });
  connect(action_generate_topology_, &QAction::triggered, this,
          [this, index]() { ContextGenerateTopology(index); });
  connect(action_new_from_template_, &QAction::triggered, this,
          [this, index]() { emit NewFromTemplateRequested(DirectoryForIndex(index)); });
...
void FileBrowser::ContextNewFile(const QModelIndex &index) {
  ...
  if (!file.open(QIODevice::WriteOnly)) {
    QMessageBox::critical(this, "New File", "Failed to create file: " + file.errorString());
    return;
  }
  file.close();
...
void FileBrowser::ContextNewFolder(const QModelIndex &index) {
  ...
  bool ok = false;
  QString name =
      QInputDialog::getText(this, "New Folder", "Folder name:", QLineEdit::Normal, "", &ok);
  if (!ok || name.isEmpty())
    return;
  ...
  if (!parent.mkdir(name)) {
    QMessageBox::critical(this, "New Folder", "Failed to create folder.");
    return;
  }
...
void FileBrowser::ContextRename(const QModelIndex &index) {
  ...
  bool ok = false;
  QString new_name =
      QInputDialog::getText(this, "Rename", "New name:", QLineEdit::Normal, old_name, &ok);
  ...
  if (QFileInfo::exists(new_path)) {
    QMessageBox::warning(this, "Rename", "An item with that name already exists.");
    return;
  }
