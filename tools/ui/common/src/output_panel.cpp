...
void OutputPanel::SetupUi() {
  ...
  error_table_->setHorizontalHeaderLabels({"Severity", "Code", "Line", "Column", "Message"});
  ...
  error_table_->verticalHeader()->setVisible(false);
  {
    ...
    error_table_->setStyleSheet(QString("QTableWidget { background: %1; color: %2; border: none; "
  ...
  connect(error_table_, &QTableWidget::cellDoubleClicked, this,
          &OutputPanel::OnErrorRowDoubleClicked);
...
void OutputPanel::ShowDiagnostics(const std::vector<DiagnosticInfo> &diagnostics,
  ...
  for (const auto &d : diagnostics) {
    ...
    error_table_->setItem(row, 1, new QTableWidgetItem(QString::fromStdString(d.code)));
    error_table_->setItem(row, 2, new QTableWidgetItem(QString::number(d.line)));
    error_table_->setItem(row, 3, new QTableWidgetItem(QString::number(d.column)));
    error_table_->setItem(row, 4, new QTableWidgetItem(QString::fromStdString(d.message)));
  }
...
void OutputPanel::OnErrorRowDoubleClicked(int row, int /*column*/) {
  ...
  QString file = severity_item ? severity_item->data(Qt::UserRole).toString() : QString();
