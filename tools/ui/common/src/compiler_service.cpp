...
static std::string ClassifyToken(frontends::TokenKind kind, const std::string &lexeme) {
  ...
  case frontends::TokenKind::kKeyword: {
    ...
    if (ploy_type_keywords.count(lexeme))
      return "type";
    if (ploy_literal_keywords.count(lexeme))
      return "builtin";
    return "keyword";
  }
  case frontends::TokenKind::kIdentifier: {
    ...
    static const std::unordered_set<std::string> builtins = {
        // Python builtins
        "print", "println", "len", "range", "enumerate", "zip", "map", "filter", "sorted", "type",
...
std::vector<DiagnosticInfo> CompilerService::ConvertDiagnostics(
  ...
  for (const auto &d : diags.All()) {
    ...
    switch (d.severity) {
    case frontends::DiagnosticSeverity::kError:
      info.severity = "error";
      break;
    case frontends::DiagnosticSeverity::kWarning:
      info.severity = "warning";
      break;
    case frontends::DiagnosticSeverity::kNote:
      info.severity = "note";
      break;
    }
...
std::vector<CompletionItem> CompilerService::Complete(const std::string &source,
  ...
  static const std::unordered_map<std::string, std::vector<std::string>> lang_keywords = {
      {"cpp",
       {"auto",     "break",   "case",      "catch",       "class",    "const",    "constexpr",
        ...
        "override", "private", "protected", "public",      "register", "return",   "short",
      ...
      {"python",
       {"and",      "as",     "assert", "async",  "await",  "break",  "class",   "continue",
        ...
        "while",    "with",   "yield",  "print",  "range",  "len",    "list",    "dict",
        "set",      "tuple",  "int",    "float",  "str",    "bool",   "type",    "isinstance"}},
      {"rust", {"as",     "async",  "await",  "break",    "const",     "continue", "crate", "dyn",
                ...
                "true",   "type",   "unsafe", "use",      "where",     "while",    "Vec",   "Box",
                "Option", "Result", "String", "println!", "eprintln!", "format!",  "vec!"}},
      {"java",
      ...
      {"ruby",
       {"BEGIN",      "END",   "alias",    "and",  "begin",   "break",         "case",
        ...
        "when",       "while", "yield",    "puts", "require", "attr_accessor", "attr_reader",
        "attr_writer"}},
      {"go",
       {"break",       "case",    "chan",   "const",  "continue", "default", "defer",   "else",
        "fallthrough", "for",     "func",   "go",     "goto",     "if",      "import",  "interface",
        "map",         "package", "range",  "return", "select",   "struct",  "switch",  "type",
        "var",         "true",    "false",  "nil",    "iota",     "int",     "int8",    "int16",
        "int32",       "int64",   "uint",   "uint8",  "uint16",   "uint32",  "uint64",  "float32",
        "float64",     "string",  "bool",   "byte",   "rune",     "error",   "make",    "new",
        "len",         "cap",     "append", "copy",   "delete",   "panic",   "recover", "fmt"}},
  };
...
std::vector<CompletionItem> CompilerService::GetPloyCompletions(const std::string &source,
                                                                size_t /*line*/,
                                                                size_t /*column*/) const {
  std::vector<CompletionItem> result;
  ...
  static const std::vector<CompletionItem> ploy_keywords = {
      {"LINK", "keyword", "Link external function",
       "LINK ${1:lang}::${2:module}::${3:func} AS FUNC(${4:params}) -> ${5:return_type};"},
      ...
      {"WITH", "keyword", "Resource management block",
      ...
      {"MATCH", "keyword", "Match expression",
       "MATCH ${1:value} {\n    CASE ${2:pattern}: ${3}\n    DEFAULT: ${4}\n}"},
...
void CompilerService::ExtractSourceSymbols(const std::string &source,
  ...
  while (std::getline(stream, line_text)) {
    ...
    if (first_non_space == std::string::npos)
      continue;
    if (line_text.size() > first_non_space + 1 && line_text[first_non_space] == '/' &&
        line_text[first_non_space + 1] == '/')
      continue;
    ...
    if (tok == "LINK") {
      ...
      std::string short_name =
          (last_sep != std::string::npos) ? qualified.substr(last_sep + 2) : qualified;
    ...
    if (tok == "IMPORT") {
      ...
      if (semi != std::string::npos)
        pkg = pkg.substr(0, semi);
      auto selectors = pkg.find("::");
      if (selectors != std::string::npos)
        pkg = pkg.substr(0, selectors);
      auto ge = pkg.find(">=");
  ...
  while (std::getline(stream, line_text)) {
    ...
    if (language == "ploy") {
      ...
      if (tok == "FUNC" || tok == "PIPELINE" || tok == "STRUCT") {
        ...
        std::string short_name = (last_sep != std::string::npos) ? name.substr(last_sep + 2) : name;
        auto dot = short_name.find('.');
        if (dot != std::string::npos)
          short_name = short_name.substr(0, dot);
        if (!short_name.empty()) {
      ...
      if (std::regex_search(trimmed, m, class_re)) {
        ...
        static const std::unordered_set<std::string> skip = {"if",     "while",  "for",
                                                             "switch", "return", "catch"};
        if (!skip.count(name)) {
          workspace_index_[name].push_back(
      ...
      if (std::regex_search(trimmed, m, def_re)) {
        workspace_index_[m[1].str()].push_back(
            {m[1].str(), path, line_num, "function", language, "def " + m[1].str()});
      } else if (std::regex_search(trimmed, m, cls_re)) {
        workspace_index_[m[1].str()].push_back(
            {m[1].str(), path, line_num, "type", language, "class " + m[1].str()});
      }
      ...
      if (std::regex_search(trimmed, m, fn_re)) {
        workspace_index_[m[1].str()].push_back(
            {m[1].str(), path, line_num, "function", language, "fn " + m[1].str()});
      } else if (std::regex_search(trimmed, m, struct_re)) {
        workspace_index_[m[1].str()].push_back(
            {m[1].str(), path, line_num, "type", language, "struct " + m[1].str()});
      } else if (std::regex_search(trimmed, m, impl_re)) {
        workspace_index_[m[1].str()].push_back(
            {m[1].str(), path, line_num, "type", language, "impl " + m[1].str()});
      }
      ...
      if (std::regex_search(trimmed, m, cls_re)) {
        ...
        if (!skip.count(m[1].str())) {
          workspace_index_[m[1].str()].push_back(
              {m[1].str(), path, line_num, "function", language, m[1].str() + "(...)"});
        }
      }
      ...
      std::string trimmed = line_text.substr(first);
      static const std::regex fn_re(R"((?:async\s+)?function\s+(\w+)\s*\()");
      static const std::regex cls_re(R"(class\s+(\w+))");
      ...
      if (std::regex_search(trimmed, m, fn_re)) {
        workspace_index_[m[1].str()].push_back(
            {m[1].str(), path, line_num, "function", language, "function " + m[1].str()});
      } else if (std::regex_search(trimmed, m, cls_re)) {
        workspace_index_[m[1].str()].push_back(
            {m[1].str(), path, line_num, "type", language, "class " + m[1].str()});
      ...
      if (std::regex_search(trimmed, m, def_re)) {
        ...
        workspace_index_[m[1].str()].push_back(
            {m[1].str(), path, line_num, "type", language, "module " + m[1].str()});
      }
    } else if (language == "go") {
      std::string trimmed = line_text.substr(first);
      static const std::regex fn_re(R"(func\s+(?:\([^)]*\)\s+)?(\w+)\s*\()");
      static const std::regex type_re(R"(type\s+(\w+)\s+)");
      std::smatch m;
      if (std::regex_search(trimmed, m, fn_re)) {
        workspace_index_[m[1].str()].push_back(
            {m[1].str(), path, line_num, "function", language, "func " + m[1].str()});
      } else if (std::regex_search(trimmed, m, type_re)) {
        workspace_index_[m[1].str()].push_back(
            {m[1].str(), path, line_num, "type", language, "type " + m[1].str()});
      }
    }
  }
...
CompilerService::DefinitionLocation CompilerService::FindDefinition(
  ...
  if (language == "ploy") {
    ...
    def_patterns = {"function " + symbol, "class " + symbol, "const " + symbol, "let " + symbol,
                    "var " + symbol};
  } else if (language == "ruby") {
    def_patterns = {"def " + symbol, "class " + symbol, "module " + symbol};
  } else if (language == "go") {
    def_patterns = {"func " + symbol, "type " + symbol, "var " + symbol, "const " + symbol};
  }
