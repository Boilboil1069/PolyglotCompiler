...
TerminalInputFilter::TerminalInputFilter(TerminalWidget *terminal, QPlainTextEdit *output,
                                         QObject *parent) :
...
void TerminalWidget::SetupUi() {
  ...
  // Dark terminal theme
  output_area_->setStyleSheet("QPlainTextEdit {"
  ...
  connect(filter, &TerminalInputFilter::KeyPressed, this, &TerminalWidget::HandleInputKey);
}
...
void TerminalWidget::StartShell() {
  if (!process_) {
    ...
    connect(process_, &QProcess::readyReadStandardOutput, this, &TerminalWidget::OnReadyReadStdout);
    connect(process_, &QProcess::readyReadStandardError, this, &TerminalWidget::OnReadyReadStderr);
    connect(process_, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
            &TerminalWidget::OnProcessFinished);
    connect(process_, &QProcess::errorOccurred, this, &TerminalWidget::OnProcessError);
  }
...
void TerminalWidget::OnProcessError(QProcess::ProcessError error) {
  static const char *kErrorNames[] = {"FailedToStart", "Crashed",    "Timedout",
                                      "ReadError",     "WriteError", "UnknownError"};
...
QString TerminalWidget::ParseAnsiToHtml(const QString &raw) const {
  ...
  static const char *kFgColours[] = {"#000000", "#cd3131", "#0dbc79", "#e5e510",
                                     "#2472c8", "#bc3fbc", "#11a8cd", "#e5e5e5"};
  static const char *kBrightFgColours[] = {"#666666", "#f14c4c", "#23d18b", "#f5f543",
  ...
  while (it.hasNext()) {
    ...
    if (match_start > pos) {
      ...
      result += QString("<span style=\"color:%1;%2\">%3</span>")
                    .arg(fg, bold ? "font-weight:bold;" : "", segment);
    }
  ...
  if (pos < raw.size()) {
    QString segment =
        raw.mid(pos).toHtmlEscaped().replace("\n", "<br>").replace("\r", "").replace(" ", "&nbsp;");
    result += QString("<span style=\"color:%1;%2\">%3</span>")
                  .arg(fg, bold ? "font-weight:bold;" : "", segment);
  }
...
QString TerminalWidget::GetCurrentInput() const {
  ...
  if (prompt_position_ >= all_text.size())
    return {};
