...
void GitPanel::SetupUi() {
  ...
  connect(branch_combo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &GitPanel::OnBranchChanged);
...
void GitPanel::SetupStatusView() {
  ...
  connect(status_tree_, &QTreeWidget::itemDoubleClicked, this,
          &GitPanel::OnStatusItemDoubleClicked);
  connect(status_tree_, &QTreeWidget::customContextMenuRequested, this,
          &GitPanel::OnStatusContextMenu);
...
void GitPanel::RunGitAsync(const QStringList &args,
  ...
  connect(async_process_, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
          [this, callback](int exit_code, QProcess::ExitStatus) {
            GitResult result;
...
void GitPanel::UpdateStatusTree() {
  ...
  for (const auto &fs : file_statuses_) {
    ...
    QColor color;
    if (fs.status == "M")
      color = QColor("#e8a838"); // Modified
    else if (fs.status == "A")
      color = QColor("#73c991"); // Added
    else if (fs.status == "D")
      color = QColor("#f44747"); // Deleted
    else if (fs.status == "R")
      color = QColor("#569cd6"); // Renamed
    else if (fs.status == "?")
      color = QColor("#999999"); // Untracked
    else
      color = QColor("#cccccc");
...
void GitPanel::RefreshLog() {
  auto result = RunGit(
      {"log", "--oneline", "--no-color", "-50", "--format=%H|%h|%an|%ad|%s", "--date=short"});
  if (!result.success)
    return;
...
void GitPanel::OnDiscardChanges() {
  ...
  QString path = item->data(0, Qt::UserRole).toString();
  auto confirm =
      QMessageBox::question(this, "Discard Changes",
...
void GitPanel::OnCreateBranch() {
  bool ok = false;
  QString name =
      QInputDialog::getText(this, "New Branch", "Branch name:", QLineEdit::Normal, QString(), &ok);
  if (!ok || name.isEmpty())
    return;
...
void GitPanel::OnDeleteBranch() {
  ...
  auto confirm =
      QMessageBox::question(this, "Delete Branch", QString("Delete branch '%1'?").arg(branch),
                            QMessageBox::Yes | QMessageBox::No);
  if (confirm != QMessageBox::Yes)
    return;
...
void GitPanel::OnMergeBranch() {
  ...
  QString branch = QInputDialog::getItem(
      this, "Merge Branch", "Merge into " + current_branch_ + ":", branches, 0, false, &ok);
...
void GitPanel::OnLogEntryClicked(QListWidgetItem *item) {
  ...
  if (idx < 0 || idx >= static_cast<int>(log_entries_.size()))
    return;
...
void GitPanel::OnStatusContextMenu(const QPoint &pos) {
  ...
  menu.addSeparator();
  menu.addAction("Open File", [this, path]() { emit FileOpenRequested(repo_path_ + "/" + path); });
  menu.addAction("Show Diff", [this, item]() { OnStatusItemDoubleClicked(item, 0); });
