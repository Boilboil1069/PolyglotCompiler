...
void PanelManager::RegisterPanel(const QString &id, QWidget *widget, const QString &title) {
  PanelEntry entry;
  entry.id = id;
  entry.widget = widget;
  entry.title = title;
...
bool PanelManager::IsPanelActive(const QString &id) const {
  if (!tab_widget_ || !tab_widget_->isVisible())
    return false;
  auto *entry = FindPanel(id);
  if (!entry || !entry->widget)
    return false;
...
void PanelManager::RegisterPluginPanel(const QString &plugin_id, const QString &panel_id,
...
void PanelManager::RemovePluginPanels(const QString &plugin_id) {
  ...
  auto it = std::remove_if(panels_.begin(), panels_.end(), [&](const PanelEntry &entry) {
