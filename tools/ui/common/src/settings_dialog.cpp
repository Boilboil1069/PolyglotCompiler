...
void SettingsDialog::SetupUi() {
  ...
  pages_ = new QStackedWidget();
  {
    ...
    pages_->setStyleSheet(tm.GroupBoxStylesheet() + tm.LabelStylesheet() + tm.LineEditStylesheet() +
                          tm.SpinBoxStylesheet() + tm.ComboBoxStylesheet() +
                          tm.CheckBoxStylesheet() + tm.PushButtonPrimaryStylesheet());
  }
  ...
  connect(button_box->button(QDialogButtonBox::Ok), &QPushButton::clicked, this,
          &SettingsDialog::OnOk);
  connect(button_box->button(QDialogButtonBox::Cancel), &QPushButton::clicked, this,
          &QDialog::reject);
  ...
  connect(button_box->button(QDialogButtonBox::RestoreDefaults), &QPushButton::clicked, this,
          &SettingsDialog::OnResetDefaults);
...
QWidget *SettingsDialog::CreateCompilerPage() {
  ...
  default_language_combo_->addItems(
...
QWidget *SettingsDialog::CreateBuildPage() {
  ...
  build_generator_combo_->addItems(
      {"Ninja", "Unix Makefiles", "Visual Studio 17 2022", "Xcode", "MinGW Makefiles"});
...
QWidget *SettingsDialog::CreateDebugPage() {
  ...
  connect(dbg_browse, &QPushButton::clicked, this, [this]() {
    ...
    if (!path.isEmpty())
      debugger_path_edit_->setText(path);
  });
...
QWidget *SettingsDialog::CreateKeybindingsPage() {
  ...
  keybinding_tree_->setStyleSheet(ThemeManager::Instance().TreeWidgetStylesheet() +
  ...
  for (const auto &entry : keybinding_entries_) {
    auto *item = new QTreeWidgetItem(
        {entry.display_name,
         entry.custom_shortcut.isEmpty() ? entry.default_shortcut.toString(QKeySequence::NativeText)
    ...
    if (!entry.custom_shortcut.isEmpty() && entry.custom_shortcut != entry.default_shortcut) {
  ...
  connect(keybinding_tree_, &QTreeWidget::currentItemChanged, this,
          [this](QTreeWidgetItem *current, QTreeWidgetItem *) {
            ...
            for (const auto &entry : keybinding_entries_) {
              if (entry.action_id == action_id) {
                QKeySequence seq = entry.custom_shortcut.isEmpty() ? entry.default_shortcut
...
void SettingsDialog::LoadSettings() {
  ...
  highlight_current_line_check_->setChecked(
      s.value("editor/highlight_current_line", true).toBool());
...
void SettingsDialog::OnResetDefaults() {
  auto result = QMessageBox::question(
      this, "Reset to Defaults", "This will reset all settings to their default values. Continue?",
...
QWidget *SettingsDialog::CreatePluginsPage() {
  ...
  plugin_tree_->setHeaderLabels(
      {"Name", "Version", "Author", "Status", "Capabilities", "Load Order"});
  ...
  // -- Signals --
  connect(plugin_tree_, &QTreeWidget::currentItemChanged, this,
          [desc_label](QTreeWidgetItem *current, QTreeWidgetItem *) {
            if (!current) {
            ...
            if (info->homepage)
              desc += QString("<br>Homepage: <a href=\"%1\">%1</a>").arg(info->homepage);
...
#endif
    QString file = QFileDialog::getOpenFileName(this, "Load Plugin", QString(), filter);
    if (file.isEmpty())
      return;
...
void SettingsDialog::RefreshPluginList() {
  ...
  for (const auto *info : pm.ListPlugins()) {
    ...
    if (c & POLYGLOT_CAP_LANGUAGE)
      caps << "Language";
    if (c & POLYGLOT_CAP_OPTIMIZER)
      caps << "Optimizer";
    if (c & POLYGLOT_CAP_BACKEND)
      caps << "Backend";
    if (c & POLYGLOT_CAP_TOOL)
      caps << "Tool";
    if (c & POLYGLOT_CAP_UI_PANEL)
      caps << "UI Panel";
    if (c & POLYGLOT_CAP_SYNTAX_THEME)
      caps << "Theme";
    if (c & POLYGLOT_CAP_FILE_TYPE)
      caps << "File Type";
    if (c & POLYGLOT_CAP_CODE_ACTION)
      caps << "Code Action";
    if (c & POLYGLOT_CAP_FORMATTER)
      caps << "Formatter";
    if (c & POLYGLOT_CAP_LINTER)
      caps << "Linter";
    if (c & POLYGLOT_CAP_DEBUGGER)
      caps << "Debugger";
    if (c & POLYGLOT_CAP_COMPLETION)
      caps << "Completion";
    if (c & POLYGLOT_CAP_DIAGNOSTIC)
      caps << "Diagnostic";
    if (c & POLYGLOT_CAP_TEMPLATE)
      caps << "Template";
    if (c & POLYGLOT_CAP_TOPOLOGY_PROC)
      caps << "Topology Proc";
