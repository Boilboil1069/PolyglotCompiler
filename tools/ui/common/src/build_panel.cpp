...
void BuildPanel::SetupConfigSection() {
  ...
  connect(generator_combo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &BuildPanel::OnGeneratorChanged);
  ...
  connect(build_type_combo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &BuildPanel::OnBuildTypeChanged);
...
void BuildPanel::SetupTargetTree() {
  ...
  connect(target_tree_, &QTreeWidget::itemDoubleClicked, this, &BuildPanel::OnTargetDoubleClicked);
...
void BuildPanel::OnConfigure() {
  ...
  connect(configure_process_, &QProcess::readyReadStandardOutput, this,
          &BuildPanel::OnConfigureReadyRead);
  connect(configure_process_, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
          &BuildPanel::OnConfigureFinished);
...
void BuildPanel::OnClean() {
  ...
  connect(build_process_, &QProcess::readyReadStandardOutput, this, &BuildPanel::OnBuildReadyRead);
...
void BuildPanel::OnRebuild() {
  ...
  if (build_process_) {
    connect(
        build_process_, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
        [this]() { OnBuild(); }, Qt::SingleShotConnection);
  } else {
    OnBuild();
  }
}
...
void BuildPanel::DetectTargets() {
  ...
  for (const QString &line : lines) {
    ...
    if (match.hasMatch()) {
      ...
      // Skip meta targets
      if (name == "all" || name == "clean" || name == "depend" || name == "edit_cache" ||
          name == "rebuild_cache" || name == "help")
        continue;
...
void BuildPanel::OnBuildFinished(int exit_code, QProcess::ExitStatus /*status*/) {
  ...
  if (exit_code == 0) {
    ...
    build_status_label_->setText(QString("Build failed (%1 errors, %2 warnings)")
                                     .arg(build_error_count_)
                                     .arg(build_warning_count_));
  }
...
void BuildPanel::ParseBuildOutput(const QString &line) {
  ...
  static const QRegularExpression error_re(R"(^(.+?):(\d+):\d+:\s*error:\s*(.+)$)");
  static const QRegularExpression warning_re(R"(^(.+?):(\d+):\d+:\s*warning:\s*(.+)$)");

  auto error_match = error_re.match(line);
  if (error_match.hasMatch()) {
    ++build_error_count_;
    emit BuildErrorFound(error_match.captured(1), error_match.captured(2).toInt(),
  ...
  static const QRegularExpression msvc_error_re(R"(^(.+?)\((\d+)\):\s*error\s+\w+:\s*(.+)$)");
  auto msvc_match = msvc_error_re.match(line);
  if (msvc_match.hasMatch()) {
    ++build_error_count_;
    emit BuildErrorFound(msvc_match.captured(1), msvc_match.captured(2).toInt(),
...
void BuildPanel::OnBrowseBuildDir() {
  QString dir = QFileDialog::getExistingDirectory(
      this, "Select Build Directory", project_path_.isEmpty() ? QDir::homePath() : project_path_);
  if (!dir.isEmpty()) {
    build_dir_ = dir;
...
void BuildPanel::OnBrowseCmakePath() {
  QString file = QFileDialog::getOpenFileName(this, "Select CMake Executable", "/usr/local/bin",
                                              "Executables (*)");
  if (!file.isEmpty()) {
    cmake_path_ = file;
