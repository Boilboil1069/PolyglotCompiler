...
bool PolyglotLinker::IsDictType(const std::string &type_name) {
  return type_name.find("dict") == 0 || type_name.find("Dict") == 0 || type_name.find("map") == 0 ||
         type_name.find("Map") == 0 || type_name.find("HashMap") == 0 ||
         type_name.find("std::map") == 0 || type_name.find("std::unordered_map") == 0;
}
...
bool PolyglotLinker::IsContainerType(const std::string &type_name) {
  return IsListType(type_name) || IsDictType(type_name) || IsTupleType(type_name) ||
         IsStructType(type_name);
}
...
PolyglotLinker::PolyglotLinker(const LinkerConfig &config) : config_(config) {}
  ...
  while (std::getline(ifs, line)) {
    ...
    if (kind == "LINK") {
      ...
      if (!is_valid_language(target_lang)) {
        ReportError("unknown target language '" + target_lang + "' in LINK at " + path + ":" +
                    std::to_string(line_num));
        has_errors = true;
      }
      if (!is_valid_language(source_lang)) {
        ReportError("unknown source language '" + source_lang + "' in LINK at " + path + ":" +
                    std::to_string(line_num));
        has_errors = true;
      }
      ...
      if (target_sym.empty() || source_sym.empty()) {
        ReportError("empty symbol name in LINK at " + path + ":" + std::to_string(line_num));
        has_errors = true;
        continue;
      }
      ...
      std::string link_key =
          target_lang + "::" + target_sym + "<->" + source_lang + "::" + source_sym;
      if (!seen_link_keys.insert(link_key).second) {
        ReportWarning("duplicate LINK entry at " + path + ":" + std::to_string(line_num) + " for " +
                      link_key);
      }
      ...
      if (!(iss >> src_spec >> tgt_spec)) {
        ReportWarning("malformed MAP_TYPE at " + path + ":" + std::to_string(line_num));
        continue;
      }
      ...
      if (!(iss >> stub >> src_lang >> tgt_lang >> src_func >> tgt_func)) {
        ReportError(
            "malformed CALL at " + path + ":" + std::to_string(line_num) +
      ...
      if (!is_valid_language(src_lang)) {
        ReportError("unknown source language '" + src_lang + "' in CALL at " + path + ":" +
                    std::to_string(line_num));
        has_errors = true;
      }
      if (!is_valid_language(tgt_lang)) {
        ReportError("unknown target language '" + tgt_lang + "' in CALL at " + path + ":" +
                    std::to_string(line_num));
        has_errors = true;
      }
      ...
      // Validate language
      if (!is_valid_language(lang)) {
        ReportError("unknown language '" + lang + "' in SYMBOL at " + path + ":" +
                    std::to_string(line_num));
        has_errors = true;
      }
      ...
      if (!seen_symbol_keys.insert(sym_key).second) {
        ReportWarning("duplicate SYMBOL entry at " + path + ":" + std::to_string(line_num) +
                      " for " + sym_key);
      }
      ...
      ReportWarning("unknown descriptor kind '" + kind + "' at " + path + ":" +
                    std::to_string(line_num));
    }
  }
  return !has_errors;
...
void PolyglotLinker::DiscoverDescriptors(const std::string &aux_dir) {
  ...
  std::error_code ec;
  if (!fs::is_directory(aux_dir, ec))
    return;

  for (const auto &entry : fs::directory_iterator(aux_dir, ec)) {
    if (entry.is_regular_file() && entry.path().extension() == ".paux" &&
  ...
  // conflicting views of the same foreign function.
  {
    ...
    for (const auto &desc : call_descriptors_) {
      ...
      if (prev.source_param_types.size() != desc.source_param_types.size()) {
        ReportError("cross-module signature mismatch for '" + desc.source_function +
                    "': one module declares " + std::to_string(prev.source_param_types.size()) +
      ...
      for (size_t i = 0; i < desc.source_param_types.size(); ++i) {
        if (prev.source_param_types[i].kind != desc.source_param_types[i].kind) {
          ReportError("cross-module parameter type mismatch for '" + desc.source_function +
                      "' parameter " + std::to_string(i + 1) +
                      ": conflicting IR types across modules");
          success = false;
          break;
        }
      }
      ...
      if (prev.source_return_type.kind != desc.source_return_type.kind) {
        ReportError("cross-module return type mismatch for '" + desc.source_function +
                    "': conflicting return types across modules");
        success = false;
      }
    }
  }
  ...
  for (const auto &desc : call_descriptors_) {
    ...
    if (!covered) {
      ...
      if (!config_.allow_adhoc_link) {
        ReportError("CALL to '" + desc.source_function + "' (language: " + desc.source_language +
...
bool PolyglotLinker::ResolveSymbolPair(const ploy::LinkEntry &entry) {
  ...
  if ((source_sym->params.empty() || target_sym->params.empty()) && entry.param_mappings.empty()) {
    ReportWarning("LINK '" + entry.target_symbol + "' <-> '" + entry.source_symbol +
...

  {
    if (source_sym->params.size() != target_sym->params.size()) {
      ReportError("parameter count mismatch in LINK '" + entry.target_symbol + "' <-> '" +
                  ...
                  std::to_string(target_sym->params.size()) + " parameter(s) but source has " +
    ...
    for (size_t i = 0; i < source_sym->params.size(); ++i) {
      ...
      if (tp.size != 0 && sp.size != 0 && tp.size != sp.size) {
        ...
        if (i < entry.param_mappings.size() && !entry.param_mappings[i].source_type.empty() &&
            !entry.param_mappings[i].target_type.empty()) {
          has_mapping = true;
        }
        if (!has_mapping) {
          ReportError("parameter " + std::to_string(i + 1) + " size mismatch in LINK '" +
                      entry.target_symbol + "' <-> '" + entry.source_symbol + "': target expects " +
                      std::to_string(tp.size) + " bytes (" + tp.type_name +
                      ") but source provides " + std::to_string(sp.size) + " bytes (" +
                      sp.type_name + ") with no MAP_TYPE conversion");
          return false;
        }
      }
      ...
      if (tp.is_pointer != sp.is_pointer) {
        ...
        if (!has_mapping) {
          ReportError("parameter " + std::to_string(i + 1) +
                      " passing convention mismatch in LINK '" + entry.target_symbol + "' <-> '" +
                      entry.source_symbol +
                      "': " + (tp.is_pointer ? "target is pointer" : "target is value") + " vs " +
                      (sp.is_pointer ? "source is pointer" : "source is value"));
          return false;
        }
      }
      ...
      if (!tp.type_name.empty() && !sp.type_name.empty() && tp.type_name != sp.type_name) {
        bool has_mapping = false;
        if (i < entry.param_mappings.size() && !entry.param_mappings[i].source_type.empty() &&
            !entry.param_mappings[i].target_type.empty()) {
          has_mapping = true;
        ...
        if (!has_mapping) {
          ReportError("parameter " + std::to_string(i + 1) + " type mismatch in LINK '" +
                      entry.target_symbol + "' <-> '" + entry.source_symbol + "': target type '" +
    ...
    if (entry.param_mappings.empty()) {
      ...
      if (!source_sym->return_desc.type_name.empty() &&
        ...
        ReportError("return type mismatch in LINK '" + entry.target_symbol + "' <-> '" +
                    entry.source_symbol + "': target type '" + target_sym->return_desc.type_name +
                    "' vs source type '" + source_sym->return_desc.type_name + "'");
        return false;
      }
    }
  ...
  if (!entry.param_mappings.empty()) {
    if (!target_sym->params.empty() && entry.param_mappings.size() != target_sym->params.size()) {
      ReportError("MAP_TYPE entry count (" + std::to_string(entry.param_mappings.size()) +
                  ") does not match target symbol '" + entry.target_symbol + "' parameter count (" +
                  std::to_string(target_sym->params.size()) + ")");
      return false;
    }
    if (!source_sym->params.empty() && entry.param_mappings.size() != source_sym->params.size()) {
      ReportError("MAP_TYPE entry count (" + std::to_string(entry.param_mappings.size()) +
                  ") does not match source symbol '" + entry.source_symbol + "' parameter count (" +
                  std::to_string(source_sym->params.size()) + ")");
      return false;
    }
  }
...
void PolyglotLinker::GenerateX86_64Stub(GlueStub &stub, const ploy::LinkEntry &entry,
  ...
  code.push_back(0x48);
  code.push_back(0x89);
  code.push_back(0xE5); // mov rbp, rsp
  ...
  code.push_back(0x41);
  code.push_back(0x50); // push r8
  code.push_back(0x41);
  code.push_back(0x51); // push r9
#else
...
#ifdef _WIN32
  code.push_back(0x41);
  code.push_back(0x59); // pop r9
  code.push_back(0x41);
  code.push_back(0x58); // pop r8
  ...
  if (target_is_java) {
    ...
    code.push_back(0x4D);
    code.push_back(0x89);
    code.push_back(0xC1); // mov r9, r8
    code.push_back(0x49);
    code.push_back(0x89);
    code.push_back(0xD0); // mov r8, rdx
    code.push_back(0x48);
    code.push_back(0x89);
    code.push_back(0xCA); // mov rdx, rcx
    ...
    code.push_back(0x48);
    code.push_back(0x89);
    code.push_back(0xD1); // mov rcx, rdx
    code.push_back(0x48);
    code.push_back(0x89);
    code.push_back(0xF2); // mov rdx, rsi
    code.push_back(0x48);
    code.push_back(0x89);
    code.push_back(0xFE); // mov rsi, rdi
  ...
  code.push_back(0x48);
  code.push_back(0x89);
  code.push_back(0xEC); // mov rsp, rbp
...
void PolyglotLinker::EmitIntToFloatMarshal(std::vector<std::uint8_t> &code, size_t param_idx) {
  ...
  std::uint8_t modrm =
      static_cast<std::uint8_t>(0xC0 | (param_idx << 3) | (gpr_regs[param_idx] & 0x07));
...
void PolyglotLinker::EmitListMarshal(std::vector<std::uint8_t> &code, const std::string &from_lang,
...
void PolyglotLinker::EmitTupleMarshal(std::vector<std::uint8_t> &code, const std::string &from_lang,
                                      const std::string &to_lang, size_t param_idx) {
...
void PolyglotLinker::EmitDictMarshal(std::vector<std::uint8_t> &code, const std::string &from_lang,
                                     const std::string &to_lang, size_t param_idx) {
...
void PolyglotLinker::EmitReturnMarshal(std::vector<std::uint8_t> &code,
  ...
  if (from_lang == "python" && (to_lang == "cpp" || to_lang == "rust")) {
    ...
    bool is_float = (return_type == "float" || return_type == "double" || return_type == "f64" ||
                     return_type == "f32");
    if (is_float) {
      ...
      code.push_back(0x48);
      code.push_back(0x89);
      code.push_back(0xC1); // mov rcx, rax
#else
      code.push_back(0x48);
      code.push_back(0x89);
      code.push_back(0xC7); // mov rdi, rax
#endif
...
void PolyglotLinker::EmitGILRelease(std::vector<std::uint8_t> &code,
  ...
  std::uint8_t disp =
      static_cast<std::uint8_t>(static_cast<std::uint8_t>(256 - gil_state_stack_offset));
#ifdef _WIN32
  code.push_back(0x8B);
  code.push_back(0x4D);
  code.push_back(disp); // mov ecx, [rbp-disp]
#else
  code.push_back(0x8B);
  code.push_back(0x7D);
  code.push_back(disp); // mov edi, [rbp-disp]
#endif
...
void PolyglotLinker::EmitRustBorrowAnnotation(std::vector<std::uint8_t> &code, size_t param_idx,
                                              bool is_mutable) {
  ...
  code.push_back(0x1F);
  std::uint8_t modrm =
      static_cast<std::uint8_t>(((param_idx & 0x07) << 3) | (is_mutable ? 0x01 : 0x00));
  code.push_back(modrm);
}
...
bool PolyglotLinker::ValidateABICompatibility(const CrossLangSymbol &target,
  ...
  if (target.params.size() != source.params.size()) {
    ReportError("ABI mismatch: '" + target.name + "' (" + target.language + ") expects " +
                std::to_string(target.params.size()) + " parameter(s) but '" + source.name + "' (" +
                source.language + ") provides " + std::to_string(source.params.size()));
    return false;
  }
  ...
  for (size_t i = 0; i < target.params.size(); ++i) {
    ...
    if (tp.size != 0 && sp.size != 0 && tp.size != sp.size) {
      ReportWarning("ABI warning: parameter " + std::to_string(i + 1) + " of '" + target.name +
                    "' has size " + std::to_string(tp.size) + " but source provides size " +
                    std::to_string(sp.size));
    }

    // Pointer vs non-pointer mismatch.
    if (tp.is_pointer != sp.is_pointer) {
      ReportError("ABI mismatch: parameter " + std::to_string(i + 1) + " of '" + target.name +
                  "' is " + (tp.is_pointer ? "a pointer" : "not a pointer") + " but source '" +
                  source.name + "' provides " + (sp.is_pointer ? "a pointer" : "a value"));
      compatible = false;
    }
  }
  ...
  if (target.return_desc.size != 0 && source.return_desc.size != 0 &&
    ...
    ReportWarning("ABI warning: return type of '" + target.name + "' has size " +
                  std::to_string(target.return_desc.size) + " but source return size is " +
  ...
  if (target.return_desc.is_pointer != source.return_desc.is_pointer) {
    ReportError("ABI mismatch: '" + target.name + "' returns " +
                (target.return_desc.is_pointer ? "a pointer" : "a value") + " but source '" +
                source.name + "' returns " +
