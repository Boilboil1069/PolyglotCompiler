...
struct relocation_info {
  ...
  std::uint32_t r_symbolnum : 24, r_pcrel : 1, r_length : 2, r_extern : 1, r_type : 4;
};
...
ObjectFormat DetectObjectFormat(const std::vector<std::uint8_t> &data) {
  ...
  if (data.size() >= 4 && data[0] == 'P' && data[1] == 'O' && data[2] == 'B' && data[3] == 'J') {
  ...
  if (magic == macho::MH_MAGIC_64 || magic == macho::MH_MAGIC || magic == macho::MH_CIGAM_64 ||
      magic == macho::MH_CIGAM || magic == macho::FAT_MAGIC || magic == macho::FAT_CIGAM) {
  ...
  // Check archive magic
  if (data.size() >= AR_MAGIC_LEN && std::memcmp(data.data(), AR_MAGIC, AR_MAGIC_LEN) == 0) {
...
SectionType GetSectionTypeFromName(const std::string &name) {
  if (name == ".bss" || name == "__bss")
    return SectionType::kNobits;
  if (name == ".symtab")
    return SectionType::kSymtab;
  if (name == ".strtab" || name == ".shstrtab")
    return SectionType::kStrtab;
  if (name == ".dynamic")
    return SectionType::kDynamic;
  if (name == ".dynsym")
    return SectionType::kDynsym;
  if (name == ".rela" || name.find(".rela.") == 0)
    return SectionType::kRela;
  if (name == ".rel" || name.find(".rel.") == 0)
    return SectionType::kRel;
  if (name == ".hash")
    return SectionType::kHash;
  if (name == ".note" || name.find(".note.") == 0)
    return SectionType::kNote;
  if (name == ".init_array")
    return SectionType::kInitArray;
  if (name == ".fini_array")
    return SectionType::kFiniArray;
...
template <typename T> bool Linker::ReadValue(std::ifstream &file, T &value) {
  file.read(reinterpret_cast<char *>(&value), sizeof(T));
  return file.good() || file.eof();
}

template <typename T> void Linker::WriteValue(std::vector<std::uint8_t> &out, T value) {
...
bool Linker::ParseELFSections(std::ifstream &file, ObjectFile &obj) {
  ...
  for (std::size_t i = 0; i < shdrs.size(); ++i) {
    ...
    case SHT_NOBITS:
      ...
      break;
    case SHT_NOTE:
      section.type = SectionType::kNote;
      break;
    case SHT_INIT_ARRAY:
      section.type = SectionType::kInitArray;
      break;
    case SHT_FINI_ARRAY:
      section.type = SectionType::kFiniArray;
      break;
    default:
      section.type = SectionType::kProgbits;
      break;
  ...
  for (std::size_t i = 0; i < shdrs.size(); ++i) {
    ...
    for (std::size_t si = 0; si < syms.size(); ++si) {
      ...
      case STB_GLOBAL:
        ...
        break;
      case STB_WEAK:
        symbol.binding = SymbolBinding::kWeak;
        break;
      default:
        symbol.binding = SymbolBinding::kLocal;
        break;
      ...
      case STT_SECTION:
        ...
        break;
      case STT_FILE:
        symbol.type = SymbolType::kFile;
        break;
      case STT_COMMON:
        symbol.type = SymbolType::kCommon;
        break;
      case STT_TLS:
        symbol.type = SymbolType::kTLS;
        break;
      default:
        symbol.type = SymbolType::kNoType;
        break;
      ...
      case STV_DEFAULT:
        symbol.visibility = SymbolVisibility::kDefault;
        break;
      case STV_INTERNAL:
        symbol.visibility = SymbolVisibility::kInternal;
        break;
      case STV_HIDDEN:
        symbol.visibility = SymbolVisibility::kHidden;
        break;
      case STV_PROTECTED:
        symbol.visibility = SymbolVisibility::kProtected;
        break;
      default:
        symbol.visibility = SymbolVisibility::kDefault;
        break;
    ...
    if (shdr.sh_type == SHT_RELA) {
      ...
      for (const auto &rela : relas) {
        ...
        reloc.is_pc_relative =
            (rtype == 2 || rtype == 4 || rtype == 9 || rtype == 13 || rtype == 15 || rtype == 24);

        // Determine relocation size
        if (rtype == 1 || rtype == 24 || rtype == 25) {
          reloc.size = 8; // 64-bit
        } else if (rtype == 2 || rtype == 3 || rtype == 4 || rtype == 9 || rtype == 10 ||
                   rtype == 11 || rtype == 26) {
          reloc.size = 4; // 32-bit
...
bool Linker::ParseMachOLoadCommands(std::ifstream &file, ObjectFile &obj) {
  ...
  for (std::uint32_t i = 0; i < header.ncmds; ++i) {
    ...
    if (lc.cmd == macho::LC_SEGMENT_64) {
      ...
      if (!ParseMachOSymtab(file, obj, symtab.symoff, symtab.nsyms, symtab.stroff,
                            symtab.strsize)) {
        return false;
      }
    }
...
bool Linker::ParseMachOSegments(std::ifstream &file, ObjectFile &obj, std::uint64_t offset,
                                std::uint32_t /*size*/) {
  file.seekg(offset);
...
bool Linker::ParseMachOSymtab(std::ifstream &file, ObjectFile &obj, std::uint32_t symoff,
                              std::uint32_t nsyms, std::uint32_t stroff, std::uint32_t strsize) {
  // Read string table
...
bool Linker::LoadCOFF(const std::string &path, ObjectFile &obj) {
  ...
  if (hdr.symbol_table_offset > 0) {
    std::streamoff strtab_off =
        static_cast<std::streamoff>(hdr.symbol_table_offset + hdr.number_of_symbols * 18);
  ...
  if (hdr.symbol_table_offset > 0) {
    ...
    while (i < hdr.number_of_symbols) {
      ...
      if (cs.name.long_name.zeroes == 0) {
        ...
        sym.name = std::string(cs.name.short_name, strnlen(cs.name.short_name, 8));
      }
    ...
    for (auto &sec : obj.sections) {
      for (auto &r : sec.relocations) {
        if (r.symbol_index >= 0 && r.symbol_index < static_cast<int>(obj.symbols.size())) {
...
#pragma pack(pop)
  ...
  for (const auto &srec : sec_recs) {
    ...
    if (!(srec.flags & kPobjSectionFlagBss) && srec.size > 0 && srec.offset > 0) {
      ...
      file.read(reinterpret_cast<char *>(sec.data.data()), static_cast<std::streamsize>(srec.size));
      file.seekg(saved);
    }
...
bool Linker::ParseArchiveSymbolTable(std::ifstream &file, Archive &archive) {
  ...
  if (name == "/" || name == "/SYM64/") {
    ...
    nsyms = ((nsyms >> 24) & 0xFF) | ((nsyms >> 8) & 0xFF00) | ((nsyms << 8) & 0xFF0000) |
            ((nsyms << 24) & 0xFF000000);
    ...
    for (auto &off : offsets) {
      off = ((off >> 24) & 0xFF) | ((off >> 8) & 0xFF00) | ((off << 8) & 0xFF0000) |
            ((off << 24) & 0xFF000000);
    }
...
bool Linker::LoadArchiveMember(const Archive &archive, const ArchiveMember &member) {
  ...
  std::string temp_path =
      "/tmp/polyld_" + std::to_string(reinterpret_cast<std::uintptr_t>(&member)) + ".o";
  {
...
bool Linker::ResolveSymbols() {
  ...
  Trace("Symbol resolution complete: " + std::to_string(stats_.symbols_defined) + " defined, " +
        std::to_string(stats_.symbols_undefined) + " undefined");
        ...
        if (it != symbol_table_.end() && !it->second.is_defined) {
          ...
          for (const auto &member : archive.members) {
            if (member.offset == arsym.member_offset ||
              ...
              Trace("Loading " + member.name + " from " + archive.path + " for symbol " +
                    arsym.name);
...
void Linker::CreateStandardSections() {
  ...
  std::vector<std::string> standard_sections = {".text", ".rodata",     ".data",
                                                ".bss",  ".init_array", ".fini_array"};
...
void Linker::AssignSectionAddresses() {
  ...
  auto section_priority = [](const std::string &name) -> int {
    ...
    if (name.find(".init") != std::string::npos)
      return 4;
    if (name.find(".fini") != std::string::npos)
      return 5;
    return 10;
  };
...
bool Linker::LayoutSections() {
  ...
  output_sections_.erase(std::remove_if(output_sections_.begin(), output_sections_.end(),
...
bool Linker::ApplyELFRelocation_ARM64(const Relocation &reloc, std::vector<std::uint8_t> &data,
                                      std::uint64_t offset, std::int64_t value) {
  if (offset >= data.size()) {
    ReportWarning("Relocation offset out of bounds");
    return true;
...
bool Linker::ApplyMachORelocation(const Relocation &reloc, std::vector<std::uint8_t> &data,
                                  std::uint64_t offset, std::int64_t value) {
  if (offset >= data.size()) {
    return true;
  }
...
bool Linker::GenerateELFExecutableLegacy() {
  ...
  case TargetArch::kAArch64:
    ...
    break;
  case TargetArch::kX86:
    ehdr.e_machine = EM_386;
    break;
  case TargetArch::kARM:
...
bool Linker::GenerateMachOExecutableLegacy() {
  ...
  auto align_page = [](std::uint64_t v) { return (v + page - 1) & ~(page - 1); };
  ...
  auto push32 = [&cmds](std::uint32_t v) {
    for (int i = 0; i < 4; ++i)
      cmds.push_back(static_cast<std::uint8_t>((v >> (i * 8)) & 0xFF));
  };
  ...
  if (macho_emit_id_dylib_) {
    ...
    std::uint64_t entry_off =
        entry && entry->is_defined ? entry->value - config_.base_address : text_file_off;
    push64(entry_off);
    push64(0); // stack size
    ++num_cmds;
  }
...
bool Linker::GenerateRelocatable() {
  ...
  for (auto &sec : output_sections_) {
    ...
    e.shdr.sh_flags = 0;
    if (static_cast<std::uint64_t>(sec.flags) & static_cast<std::uint64_t>(SectionFlags::kAlloc))
      e.shdr.sh_flags |= SHF_ALLOC;
    if (static_cast<std::uint64_t>(sec.flags) & static_cast<std::uint64_t>(SectionFlags::kWrite))
  ...
  // Write sections
  for (std::size_t i = 1; i < sec_entries.size(); ++i) {
    while (output.size() < sec_entries[i].shdr.sh_offset)
      output.push_back(0);
    output.insert(output.end(), sec_entries[i].data.begin(), sec_entries[i].data.end());
  }

  // Pad to section header table
  while (output.size() < sh_offset)
    output.push_back(0);
...
bool Linker::GenerateStaticLibrary() {
  ...
  for (auto &obj : objects_) {
    ...
    std::memcpy(header, basename.c_str(), std::min(basename.size(), static_cast<std::size_t>(16)));
...
bool Linker::Link() {
  ...
  if (config_.verbose) {
    ...
    std::cout << "Output: " << config_.output_file << " (" << stats_.total_output_size
              << " bytes)\n";
  }
...
bool LinkerScriptParser::SkipWhitespace() {
  while (pos_ < script_.size()) {
    ...
    if (std::isspace(c)) {
      ...
      pos_ += 2;
      while (pos_ + 1 < script_.size() && !(script_[pos_] == '*' && script_[pos_ + 1] == '/')) {
        ++pos_;
      }
      pos_ += 2;
    } else if (c == '/' && pos_ + 1 < script_.size() && script_[pos_ + 1] == '/') {
...
bool LinkerScriptParser::Match(const std::string &s) {
  SkipWhitespace();
  if (pos_ + s.size() > script_.size())
    return false;

  for (std::size_t i = 0; i < s.size(); ++i) {
    if (script_[pos_ + i] != s[i])
      return false;
  }
...
// Main entry point has been consolidated into main.cpp.
// This file is compiled as part of linker_lib (library mode only).
