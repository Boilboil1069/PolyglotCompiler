...
enum class TargetArch { kX86_64, kAArch64, kX86, kARM, kRISCV64 };
...
private:
  ...
  bool ParseMachOSegments(std::ifstream &file, ObjectFile &obj, std::uint64_t offset,
                          std::uint32_t size);
  bool ParseMachOSymtab(std::ifstream &file, ObjectFile &obj, std::uint32_t symoff,
  ...
  bool ApplyELFRelocation_ARM64(const Relocation &reloc, std::vector<std::uint8_t> &data,
                                std::uint64_t offset, std::int64_t value);
  bool ApplyMachORelocation(const Relocation &reloc, std::vector<std::uint8_t> &data,
                            std::uint64_t offset, std::int64_t value);
  ...
  // File I/O helpers
  template <typename T> bool ReadValue(std::ifstream &file, T &value);
