...
private:
  ...
  GlueStub GenerateGlueStub(const ploy::LinkEntry &entry, const CrossLangSymbol &target_sym,
  ...
  void GenerateX86_64Stub(GlueStub &stub, const ploy::LinkEntry &entry,
                          const CrossLangSymbol &target_sym, const CrossLangSymbol &source_sym);
  ...
  void EmitCallingConventionAdaptor(std::vector<std::uint8_t> &code, const std::string &from_lang,
                                    const std::string &to_lang);
  ...
  void EmitContainerMarshal(std::vector<std::uint8_t> &code, const std::string &from_lang,
                            const std::string &to_lang, const std::string &from_type,
                            const std::string &to_type, size_t param_idx);
  ...
  void EmitReturnMarshal(std::vector<std::uint8_t> &code, const std::string &from_lang,
                         const std::string &to_lang, const std::string &return_type);
  ...
  // and returns false.
  bool ValidateABICompatibility(const CrossLangSymbol &target, const CrossLangSymbol &source);
