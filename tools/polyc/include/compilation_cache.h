...
public:
  explicit CompilationCache(const std::string &cache_dir) : cache_dir_(cache_dir) {
    std::error_code ec;
    std::filesystem::create_directories(cache_dir_, ec);
  }

  // Compute a 64-bit FNV-1a hash from source text and settings string.
  static uint64_t ComputeHash(const std::string &source, const std::string &settings_key) {
  ...
  bool HasCached(uint64_t hash) const { return std::filesystem::exists(CachePath(hash)); }
...
private:
  std::string CachePath(uint64_t hash) const {
    char buf[32];
    std::snprintf(buf, sizeof(buf), "%016llx.cache", static_cast<unsigned long long>(hash));
