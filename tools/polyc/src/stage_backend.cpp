...
void AbsorbMC(const MCResult &mc, std::vector<ObjSection> &sections,
  ...
  if (sec_index.count(".text")) {
    symbols.push_back(
        {"_start", sec_index[".text"], 0, sections[sec_index[".text"]].data.size(), true, true});
  }
  ...
  for (const auto &sym : mc.symbols) {
    ...
    sym_index[sym.name] = static_cast<std::uint32_t>(symbols.size());
...
BackendResult RunBackendStage(const DriverSettings &settings, const FrontendResult &frontend,
  ...
  if (settings.language != "ploy") {
    ...
    for (const auto &stub : bridge.stubs) {
      ...
      if (V)
        std::cerr << "[stage/backend]  stub " << stub.stub_name << "\n";
    }
  }
  ...
  }
  {
    ...
    if (!ir::Verify(*ir_ctx, vopts, &verify_msg)) {
      ...
      std::cerr << "[warn] IR verification: " << verify_msg << " (continuing in --force mode)\n";
    }
  }
  ...
  if (settings.opt_level > 0) {
    passes::PassManager pm(static_cast<passes::PassManager::OptLevel>(settings.opt_level));
    pm.Build();
    std::size_t n = pm.RunOnModule(*ir_ctx, V);
    if (V)
      std::cerr << "[stage/backend]  " << n << " opt passes/fn\n";
    ...
    if (!ir::Verify(*ir_ctx, post_vopts, &post_msg)) {
      ...
      std::cerr << "[warn] post-opt IR: " << post_msg << " (continuing in --force/dev mode)\n";
    }
  }
  ...
  if (settings.pgo_generate) {
    ...
    if (V)
      std::cerr << "[stage/backend]  PGO instrumentation inserted\n";
  }
  ...
  if (!settings.pgo_use_path.empty()) {
    ...
    if (profile.LoadFromFile(settings.pgo_use_path)) {
      ...
      if (V) {
        std::cerr << "[stage/backend]  PGO: " << report.hot_functions << " hot fn(s), "
  ...
  if (use_wasm) {
    ...
    result.symbols.push_back({"_start", 0, 0,
  ...
  auto run_native = [&](auto &target) {
    ...
    if (mc.sections.empty()) {
      ...
      SectionT text;
      text.name = ".text";
      if constexpr (std::is_same_v<std::decay_t<decltype(target)>, backends::x86_64::X86Target>) {
      ...
      mc.symbols.push_back(
          {"_start", ".text", 0, static_cast<std::uint64_t>(text.data.size()), true, true});
