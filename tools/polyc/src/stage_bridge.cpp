...
BridgeResult RunBridgeStage(const DriverSettings &settings, const MarshalResult &marshal,
  ...
  auto register_sym = [&](const std::string &name, const std::string &language) {
    ...
    if (V)
      std::cerr << "[stage/bridge]   sym " << language << "::" << name << "\n";
  };
  ...
  if (!link_ok) {
    for (const auto &e : poly_linker.GetErrors()) {
      result.diagnostics.Report(core::SourceLoc{"<bridge>", 1, 1}, e);
    }
    if (!settings.force) {
  ...
  for (const auto &stub : poly_linker.GetStubs()) {
    ...
    if (V) {
      std::cerr << "[stage/bridge]   stub " << stub.stub_name << " (" << stub.code.size()
                << " bytes)\n";
    }
  }
  ...
  if (!aux_dir.empty()) {
    ...
    fs::path desc_path = fs::path(aux_dir) / (stem + "_link_descriptors.paux");
    ...
    if (ofs.is_open()) {
      ...
      auto emit_sym = [&](const std::string &name, const std::string &lang) {
        if (!seen.insert(lang + "::" + name).second)
          return;
      ...
      if (V)
        std::cerr << "[stage/bridge] descriptors -> " << result.descriptor_file << "\n";
    }
  }
