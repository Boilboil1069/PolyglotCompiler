        ...
        if (ofs.is_open()) {
            ...
            for (const auto &entry : context_.semantic_db->link_entries) {
                ...
                for (const auto &m : entry.param_mappings) {
                    ofs << "MAP_TYPE";
                    if (!m.source_language.empty()) ofs << " " << m.source_language;
                    else ofs << " " << entry.target_language;
                    ofs << "::" << m.source_type;
                    if (!m.target_language.empty()) ofs << " " << m.target_language;
                    else ofs << " " << entry.source_language;
                    ofs << "::" << m.target_type << "\n";
                }
            }
