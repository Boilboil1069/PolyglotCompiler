...
std::string BuildPobj(const std::string &path, const std::vector<ObjSection> &obj_sections,
  ...
  for (std::size_t i = 0; i < obj_sections.size(); ++i) {
    ...
    if (!obj_sections[i].bss)
      data_cursor += obj_sections[i].data.size();
  }
...
std::string BuildElf64(const std::string &path, const std::vector<ObjSection> &obj_sections,
  ...
  auto align_to = [](std::size_t v, std::size_t a) { return a ? (v + a - 1) & ~(a - 1) : v; };
...
#undef WRITE_IF
  ...
  if (idx_rela != SHN_UNDEF && text_it != obj_sections.end()) {
    ...
    for (const auto &r : text_it->relocs) {
      ...
      std::uint32_t rtype = is_arm64 ? (r.type == 1 ? R_AARCH64_CALL26 : R_AARCH64_JUMP26)
...
std::string BuildMachO64(const std::string &path, const std::vector<ObjSection> &obj_sections,
  ...
  auto text_it = std::find_if(obj_sections.begin(), obj_sections.end(),
                              [](const ObjSection &s) { return s.name == ".text"; });
  auto data_it = std::find_if(obj_sections.begin(), obj_sections.end(), [](const ObjSection &s) {
  ...
  if (text_it != obj_sections.end()) {
    ...
    for (const auto &r : text_it->relocs) {
      ...
      mr.packed = (sym_idx & 0x00FFFFFF) | (pcrel ? (1u << 24) : 0) | ((length & 3u) << 25) |
                  (1u << 27) // extern = 1
                  | ((rtype & 0x0Fu) << 28);
...
struct CoffSectionHeader {
  char Name[8];
  std::uint32_t VirtualSize, VirtualAddress, SizeOfRawData, PointerToRawData, PointerToRelocations,
...
std::string BuildCoff(const std::string &path, const std::vector<ObjSection> &obj_sections,
  ...
  for (const auto &sym : obj_symbols) {
    ...
    cs.SectionNumber = (sym.defined && sym.section_index != 0xFFFFFFFF)
                           ? static_cast<std::int16_t>(sym.section_index + 1)
                           : 0;
...
std::string EmitObject(const DriverSettings &settings, const std::vector<ObjSection> &sections,
...
void WriteAuxBinary(const std::string &aux_dir, const std::string &filename,
                    const std::vector<std::pair<std::string, std::string>> &secs, bool verbose) {
  if (aux_dir.empty())
    return;
  ...
  if (verbose)
    std::cerr << "[polyc]   -> " << path.string() << " (" << total << " bytes, binary)\n";
}

void WriteAuxBinarySingle(const std::string &aux_dir, const std::string &filename,
                          const std::string &sec_name, const std::string &content, bool verbose) {
...
PackagingResult RunPackagingStage(const DriverSettings &settings, const BackendResult &backend,
  ...
  if (settings.lto_enabled && settings.mode == "link") {
    ...
    if (!tmp_obj.empty()) {
      ...
      if (lto_linker.Link(lto_out)) {
        ...
        if (V) {
          std::cerr << "[stage/packaging] LTO: " << stats.modules_linked << " module(s), "
                    << stats.functions_inlined << " inlined, " << stats.functions_removed
                    << " removed, " << stats.optimization_time_ms << "ms\n";
        }
      ...
      if (V)
        std::cerr << "[stage/packaging] LTO link failed; falling back to standard path\n";
    }
  }
  ...
  auto emit_and_link = [&](bool do_link) -> bool {
    ...
    if (obj_path.empty()) {
      result.diagnostics.Report(core::SourceLoc{"<packaging>", 1, 1}, "failed to emit object file");
      return false;
    }
    result.obj_path = obj_path;
    if (V)
      std::cerr << "[polyc] Produced: " << obj_path << "\n";
    ...
    if (!aux_dir.empty()) {
      ...
      if (aux_obj.string() != obj_path) {
        std::error_code ec;
        fs::copy_file(obj_path, aux_obj, fs::copy_options::overwrite_existing, ec);
        if (!ec && V)
          std::cerr << "[polyc]   -> " << aux_obj.string() << " (object copy in aux)\n";
      }
    }
    ...
    if (V)
      std::cerr << "[polyc] Link stage completed\n";
    return true;
  };

  bool ok = false;
  if (settings.mode == "link")
    ok = emit_and_link(true);
  else /* compile | assemble */
    ok = emit_and_link(false);
