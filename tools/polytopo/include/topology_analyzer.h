...
private:
  ...
  void AnalyzeBodyStatement(const std::shared_ptr<ploy::Statement> &stmt, uint64_t context_node_id);
  ...
  uint64_t FindOrCreateExternalNode(const std::string &language, const std::string &function_name,
                                    const core::SourceLoc &loc);

  // Connect an expression result to a target node's input port
  void ConnectEdge(const ExprResult &source, uint64_t target_node_id, uint64_t target_port_id,
                   const core::SourceLoc &loc);
