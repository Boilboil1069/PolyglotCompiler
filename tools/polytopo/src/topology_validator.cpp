...
TopologyValidator::TopologyValidator(const ValidationOptions &opts) : opts_(opts) {}
...
size_t TopologyValidator::ErrorCount() const {
  return static_cast<size_t>(
      std::count_if(diagnostics_.begin(), diagnostics_.end(), [](const ValidationDiagnostic &d) {
...
void TopologyValidator::ValidateEdgeTypes(const TopologyGraph &graph) {
  for (const auto &edge : graph.Edges()) {
    ...
    if (!src_node || !tgt_node) {
      AddDiagnostic(ValidationDiagnostic::Severity::kError, "Edge references non-existent node",
                    edge.loc, 0, edge.id);
      continue;
    }
    ...
    if (!src_port) {
      AddDiagnostic(ValidationDiagnostic::Severity::kError,
                    "Edge source port not found on node '" + src_node->name + "'", edge.loc,
                    src_node->id, edge.id);
      continue;
    }
    if (!tgt_port) {
      AddDiagnostic(ValidationDiagnostic::Severity::kError,
                    "Edge target port not found on node '" + tgt_node->name + "'", edge.loc,
                    tgt_node->id, edge.id);
      continue;
    }

    // Check type compatibility
    auto status = CheckTypeCompatibility(src_port->type, tgt_port->type, src_node->language,
                                         tgt_node->language);
    ...
    case TopologyEdge::Status::kImplicitConvert:
      AddDiagnostic(ValidationDiagnostic::Severity::kInfo,
                    "Implicit type conversion: " + src_port->type.name + " (" + src_node->language +
    ...
    case TopologyEdge::Status::kExplicitConvert:
      AddDiagnostic(ValidationDiagnostic::Severity::kWarning,
                    "Explicit type conversion required: " + src_port->type.name + " (" +
                        src_node->language + ") -> " + tgt_port->type.name + " (" +
                        tgt_node->language + ")",
                    edge.loc, 0, edge.id);
      break;
    case TopologyEdge::Status::kIncompatible:
      AddDiagnostic(ValidationDiagnostic::Severity::kError,
                    "Incompatible types: " + src_port->type.name + " (" + src_node->language +
                        ") cannot convert to " + tgt_port->type.name + " (" + tgt_node->language +
                        ")",
                    edge.loc, 0, edge.id);
      break;
    case TopologyEdge::Status::kUnknown: {
      auto severity = opts_.strict_any ? ValidationDiagnostic::Severity::kError
                                       : ValidationDiagnostic::Severity::kWarning;
      AddDiagnostic(severity,
                    "Type unknown on edge from '" + src_node->name + ":" + src_port->name +
                        "' to '" + tgt_node->name + ":" + tgt_port->name + "'",
                    edge.loc, 0, edge.id);
      break;
    }
    }
    ...
    if (opts_.warn_unconnected_inputs) {
      for (const auto &port : node.inputs) {
        if (connected_inputs.find(port.id) == connected_inputs.end()) {
          AddDiagnostic(ValidationDiagnostic::Severity::kWarning,
                        "Unconnected input port '" + port.name + "' on node '" + node.name + "'",
    ...
    if (opts_.warn_unconnected_outputs) {
      for (const auto &port : node.outputs) {
        if (connected_outputs.find(port.id) == connected_outputs.end()) {
          AddDiagnostic(ValidationDiagnostic::Severity::kWarning,
                        "Unconnected output port '" + port.name + "' on node '" + node.name + "'",
                        node.loc, node.id, 0, port.id);
        }
      }
    }
  ...
  for (const auto &node : graph.Nodes()) {
    ...
    for (const auto &[port_id, count] : incoming_count) {
      if (count > 1) {
        ...
        AddDiagnostic(ValidationDiagnostic::Severity::kWarning,
                      "Input port '" + port_name + "' on node '" + node.name + "' has " +
                          std::to_string(count) + " incoming connections (possible ambiguity)",
...
void TopologyValidator::ValidateLanguageCompatibility(const TopologyGraph &graph) {
  ...
  for (const auto &edge : graph.Edges()) {
    ...
    if (src->language != tgt->language) {
      AddDiagnostic(ValidationDiagnostic::Severity::kInfo,
                    "Cross-language edge: " + src->language + " -> " + tgt->language +
...
void TopologyValidator::AddDiagnostic(ValidationDiagnostic::Severity severity,
                                      const std::string &message, const core::SourceLoc &loc,
                                      uint64_t node_id, uint64_t edge_id, uint64_t port_id) {
