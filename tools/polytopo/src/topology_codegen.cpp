...
std::string ToPloySrcType(const std::string &type_name) {
  ...
  if (type_name == "int" || type_name == "i32" || type_name == "i64" || type_name == "I64") {
    return "INT";
  }
  if (type_name == "float" || type_name == "f32" || type_name == "f64" || type_name == "double") {
    return "FLOAT";
  }
  ...
  if (type_name == "bool" || type_name == "Bool")
    return "BOOL";
  if (type_name == "void" || type_name == "None")
    return "VOID";
...
std::string GeneratePloySrc(const TopologyGraph &graph) {
  ...
  out << "// Nodes: " << graph.NodeCount() << "  Edges: " << graph.EdgeCount() << "\n";
  out << "// ============================================================================\n\n";
  ...
  for (const auto &edge : graph.Edges()) {
    ...
    if (!src_node || !tgt_node)
      continue;
    if (src_node->language == tgt_node->language)
      continue;
    ...
    if (!src_type.empty() && !tgt_type.empty() && src_type != "Any" && tgt_type != "Any") {
      std::string key =
          src_node->language + "::" + src_type + " -> " + tgt_node->language + "::" + tgt_type;
      if (emitted_map_types.count(key) == 0) {
        out << "MAP_TYPE(" << src_node->language << "::" << src_type << ", " << tgt_node->language
            << "::" << tgt_type << ");\n";
  ...
  for (const auto &edge : graph.Edges()) {
    ...
    if (src_node->language == tgt_node->language)
      continue;
    if (src_node->language.empty() || tgt_node->language.empty())
      continue;
    ...
    std::string link_key =
        src_node->language + "::" + src_func + " -> " + tgt_node->language + "::" + tgt_func;
    if (emitted_links.count(link_key) > 0)
      continue;
    ...
    out << "LINK(" << src_node->language << ", " << tgt_node->language << ", " << src_func << ", "
        << tgt_func << ")";
    ...
    for (const auto &e2 : graph.Edges()) {
      if (e2.source_node_id != edge.source_node_id || e2.target_node_id != edge.target_node_id) {
        continue;
      }
      std::string st, tt;
      for (const auto &p : src_node->outputs) {
        if (p.id == e2.source_port_id) {
          st = PortTypeName(p);
          break;
        }
      }
      for (const auto &p : tgt_node->inputs) {
        if (p.id == e2.target_port_id) {
          tt = PortTypeName(p);
          break;
        }
      }
      if (!st.empty() && !tt.empty() && st != "Any" && tt != "Any") {
        out << "    MAP_TYPE(" << src_node->language << "::" << ToPloySrcType(st) << ", "
  ...
  for (const auto &node : graph.Nodes()) {
    if (node.kind != TopologyNode::Kind::kPipeline)
      continue;
    ...
    if (!node.inputs.empty() || !node.outputs.empty()) {
      ...
      for (size_t i = 0; i < node.inputs.size(); ++i) {
        if (i > 0)
          out << ", ";
        out << node.inputs[i].name << ": " << ToPloySrcType(PortTypeName(node.inputs[i]));
      }
      out << ")";
      ...
      for (const auto *e : out_edges) {
        ...
        if (!target)
          continue;
        out << "        CALL(" << target->language << ", " << target->name;
  ...
  for (const auto &node : graph.Nodes()) {
    ...
    out << "// " << KindString(node.kind) << ": " << node.name << " [" << node.language << "]\n";
    ...
    if (node.kind == TopologyNode::Kind::kMapFunc) {
      ...
      for (const auto *e : out_edges) {
        ...
        if (!target)
          continue;
        out << "    LET result = CALL(" << target->language << ", " << target->name;
        for (const auto &inp : node.inputs) {
          out << ", " << inp.name;
        }
        out << ");\n";
    ...
    for (size_t ei = 0; ei < out_edges.size(); ++ei) {
      ...
      if (target->kind == TopologyNode::Kind::kConstructor) {
        // NEW expression
        out << "    LET " << var_name << " = NEW(" << target->language << ", " << target->name;
        for (const auto &inp : node.inputs) {
          out << ", " << inp.name;
        }
        out << ");\n";
        ...
        // METHOD expression
        out << "    LET " << var_name << " = METHOD(" << target->language << ", "
            << node.inputs[0].name << ", " << target->name;
        ...
        out << "    LET " << var_name << " = CALL(" << target->language << ", " << target->name;
        for (const auto &inp : node.inputs) {
          out << ", " << inp.name;
        }
...
TopologyNode::Kind ParseKind(const std::string &s) {
  ...
  if (s == "constructor")
    return TopologyNode::Kind::kConstructor;
  if (s == "method")
    return TopologyNode::Kind::kMethod;
  if (s == "pipeline")
