...
TopologyPrinter::TopologyPrinter(const PrintOptions &opts) : opts_(opts) {}
...
std::string TopologyPrinter::ColorString(const std::string &text, const std::string &color) const {
  if (!opts_.use_color)
    return text;
...
void TopologyPrinter::PrintText(const TopologyGraph &graph, std::ostream &out) const {
  ...
  // Summary line
  out << ColorString("Nodes: ", kBold) << graph.NodeCount() << "  " << ColorString("Edges: ", kBold)
      << graph.EdgeCount() << "\n\n";

  // Topological order (or all nodes if cycle)
  auto sorted = graph.TopologicalSort();
  const auto &nodes = sorted.empty() ? graph.Nodes() : std::vector<TopologyNode>{};
  // Build display list
  ...
  for (const auto *node : display_list) {
    ...
    for (const auto *edge : out_edges) {
      ...
      out << arrow << ColorString(target->name, kBlue) << ":" << tgt_port_name;
  ...
  if (!edge_dist.empty()) {
    ...
    auto status_name = [](TopologyEdge::Status s) -> std::string {
      ...
      case TopologyEdge::Status::kImplicitConvert:
        return "Implicit Convert";
      case TopologyEdge::Status::kExplicitConvert:
        return "Explicit Convert";
...
std::string TopologyPrinter::FormatNodeBox(const TopologyNode &node) const {
  ...
  oss << ColorString("[" + kind_icon + "]", kMagenta) << " "
      << ColorString(node.name, kBold + std::string(kGreen)) << " " << ColorString(lang_tag, kGray);

  if (node.is_linked) {
    oss << " " << ColorString("LINKED", kYellow)
        << ColorString("(" + node.link_source_language + "::" + node.link_source_function + ")",
                       kGray);
  }
  oss << "\n";
  ...
  if (!node.inputs.empty()) {
    ...
    for (const auto &port : node.inputs) {
      oss << ColorString("│", kGray) << "   " << ColorString("◉", kBlue) << " " << port.name;
      if (opts_.show_types) {
  ...
  if (!node.outputs.empty()) {
    ...
    for (const auto &port : node.outputs) {
      oss << ColorString("│", kGray) << "   " << ColorString("◎", kGreen) << " " << port.name;
      if (opts_.show_types) {
        oss << ": " << ColorString(port.type.name.empty() ? "Any" : port.type.name, kCyan);
      }
      oss << "\n";
  ...
  for (const auto &node : graph.Nodes()) {
    ...
    if (!node.inputs.empty()) {
      ...
      for (size_t i = 0; i < node.inputs.size(); ++i) {
        if (i > 0)
          label << "|";
        label << "<" << DotPortId(node.inputs[i].id) << "> " << node.inputs[i].name;
        if (opts_.show_types) {
          label << ": " << (node.inputs[i].type.name.empty() ? "Any" : node.inputs[i].type.name);
        }
      }
      label << "}|";
    }
    ...
    if (!node.outputs.empty()) {
      ...
      for (size_t i = 0; i < node.outputs.size(); ++i) {
        if (i > 0)
          label << "|";
        label << "<" << DotPortId(node.outputs[i].id) << "> " << node.outputs[i].name;
        if (opts_.show_types) {
          label << ": " << (node.outputs[i].type.name.empty() ? "Any" : node.outputs[i].type.name);
        }
      }
      label << "}";
    }
...
void TopologyPrinter::PrintDiagnostics(const std::vector<ValidationDiagnostic> &diagnostics,
  ...
  for (const auto &diag : diagnostics) {
    ...
    if (diag.loc.line > 0) {
      out << " "
          << ColorString("(" + diag.loc.file + ":" + std::to_string(diag.loc.line) + ")", kGray);
    }
    out << "\n";
  }
}
