...
static TopoOptions ParseArgs(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (arg == "generate" && opts.subcommand.empty() && opts.input_file.empty()) {
...
static int Run(const TopoOptions &opts) {
  ...
  if (!opts.view_mode.empty()) {
    ...
    if (valid_mode) {
      graph.RemoveEdgesIf([&](const topo::TopologyEdge &e) { return e.origin != target_origin; });
      ...
      graph.RemoveNodesIf(
          [&](const topo::TopologyNode &n) { return connected.find(n.id) == connected.end(); });
    }
  }
  ...
  if (!opts.filter_language.empty()) {
    ...
    graph.RemoveEdgesIf([&](const topo::TopologyEdge &e) {
      return !graph.GetNode(e.source_node_id) || !graph.GetNode(e.target_node_id);
    });
  }
  ...
  std::cerr << "\nTopology: " << graph.NodeCount() << " nodes, " << graph.EdgeCount() << " edges";
  if (opts.validate) {
    size_t errors = 0, warnings = 0;
    for (const auto &d : validation_diags) {
      if (d.severity == topo::ValidationDiagnostic::Severity::kError)
        errors++;
      if (d.severity == topo::ValidationDiagnostic::Severity::kWarning)
        warnings++;
    }
