...
TEST_CASE("Behavior: C++ add function IR has correct signature", "[behavior][cpp][ir]") {
  std::string code = R"(
        int add(int a, int b) {
            return a + b;
        }
...
TEST_CASE("Behavior: C++ multiple functions produce separate IR entries", "[behavior][cpp][ir]") {
  std::string code = R"(
...
TEST_CASE("Behavior: C++ conditional produces multiple basic blocks", "[behavior][cpp][ir]") {
  std::string code = R"(
...
TEST_CASE("Behavior: x86_64 assembly contains function label and ret", "[behavior][x86][asm]") {
  std::string code = R"(
...
TEST_CASE("Behavior: x86_64 object code has .text with non-zero bytes", "[behavior][x86][obj]") {
  std::string code = R"(
...
TEST_CASE("Failure: C++ syntax error produces diagnostic", "[behavior][failure][cpp]") {
...
FUNC bad_func(x: i32) -> i32 {
  ...
  [[maybe_unused]] bool has_relevant_diag = false;
...
TEST_CASE("Behavior: IR verifier accepts well-formed function", "[behavior][verifier]") {
  ir::IRContext ctx;
  auto fn = ctx.CreateFunction("valid_fn", ir::IRType::I64(), {{"x", ir::IRType::I64()}});
  auto blk = fn->CreateBlock("entry");
  auto ret = std::make_shared<ir::ReturnStatement>();
...
TEST_CASE("Behavior: IR verifier rejects function without terminator", "[behavior][verifier]") {
  ir::IRContext ctx;
  auto fn = ctx.CreateFunction("bad_fn", ir::IRType::I64(), {{"x", ir::IRType::I64()}});
  [[maybe_unused]] auto blk = fn->CreateBlock("entry");
