...
static std::shared_ptr<BinaryInstruction> MakeBin(BinaryInstruction::Op op,
                                                  const std::string &result, const std::string &lhs,
...
static std::shared_ptr<StoreInstruction> MakeStore(const std::string &addr,
...
static std::shared_ptr<LoadInstruction> MakeLoad(const std::string &result, const std::string &addr,
                                                 IRType ty = IRType::I64()) {
...
[[maybe_unused]] static std::shared_ptr<CallInstruction> MakeCall(
    const std::string &result, const std::string &callee, std::vector<std::string> args = {},
...
template <typename T> static size_t CountInst(const Function &func) {
  size_t count = 0;
  for (auto &blk : func.blocks)
    for (auto &inst : blk->instructions)
      if (dynamic_cast<T *>(inst.get()))
        ++count;
  return count;
}
...
[[maybe_unused]] static Instruction *FindInst(Function &func, const std::string &name) {
  for (auto &blk : func.blocks)
    for (auto &inst : blk->instructions)
      if (inst->name == name)
        return inst.get();
...
TEST_CASE("Opt-Level Regression - O3 Stress", "[opt][regression][O3]") {
  ...
  for (int i = 0; i < 200; ++i) {
    auto inst =
        MakeBin(BinaryInstruction::Op::kAdd, "s" + std::to_string(i),
                (i == 0) ? std::string("0") : ("s" + std::to_string(i - 1)), std::to_string(i));
