...
BenchStats ComputeStats(std::vector<double> &samples) {
  ...
  return {sum / static_cast<double>(samples.size()), samples.front(), samples.back(),
          samples[samples.size() / 2], static_cast<int>(samples.size())};
}

void PrintStats(const std::string &label, const BenchStats &s) {
  std::cout << std::fixed << std::setprecision(3);
  std::cout << "  [BENCH] " << label << ": mean=" << s.mean_ms << "ms"
            << " min=" << s.min_ms << "ms"
            << " max=" << s.max_ms << "ms"
