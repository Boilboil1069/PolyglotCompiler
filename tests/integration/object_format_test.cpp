...
uint32_t ReadU32(const std::vector<uint8_t> &buf, std::size_t off) {
  uint32_t v = 0;
  for (int i = 0; i < 4; ++i)
    v |= static_cast<uint32_t>(buf[off + i]) << (i * 8);
  return v;
}
...
void BuildMinimalUnit(std::vector<ObjSection> &sections, std::vector<ObjSymbol> &symbols) {
...
TEST_CASE("Object format: Mach-O64 has LC_DYSYMTAB", "[integration][objfmt][macho]") {
  ...
  uint32_t ncmds = ReadU32(buf, 16);
  [[maybe_unused]] uint32_t sizeofcmds = ReadU32(buf, 20);
...
TEST_CASE("Object format: PDB TPI stream has type records", "[integration][objfmt][pdb]") {
  ...
  for (uint32_t s = 0; s < 2; ++s) {
    uint32_t nblocks = stream_sizes[s] == 0 ? 0 : (stream_sizes[s] + block_size - 1) / block_size;
