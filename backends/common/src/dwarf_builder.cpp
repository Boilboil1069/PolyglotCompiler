...
std::vector<std::uint8_t> DwarfBuilder::BuildDebugStr() {
  ...
  std::vector<std::pair<std::string, std::uint32_t>> sorted_strings(string_table_.begin(),
                                                                    string_table_.end());
...
std::vector<std::uint8_t> DwarfBuilder::BuildDebugInfo() {
  ...
  for (int i = 0; i < 4; ++i)
    result.push_back(0); // Length (filled later)

  result.push_back(4); // DWARF version
  result.push_back(0);

  for (int i = 0; i < 4; ++i)
    result.push_back(0); // Debug abbrev offset
...
std::vector<std::uint8_t> DwarfBuilder::BuildDebugLine() {
  ...
  for (int i = 0; i < 4; ++i)
    result.push_back(0); // Header length
  ...
  std::uint32_t header_length = static_cast<std::uint32_t>(result.size() - header_length_pos - 4);
  for (int i = 0; i < 4; ++i) {
