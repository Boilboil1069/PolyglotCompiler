...
template <typename T> void WriteLE(std::vector<uint8_t> &out, T value) {
...
private:
  ...
  void WriteAbbrevEntry(std::vector<uint8_t> &out, uint32_t code, uint16_t tag, bool has_children,
  ...
  void EncodeLineStatements(std::vector<uint8_t> &out, const std::vector<DebugLineInfo> &lines);
};
...
void DwarfSectionBuilder::WriteAbbrevEntry(std::vector<uint8_t> &out, uint32_t code, uint16_t tag,
                                           bool has_children,
                                           const std::vector<std::pair<uint16_t, uint8_t>> &attrs) {
...
std::vector<uint8_t> DwarfSectionBuilder::BuildDebugAbbrev() {
  ...
  WriteAbbrevEntry(
      result, 4, kDwTagBaseType, false,
      {{kDwAtName, kDwFormStrp}, {kDwAtByteSize, kDwFormData1}, {kDwAtEncoding, kDwFormData1}});
  ...
  WriteAbbrevEntry(
      result, 6, kDwTagFormalParameter, false,
      {{kDwAtName, kDwFormStrp}, {kDwAtType, kDwFormRef4}, {kDwAtLocation, kDwFormExprloc}});

  // Abbreviation 7: DW_TAG_pointer_type
  WriteAbbrevEntry(result, 7, kDwTagPointerType, false,
                   {{kDwAtType, kDwFormRef4}, {kDwAtByteSize, kDwFormData1}});

  // Abbreviation 8: DW_TAG_structure_type
  WriteAbbrevEntry(result, 8, kDwTagStructureType, true,
                   {{kDwAtName, kDwFormStrp}, {kDwAtByteSize, kDwFormData4}});

  // Abbreviation 9: DW_TAG_typedef
  WriteAbbrevEntry(result, 9, kDwTagTypedef, false,
                   {{kDwAtName, kDwFormStrp}, {kDwAtType, kDwFormRef4}});
...
std::vector<uint8_t> DwarfSectionBuilder::BuildDebugInfo(const DebugInfoBuilder &info) {
  ...
  std::vector<BasicTypeInfo> basic_types = {{"int", 4, kDwAteSigned},
                                            {"long", 8, kDwAteSigned},
  ...
  for (const auto &type : info.Types()) {
    if (type_offsets_.find(type.name) == type_offsets_.end()) {
      ...
      if (type.kind == "struct" || type.kind == "class") {
        ...
        WriteLE<uint32_t>(result, base_it != type_offsets_.end() ? base_it->second : 0);
      }
  ...
  for (const auto &var : info.Variables()) {
    ...
    WriteLE<uint32_t>(result, type_it != type_offsets_.end() ? type_it->second : 0);
  }
...
void DwarfSectionBuilder::EncodeLineStatements(std::vector<uint8_t> &out,
...
std::vector<uint8_t> PdbSectionBuilder::Build(const DebugInfoBuilder &info) {
  ...
  // Block 0: MSF Header
  const uint8_t magic[] = {'M', 'i', 'c',  'r',  'o',  's', 'o', 'f', 't', ' ', 'C',
                           '/', 'C', '+',  '+',  ' ',  'M', 'S', 'F', ' ', '7', '.',
                           '0', '0', '\r', '\n', 0x1A, 'D', 'S', 0,   0,   0};
  ...
  for (size_t si = 0; si < streams.size(); ++si) {
    ...
    for (uint32_t block : layout.blocks) {
      ...
      std::memcpy(result.data() + static_cast<size_t>(block) * kBlockSize, data.data() + offset,
                  chunk);
      offset += chunk;
    }
  }
