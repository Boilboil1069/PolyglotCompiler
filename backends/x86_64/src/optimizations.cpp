    ...
    if (!best) {
      ...
      for (auto &node : nodes) {
        if (!node->scheduled && IsReady(node.get())) {
          bool in_list =
              std::find(ready_list.begin(), ready_list.end(), node.get()) != ready_list.end();
          if (!in_list) {
            ready_list.push_back(node.get());
          }
        }
  ...
  for (SchedNode *succ : scheduled_node->successors) {
    if (!succ->scheduled) {
      ...
      if (all_preds_done) {
        ...
        bool in_list = std::find(ready_list.begin(), ready_list.end(), succ) != ready_list.end();
        if (!in_list) {
...
int SoftwarePipeliner::ComputeMII(const std::vector<MachineInstr> &body) {
  ...
  int res_mii = std::max({(alu_ops + 3) / 4, (mem_ops + 1) / 2, (fp_ops + 1) / 2, branch_ops});
...
bool InstructionFusion::FuseToLEA(std::vector<MachineInstr> &insts, size_t pos) {
  ...
  if (first.opcode == Opcode::kShl && second.opcode == Opcode::kAdd) {
    if (first.def >= 0 && second.def >= 0) {
      bool uses_first =
          std::find(second.uses.begin(), second.uses.end(), first.def) != second.uses.end();
...
bool InstructionFusion::FuseCmpJump(std::vector<MachineInstr> &insts, size_t pos) {
  ...
  if (first.opcode == Opcode::kCmp && second.opcode == Opcode::kJcc) {
    ...
    fused.operands.insert(fused.operands.end(), second.operands.begin(), second.operands.end());
...
bool InstructionFusion::FuseLoadOp(std::vector<MachineInstr> &insts, size_t pos) {
  ...
  if (first.opcode == Opcode::kLoad) {
    ...
    if (uses_load) {
      ...
      bool can_fuse = (second.opcode == Opcode::kAdd || second.opcode == Opcode::kSub ||
                       second.opcode == Opcode::kAnd || second.opcode == Opcode::kOr ||
                       second.opcode == Opcode::kXor || second.opcode == Opcode::kCmp);
      ...
      if (can_fuse) {
        ...
        fused.uses.erase(std::remove(fused.uses.begin(), fused.uses.end(), first.def),
                         fused.uses.end());
        fused.operands = second.operands;
        // Add memory operand from load
        for (const auto &op : first.operands) {
          if (op.kind == Operand::Kind::kMemVReg || op.kind == Operand::Kind::kMemLabel ||
              op.kind == Operand::Kind::kStackSlot) {
            fused.operands.push_back(op);
            break;
          }
        }
        fused.latency = GetInstructionLatency(Opcode::kLoad) + GetInstructionLatency(second.opcode);
        fused.cost = 2;
        insts[pos] = fused;
        return true;
      }
...
bool InstructionFusion::FuseToSIMD(std::vector<MachineInstr> &insts, size_t pos) {
  ...
  switch (first.opcode) {
  case Opcode::kAddsd:
    vec_opcode = Opcode::kAddps;
    break;
  case Opcode::kSubsd:
    vec_opcode = Opcode::kSubps;
    break;
  case Opcode::kMulsd:
    vec_opcode = Opcode::kMulps;
    break;
  case Opcode::kDivsd:
    vec_opcode = Opcode::kDivps;
    break;
  default:
    return false;
  }
  ...
  for (size_t i = 0; i < 4; ++i) {
    simd.uses.insert(simd.uses.end(), insts[pos + i].uses.begin(), insts[pos + i].uses.end());
  }
...
std::vector<MachineInstr> MicroArchOptimizer::Optimize(const std::vector<MachineInstr> &insts) {
  ...
  for (size_t i = 0; i < insts.size(); ++i) {
    ...
    if (inst.def >= 0) {
      ...
      if (it != last_writer.end()) {
        ...
        bool reads_prev =
            std::find(inst.uses.begin(), inst.uses.end(), inst.def) != inst.uses.end();
...
void MicroArchOptimizer::BalancePortPressure(std::vector<MachineInstr> &insts) {
  ...
  for (int p = 1; p < 8; ++p) {
    if (port_usage[p] > port_usage[max_port])
      max_port = p;
    if (port_usage[p] < port_usage[min_port])
      min_port = p;
  }
    ...
    for (size_t i = 0; i < insts.size(); ++i) {
      ...
      if (info.port_mask & (1 << min_port)) {
        ...
        for (size_t j = i; j > 0; --j) {
          ...
          if (has_dep || insts[j - 1].terminator)
            break;
...
void MicroArchOptimizer::OptimizeBranchAlignment(std::vector<MachineInstr> &insts) {
  ...
  for (size_t i = 0; i < insts.size(); ++i) {
    ...
    if (inst.opcode == Opcode::kMov)
      inst_size = 3;
    if (inst.opcode == Opcode::kJmp || inst.opcode == Opcode::kJcc)
      inst_size = 5;
...
std::vector<MachineInstr> RegisterRenamer::RenameRegisters(const std::vector<MachineInstr> &insts) {
  if (insts.empty())
    return insts;
...
bool BranchOptimizer::ConvertToCMOV(std::vector<MachineInstr> &insts, size_t pos) {
  ...
  cmov.operands.insert(cmov.operands.end(), branch.operands.begin(), branch.operands.end());
  cmov.latency = 2;
  cmov.cost = 1;
