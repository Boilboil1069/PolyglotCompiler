/**
 * @file     x86_target_backend.cpp
 * @brief    ITargetBackend adapter for the x86-64 backend
 *
 * Wraps the existing @c X86Target code-generation engine in the unified
...
// Translate the common-layer regalloc enum to the backend-specific enum.
::polyglot::backends::x86_64::RegAllocStrategy MapRegAlloc(
    ::polyglot::backends::RegAllocStrategy strategy) {
  switch (strategy) {
  case ::polyglot::backends::RegAllocStrategy::kGraphColoring:
    return ::polyglot::backends::x86_64::RegAllocStrategy::kGraphColoring;
  case ::polyglot::backends::RegAllocStrategy::kLinearScan:
  default:
    return ::polyglot::backends::x86_64::RegAllocStrategy::kLinearScan;
  }
}
...
class X86TargetBackend final : public ::polyglot::backends::ITargetBackend {
public:
  std::string TargetTriple() const override { return "x86_64-unknown-elf"; }

  std::string Description() const override {
    return "x86-64 (AMD64) backend with linear-scan and graph-coloring "
           "register allocators";
  }

  std::vector<std::string> Aliases() const override {
    return {"x86_64", "x86-64", "amd64", "x64", "x86_64-pc-windows-msvc",
            "x86_64-apple-darwin", "x86_64-linux-gnu"};
  }
  ...
  ::polyglot::backends::CompileResult Compile(
    ...
    if (options.emit == ::polyglot::backends::EmitKind::kBitcode ||
      ...
      diag.message = "x86_64 backend does not yet support bitcode/llvm-ir emission";
      result.diagnostics.push_back(std::move(diag));
      result.ok = false;
      return result;
    }

    // Always emit assembly so callers may also inspect the textual form.
    ...
    if (options.emit == ::polyglot::backends::EmitKind::kAssembly) {
      ...
      if (!result.ok) {
        ...
        diag.message = "x86_64 backend produced empty assembly text";
        result.diagnostics.push_back(std::move(diag));
      }
      return result;
    }

    // EmitKind::kObject — populate sections/relocations/symbols.
    ...
    if (mc.sections.empty()) {
      ...
      diag.message = "x86_64 backend produced no sections";
      result.diagnostics.push_back(std::move(diag));
      result.ok = false;
      return result;
    }
...
REGISTER_TARGET_BACKEND(std::make_shared<X86TargetBackend>());
