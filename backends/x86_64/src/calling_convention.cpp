...
const Register kIntegerArgRegs[] = {Register::kRdi, Register::kRsi, Register::kRdx,
                                    Register::kRcx, Register::kR8,  Register::kR9};

const Register kFloatArgRegs[] = {Register::kXmm0, Register::kXmm1, Register::kXmm2,
                                  Register::kXmm3, Register::kXmm4, Register::kXmm5,
                                  Register::kXmm6, Register::kXmm7};

const Register kCalleeSavedRegs[] = {Register::kRbx, Register::kRbp, Register::kR12,
                                     Register::kR13, Register::kR14, Register::kR15};

const Register kVolatileRegs[] = {Register::kRax, Register::kRdi, Register::kRsi,
...
struct CallingConvention {
  ...
  void EmitPrologue(std::ostream &os, const StackFrame &frame) {
    ...
    for (Register reg : frame.saved_regs) {
      offset -= 8;
      os << "  mov [rbp - " << (frame.total_size - offset) << "], " << RegisterName(reg) << "\n";
    }
  }
  ...
  void EmitEpilogue(std::ostream &os, const StackFrame &frame) {
    ...
    for (Register reg : frame.saved_regs) {
      offset -= 8;
      os << "  mov " << RegisterName(reg) << ", [rbp - " << (frame.total_size - offset) << "]\n";
    }
    ...
    for (int i = 0; i < num_args; ++i) {
      ...
      if (is_float) {
        if (float_arg_idx < static_cast<int>(float_regs.size())) {
          os << "  movsd " << RegisterName(float_regs[float_arg_idx]) << ", " << arg_str << "\n";
          ++float_arg_idx;
        } else {
