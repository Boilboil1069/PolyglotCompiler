  ...
  for (auto &fn : module_->Functions()) {
    ...
    for (auto &kv : alloc.vreg_to_phys) {
      if (is_callee_saved(kv.second))
        callee_saved_used.push_back(kv.second);
    }
    // unique
    std::sort(callee_saved_used.begin(), callee_saved_used.end());
    callee_saved_used.erase(std::unique(callee_saved_used.begin(), callee_saved_used.end()),
                            callee_saved_used.end());

    int stack_bytes = alloc.stack_slots * 16 + static_cast<int>(callee_saved_used.size()) * 16;
    if (stack_bytes % 16 != 0)
      stack_bytes = ((stack_bytes + 15) / 16) * 16;
      ...
      for (const auto &mi : bb.instructions) {
        ...
        auto format_op = [&](const Operand &op) -> std::string {
          ...
          case Operand::Kind::kPhysReg:
            return RegisterName(op.phys);
          case Operand::Kind::kVReg: {
            if (alloc.vreg_to_phys.count(op.vreg))
              return RegisterName(alloc.vreg_to_phys.at(op.vreg));
          ...
          case Operand::Kind::kMemVReg: {
            int base = op.vreg;
            Register reg =
                alloc.vreg_to_phys.count(base) ? alloc.vreg_to_phys.at(base) : Register::kX0;
        ...
        case Opcode::kMov:
          if (mi.operands.empty())
            break;
          os << pre.str();
          os << "  mov " << RegisterName(mi.def >= 0 ? def_reg : Resolve(mi.operands[0], alloc))
             << ", " << format_op(mi.operands[0]) << "\n";
          break;
        case Opcode::kAdd:
          os << pre.str();
          os << "  add " << RegisterName(def_reg) << ", " << format_op(mi.operands[0]) << ", "
             << format_op(mi.operands[1]) << "\n";
          break;
        case Opcode::kSub:
          os << pre.str();
          os << "  sub " << RegisterName(def_reg) << ", " << format_op(mi.operands[0]) << ", "
             << format_op(mi.operands[1]) << "\n";
          break;
        case Opcode::kMul:
          os << pre.str();
          os << "  mul " << RegisterName(def_reg) << ", " << format_op(mi.operands[0]) << ", "
             << format_op(mi.operands[1]) << "\n";
          break;
        case Opcode::kDiv:
        case Opcode::kSDiv:
          os << pre.str();
          os << "  sdiv " << RegisterName(def_reg) << ", " << format_op(mi.operands[0]) << ", "
             << format_op(mi.operands[1]) << "\n";
          break;
        case Opcode::kUDiv:
          os << pre.str();
          os << "  udiv " << RegisterName(def_reg) << ", " << format_op(mi.operands[0]) << ", "
             << format_op(mi.operands[1]) << "\n";
          break;
        case Opcode::kRem:
        case Opcode::kSRem: {
        ...
        case Opcode::kAnd:
          os << pre.str();
          os << "  and " << RegisterName(def_reg) << ", " << format_op(mi.operands[0]) << ", "
             << format_op(mi.operands[1]) << "\n";
          break;
        case Opcode::kOr:
          os << pre.str();
          os << "  orr " << RegisterName(def_reg) << ", " << format_op(mi.operands[0]) << ", "
             << format_op(mi.operands[1]) << "\n";
          break;
        case Opcode::kXor:
          os << pre.str();
          os << "  eor " << RegisterName(def_reg) << ", " << format_op(mi.operands[0]) << ", "
             << format_op(mi.operands[1]) << "\n";
          break;
        case Opcode::kShl:
          os << pre.str();
          os << "  lsl " << RegisterName(def_reg) << ", " << format_op(mi.operands[0]) << ", "
             << format_op(mi.operands[1]) << "\n";
          break;
        case Opcode::kLShr:
          os << pre.str();
          os << "  lsr " << RegisterName(def_reg) << ", " << format_op(mi.operands[0]) << ", "
             << format_op(mi.operands[1]) << "\n";
          break;
        case Opcode::kAShr:
          os << pre.str();
          os << "  asr " << RegisterName(def_reg) << ", " << format_op(mi.operands[0]) << ", "
             << format_op(mi.operands[1]) << "\n";
          break;
        case Opcode::kCmp:
          os << pre.str();
        ...
        case Opcode::kJmp:
          os << pre.str();
          if (!mi.operands.empty())
            os << "  b " << format_op(mi.operands[0]) << "\n";
          break;
        case Opcode::kJcc:
          os << pre.str();
          if (mi.operands.size() >= 1)
            os << "  b.ne " << format_op(mi.operands[0]) << "\n";
          if (mi.operands.size() >= 2)
            os << "  b " << format_op(mi.operands[1]) << "\n";
          break;
        default:
          os << "  ; unhandled opcode\n";
          break;
    ...
    for (const auto &bb : mf.blocks) {
      ...
      for (const auto &mi : bb.instructions) {
        ...
        case Opcode::kXor: {
          ...
          Register lhs = Resolve(mi.operands[0], alloc);
          Register rhs = Resolve(mi.operands[1], alloc);
          std::uint32_t insn = (mi.opcode == Opcode::kAnd)
                                   ? 0x8A000000
                                   : (mi.opcode == Opcode::kOr ? 0xAA000000 : 0xCA000000);
          insn |= (RegCode(rhs) << 16);
          insn |= (RegCode(lhs) << 5);
          insn |= RegCode(dst);
          Emit32(text_sec.data, insn);
        ...
        case Opcode::kAShr: {
          ...
          if (mi.opcode == Opcode::kLShr)
            insn = 0x9AC02400; // LSRV
          if (mi.opcode == Opcode::kAShr)
            insn = 0x9AC02800; // ASRV
          insn |= (RegCode(rhs) << 16);
          insn |= (RegCode(lhs) << 5);
          insn |= RegCode(dst);
          Emit32(text_sec.data, insn);
        ...
        case Opcode::kUDiv: {
          ...
          Register rhs = Resolve(mi.operands[1], alloc);
          std::uint32_t insn =
              (mi.opcode == Opcode::kUDiv) ? 0x9AC00800 : 0x9AC00C00; // UDIV / SDIV
          insn |= (RegCode(rhs) << 16);
          insn |= (RegCode(lhs) << 5);
          insn |= RegCode(dst);
          Emit32(text_sec.data, insn);
        ...
        case Opcode::kURem: {
          ...
          std::uint32_t div_insn =
              (mi.opcode == Opcode::kURem) ? 0x9AC00800 : 0x9AC00C00; // UDIV/SDIV q, lhs, rhs
        ...
        case Opcode::kJmp: {
          ...
          MCReloc r{.section = ".text",
                    .offset = static_cast<std::uint32_t>(reloc_offset),
                    .type = 1,
                    .symbol = mi.operands[0].label,
                    .addend = 0};
