...
bool DevirtualizationPass::TryDevirtualize(ir::CallInstruction *call, ir::BasicBlock *block,
                                           ir::Function *func) {
  if (!call)
    return false;
...
const ir::MethodInfo *DevirtualizationPass::GetUniqueImplementation(
  ...
  const ir::MethodInfo *base_impl = class_metadata_.FindVirtualMethod(class_name, method_name);
  if (base_impl) {
  ...
  for (const auto &cand : candidate_classes) {
    if (cand == class_name)
      continue;
    if (!class_metadata_.IsBaseOf(class_name, cand))
      continue;

    const ir::MethodInfo *derived_impl = class_metadata_.FindVirtualMethod(cand, method_name);
    if (derived_impl) {
