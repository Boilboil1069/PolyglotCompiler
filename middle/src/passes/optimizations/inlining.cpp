...
void RunInlining(ir::IRContext &context) {
  ...
  auto apply_subst = [](ir::Function &fn,
  ...
  auto clone_inst = [](const ir::Instruction &inst) -> std::shared_ptr<ir::Instruction> {
    if (auto *bin = dynamic_cast<const ir::BinaryInstruction *>(&inst))
    ...
    if (auto *call = dynamic_cast<const ir::CallInstruction *>(&inst))
      return std::make_shared<ir::CallInstruction>(*call);
    if (auto *alloca_inst = dynamic_cast<const ir::AllocaInstruction *>(&inst))
      return std::make_shared<ir::AllocaInstruction>(*alloca_inst);
    if (auto *load = dynamic_cast<const ir::LoadInstruction *>(&inst))
      return std::make_shared<ir::LoadInstruction>(*load);
    if (auto *store = dynamic_cast<const ir::StoreInstruction *>(&inst))
      return std::make_shared<ir::StoreInstruction>(*store);
    if (auto *cast = dynamic_cast<const ir::CastInstruction *>(&inst))
      return std::make_shared<ir::CastInstruction>(*cast);
    if (auto *gep = dynamic_cast<const ir::GetElementPtrInstruction *>(&inst))
      return std::make_shared<ir::GetElementPtrInstruction>(*gep);
    if (auto *ret = dynamic_cast<const ir::ReturnStatement *>(&inst))
      return std::make_shared<ir::ReturnStatement>(*ret);
    if (auto *br = dynamic_cast<const ir::BranchStatement *>(&inst))
      return std::make_shared<ir::BranchStatement>(*br);
    if (auto *cbr = dynamic_cast<const ir::CondBranchStatement *>(&inst))
      return std::make_shared<ir::CondBranchStatement>(*cbr);
    if (auto *sw = dynamic_cast<const ir::SwitchStatement *>(&inst))
      return std::make_shared<ir::SwitchStatement>(*sw);
    return nullptr;
  };
  ...
  for (auto &fn : functions) {
    for (auto &bb_ptr : fn->blocks) {
      ...
      for (auto it = insts.begin(); it != insts.end(); ++it) {
        ...
        if (!call || call->is_indirect || call->callee.empty())
          continue;
        auto callee_it = fn_by_name.find(call->callee);
        if (callee_it == fn_by_name.end())
          continue;
        ...
        auto rename = [&](std::string &s) {
          ...
          if (it_sub != subst_params.end())
            s = it_sub->second;
          auto it_local = local_names.find(s);
          if (it_local != local_names.end())
            s = it_local->second;
        };
