...
std::vector<InductionVariable> FindInductionVariables(
    const ir::LoopInfo &loop, const std::unordered_set<ir::BasicBlock *> &loop_blocks) {
  ...
  for (const auto &phi : loop.header->phis) {
    ...
    if (init_val.empty() || loop_val.empty())
      continue;
...
std::shared_ptr<ir::Instruction> CloneInstructionImpl(
    const std::shared_ptr<ir::Instruction> &inst, size_t iteration,
...
void ConvertTailRecursionToLoop(ir::Function &func) {
  ...
  for (auto &[bb, call] : tail_calls) {
    ...
    bb->instructions.insert(bb->instructions.end(), new_insts.begin(), new_insts.end());
    ...
    if (std::find(original_entry->predecessors.begin(), original_entry->predecessors.end(), bb) ==
...
void LoopInvariantCodeMotion(ir::Function &func) {
  ...
  for (const auto &loop : loops) {
    ...
    std::unordered_set<ir::BasicBlock *> loop_blocks(loop.blocks.begin(), loop.blocks.end());
      ...
      if (auto alloca = std::dynamic_pointer_cast<ir::AllocaInstruction>(inst)) {
        ...
        if (alloca->no_escape) {
          ...
          const auto &elem_type =
              alloca->type.subtypes.empty() ? ir::IRType::Invalid() : alloca->type.subtypes[0];
  ...
  for (auto &bb : func.blocks) {
    ...
    insts.erase(
        std::remove_if(insts.begin(), insts.end(),
                       [](const std::shared_ptr<ir::Instruction> &inst) { return inst->is_dead; }),
        insts.end());
  }
}
  ...
  while (!worklist.empty()) {
    ...
    if (auto br = std::dynamic_pointer_cast<ir::BranchStatement>(bb->terminator)) {
      ...
      if (cond_val.IsConstant()) {
        ...
        ir::BasicBlock *target = cond_val.constant ? cond->true_target : cond->false_target;
        ...
        if (auto br = std::dynamic_pointer_cast<ir::BranchStatement>(bb->terminator)) {
          if (br->target && br->target != bb.get()) {
            ...
            for (auto *pred : bb->predecessors) {
              ...
              if (auto pred_br = std::dynamic_pointer_cast<ir::BranchStatement>(pred->terminator)) {
                if (pred_br->target == bb.get()) {
                  pred_br->target = target;
                  changed = true;
                }
              } else if (auto pred_cond =
                             std::dynamic_pointer_cast<ir::CondBranchStatement>(pred->terminator)) {
              ...
              if (std::find(target->predecessors.begin(), target->predecessors.end(), pred) ==
...
void PrefetchInsertion(ir::Function &func) {
  ...
  for (const auto &loop : loops) {
    ...
    std::vector<std::pair<ir::BasicBlock *, std::shared_ptr<ir::LoadInstruction>>> loads;
      ...
      if (auto bin = std::dynamic_pointer_cast<ir::BinaryInstruction>(inst)) {
        ...
        if (lhs_const && rhs_const) {
          ...
          case ir::BinaryInstruction::Op::kLShr:
            result = static_cast<int64_t>(static_cast<uint64_t>(lhs_val) >> rhs_val);
            break;
...
void LoopFission(ir::Function &func) {
  ...
  for (const auto &loop : loops) {
    if (loop.blocks.size() != 1)
      continue; // Only single-block loops
    ir::BasicBlock *body = loop.blocks[0];
    ...
    for (auto &inst : body->instructions) {
      ...
      for (size_t g = 0; g < groups.size(); ++g) {
        ...
        if (target_group < 0 && !inst->name.empty() && groups[g].uses.count(inst->name)) {
          target_group = static_cast<int>(g);
        }
        if (target_group >= 0)
          break;
...
void LoopInterchange(ir::Function &func) {
  ...
  for (size_t i = 0; i < loops.size(); ++i) {
    for (size_t j = 0; j < loops.size(); ++j) {
      ...
      std::unordered_set<ir::BasicBlock *> outer_set(outer.blocks.begin(), outer.blocks.end());
      bool nested = outer_set.count(inner.header) > 0;
      if (!nested)
        continue;

      // Find induction variables for both loops
      std::unordered_set<ir::BasicBlock *> inner_set(inner.blocks.begin(), inner.blocks.end());
      auto outer_ivs = FindInductionVariables(outer, outer_set);
      auto inner_ivs = FindInductionVariables(inner, inner_set);
      if (outer_ivs.empty() || inner_ivs.empty())
        continue;
      ...
      if (inner.header && outer.header && !inner.header->phis.empty() &&
          !outer.header->phis.empty()) {
...
void LoopTiling(ir::Function &func, size_t tile_size) {
  ...
  for (const auto &loop : loops) {
    ...
    if (trip_count <= 0 || static_cast<size_t>(trip_count) <= tile_size)
      continue;
    ...
    for (auto *block : loop.blocks) {
      for (auto &inst : block->instructions) {
        if (inst->name == primary_iv.name) {
          if (auto bin = std::dynamic_pointer_cast<ir::BinaryInstruction>(inst)) {
            if (bin->op == ir::BinaryInstruction::Op::kAdd && bin->operands.size() >= 2) {
  ...
  for (auto &bb : func.blocks) {
    for (auto &inst : bb->instructions) {
      if (auto alloca = std::dynamic_pointer_cast<ir::AllocaInstruction>(inst)) {
        ...
        if (alias.IsLocalStack(alloca->name) && !alias.IsAddrTaken(alloca->name)) {
          alloca->no_escape = true;
        }
      }
    }
...
void CodeSinking(ir::Function &func) {
  ...
  for (auto &bb : func.blocks) {
    ...
    for (auto it = insts.begin(); it != insts.end();) {
      ...
      if (std::dynamic_pointer_cast<ir::StoreInstruction>(inst) ||
          std::dynamic_pointer_cast<ir::CallInstruction>(inst) || inst->name.empty()) {
        ++it;
        continue;
      }
      ...
      if (use_blocks.size() == 1) {
        ...
        if (target != bb.get()) {
          ...
          if (dominates) {
            ...
            auto moved = inst;
            moved->parent = target;
            target->instructions.insert(target->instructions.begin(), moved);
            it = insts.erase(it);
            continue;
          }
        }
...
void CodeHoisting(ir::Function &func) {
  ...
  for (auto &bb : func.blocks) {
    ...
    for (size_t i = 0; i < succ0->instructions.size(); ++i) {
      for (size_t j = 0; j < succ1->instructions.size(); ++j) {
        ...
        if (std::dynamic_pointer_cast<ir::StoreInstruction>(a) ||
            std::dynamic_pointer_cast<ir::CallInstruction>(a))
          continue;
        if (std::dynamic_pointer_cast<ir::StoreInstruction>(b) ||
            std::dynamic_pointer_cast<ir::CallInstruction>(b))
          continue;
    ...
    for (auto &[idx0, idx1] : common_pairs) {
      if (idx0 >= succ0->instructions.size() || idx1 >= succ1->instructions.size())
        continue;
...
void GVN(ir::Function &func) {
  ...
  std::function<void(ir::BasicBlock *)> process_block = [&](ir::BasicBlock *bb) {
    if (!bb)
      return;
    ...
    for (auto &inst : bb->instructions) {
      ...
      if (auto bin = std::dynamic_pointer_cast<ir::BinaryInstruction>(inst)) {
        ...
        if (bin->operands.size() >= 2) {
          ...
          if (value_number.count(op0))
            op0 = value_number[op0];
          if (value_number.count(op1))
            op1 = value_number[op1];
          if (commutative && op0 > op1)
            std::swap(op0, op1);
          key += "|" + op0 + "|" + op1;
        }
      } else if (auto load = std::dynamic_pointer_cast<ir::LoadInstruction>(inst)) {
        if (!load->operands.empty()) {
          ...
          if (value_number.count(addr))
            addr = value_number[addr];
          key = "load|" + addr;
        }
      }
...
void BranchPredictionOptimization(ir::Function &func) {
  ...
  for (auto &bb : func.blocks) {
    ...
    if (cond->false_target && loop_headers.count(cond->false_target) && cond->true_target &&
        !loop_headers.count(cond->true_target)) {
    ...
    if (cond->true_target && cond->true_target->successors.empty() && cond->false_target &&
        !cond->false_target->successors.empty()) {
      std::swap(cond->true_target, cond->false_target);
      if (!cond->operands.empty()) {
        if (cond->operands[0].front() == '!') {
          cond->operands[0] = cond->operands[0].substr(1);
...
void LoopPredication(ir::Function &func) {
  ...
  for (const auto &loop : loops) {
    ...
    for (auto *block : loop.blocks) {
      auto cond = std::dynamic_pointer_cast<ir::CondBranchStatement>(block->terminator);
      if (!cond)
        continue;

      // Only predicate if both targets are in the loop
      if (!cond->true_target || !cond->false_target)
        continue;
      if (!loop_blocks.count(cond->true_target) || !loop_blocks.count(cond->false_target))
        continue;
      ...
      if (true_bb->instructions.size() != 1)
        continue;

      auto br = std::dynamic_pointer_cast<ir::BranchStatement>(true_bb->terminator);
      if (!br || br->target != cond->false_target)
        continue;
...
void MemoryLayoutOptimization(ir::Function &func) {
  ...
  std::sort(allocas.begin(), allocas.end(), [](const AllocaUsage &a, const AllocaUsage &b) {
