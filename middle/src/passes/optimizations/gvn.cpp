...
GVNPass::GVNPass(ir::Function &func) : func_(func), next_value_number_(1) {}
...
GVNPass::Expression GVNPass::CreateExpression(Instruction *inst) {
  ...
  if (auto *bin = dynamic_cast<BinaryInstruction *>(inst)) {
    ...
    if (bin->operands.size() >= 2) {
      ...
      if (bin->op == BinaryInstruction::Op::kAdd || bin->op == BinaryInstruction::Op::kMul ||
...
bool GVNPass::IsPure(Instruction *inst) const {
  ...
  if (dynamic_cast<StoreInstruction *>(inst))
    return false;
  if (dynamic_cast<CallInstruction *>(inst))
    return false;
  if (dynamic_cast<InvokeInstruction *>(inst))
    return false;
    ...
    for (auto &bb_ptr : func_.blocks) {
      ...
      for (auto *pred : bb->predecessors) {
        ...
        if (first_pred) {
          ...
          std::set_intersection(new_in.begin(), new_in.end(), pred_out.begin(), pred_out.end(),
    ...
    for (auto it = func_.blocks.rbegin(); it != func_.blocks.rend(); ++it) {
      ...
      for (auto *succ : bb->successors) {
        ...
        if (first_succ) {
          ...
          std::set<Expression, ExpressionCompare> intersection;
          std::set_intersection(new_out.begin(), new_out.end(), succ_in.begin(), succ_in.end(),
                                std::inserter(intersection, intersection.begin()),
                                ExpressionCompare{});
...
AliasAnalysisPass::AliasResult AliasAnalysisPass::Query(const std::string &ptr1,
