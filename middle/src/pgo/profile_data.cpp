...
const FunctionProfile *ProfileData::GetFunctionProfile(const std::string &name) const {
...
void RuntimeProfiler::RecordBranch(const std::string &func, size_t branch_id, bool taken) {
  if (!enabled_)
    return;
...
PGOOptimizer::PGOOptimizer(const ProfileData &profile) : profile_(profile), config_() {}

PGOOptimizer::PGOOptimizer(const ProfileData &profile, const Config &config) :
    profile_(profile), config_(config) {}

std::vector<PGOOptimizer::InliningDecision> PGOOptimizer::MakeInliningDecisions() const {
  std::vector<InliningDecision> decisions;

  // Iterate over all function profiles using the public ForEachFunction API
  profile_.ForEachFunction([&](const std::string &func_name, const FunctionProfile &profile) {
    // Process each call site in this function
    for (const auto &call_site : profile.call_sites) {
      InliningDecision decision = MakeInliningDecision(func_name, call_site, profile);
...
PGOOptimizer::InliningDecision PGOOptimizer::MakeInliningDecision(
    const std::string &caller, const CallSiteProfile &call_site,
  ...
  const FunctionProfile *callee_profile = profile_.GetFunctionProfile(call_site.callee_name);
  size_t callee_size = callee_profile ? callee_profile->estimated_size : call_site.callee_size;
  ...
  if (is_hot_call && callee_size <= config_.max_inline_size) {
    ...
    if (net_benefit > 0) {
      ...
      decision.reason =
          "Hot call site but callee too large (size=" + std::to_string(callee_size) + ")";
    }
    ...
    decision.priority = 1;
    decision.reason =
        "Very small callee (" + std::to_string(callee_size) + " instructions), always inline";
    ...
    decision.reason = "Cold call site (" + std::to_string(call_site.call_count) +
                      " calls) or large callee (" + std::to_string(callee_size) + " instructions)";
  }
...
std::vector<PGOOptimizer::BranchPredictionHint> PGOOptimizer::GetBranchPredictions() const {
...
PGOOptimizer::BranchPredictionHint PGOOptimizer::MakeBranchPrediction(
    const std::string &function, const BranchProfile &branch) const {
  ...
  if (total == 0) {
    ...
    hint.reason = "Highly likely taken (" + std::to_string(static_cast<int>(prob * 100)) +
                  "% taken, " + std::to_string(total) + " samples)";
  } else if (prob <= 0.05) {
    hint.reason = "Highly unlikely taken (" + std::to_string(static_cast<int>(prob * 100)) +
                  "% taken, " + std::to_string(total) + " samples)";
  } else if (prob >= 0.8) {
    hint.reason = "Likely taken (" + std::to_string(static_cast<int>(prob * 100)) + "% taken)";
  } else if (prob <= 0.2) {
    hint.reason = "Unlikely taken (" + std::to_string(static_cast<int>(prob * 100)) + "% taken)";
  } else {
    hint.reason =
        "Uncertain prediction (" + std::to_string(static_cast<int>(prob * 100)) + "% taken)";
  }

  return hint;
}

std::vector<PGOOptimizer::CodeLayoutHint> PGOOptimizer::OptimizeCodeLayout() const {
...
std::vector<PGOOptimizer::LoopOptimizationHint> PGOOptimizer::OptimizeLoops() const {
  ...
  profile_.ForEachFunction([&](const std::string &func_name, const FunctionProfile &profile) {
    ...
    for (const auto &bb : profile.basic_blocks) {
      ...
      if (profile.invocation_count > 0 && bb.execution_count > profile.invocation_count * 10) {
        ...
        if (estimated_iterations >= config_.loop_unroll_threshold && estimated_iterations <= 16) {
          hint.should_unroll = true;
          hint.unroll_factor = static_cast<size_t>(std::min<uint64_t>(estimated_iterations, 8));
          hint.reason =
              "Loop executes ~" + std::to_string(estimated_iterations) + " iterations on average";
        }

        // Suggest vectorization for larger iteration counts
        if (estimated_iterations > 16) {
          hint.should_vectorize = true;
          hint.reason = "Loop executes ~" + std::to_string(estimated_iterations) +
...
std::vector<PGOOptimizer::DevirtualizationHint> PGOOptimizer::FindDevirtualizationOpportunities()
    const {
    ...
    for (const auto &call_site : profile.call_sites) {
      ...
      if (!call_site.target_distribution.empty()) {
        ...
        if (!most_likely.first.empty()) {
          ...
          if (total_calls > 0) {
            double dominant_ratio = static_cast<double>(most_likely.second) / total_calls;
            ...
            if (dominant_ratio >= config_.devirt_threshold) {
              ...
              hint.reason = "Virtual call has dominant target (" +
                            std::to_string(static_cast<int>(dominant_ratio * 100)) + "% calls to " +
                            most_likely.first + ")";
              hints.push_back(hint);
            }
          }
...
std::vector<PGOOptimizer::SpecializationHint> PGOOptimizer::FindSpecializationOpportunities()
    const {
...
std::vector<PGOOptimizer::PrefetchHint> PGOOptimizer::GetPrefetchHints() const {
  ...
  // Sort by expected benefit
  std::sort(hints.begin(), hints.end(), [](const PrefetchHint &a, const PrefetchHint &b) {
    return a.expected_benefit > b.expected_benefit;
  });
...
PGOOptimizer::OptimizationReport PGOOptimizer::GenerateReport() const {
  ...
  for (const auto &d : report.inlining_decisions) {
    if (d.should_inline)
      total_benefit += d.expected_benefit;
  }
...
bool PGOOptimizer::ExportReportToJson(const std::string &filename) const {
  ...
  json j;
  j["version"] = "1.0";
  j["summary"] = {{"total_functions", report.total_functions},
...
void ProfileInstrumentation::GenerateProfilerCalls(ir::Function &func) {
  ...
  for (auto &blk : func.blocks) {
    for (auto &inst : blk->instructions) {
      ...
      if (!ci || ci->callee.rfind("__polyglot_pgo_", 0) == 0)
        continue;
      auto call = std::make_shared<ir::CallInstruction>();
      call->name = "";
