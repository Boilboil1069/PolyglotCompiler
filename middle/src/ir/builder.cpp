...
IRType ComputeGEPType(const IRType &base_type, const std::vector<size_t> &indices) {
  IRType base_ptr =
      (base_type.kind == IRTypeKind::kPointer || base_type.kind == IRTypeKind::kReference)
          ? base_type
...
std::shared_ptr<StoreInstruction> IRBuilder::MakeStore(const std::string &addr,
                                                       const std::string &value, size_t align) {
  auto bb = CurrentBlock();
...
std::shared_ptr<CallInstruction> IRBuilder::MakeCall(const std::string &callee,
                                                     ...
                                                     const std::string &name, const IRType &fn_type,
                                                     bool is_vararg) {
  auto bb = CurrentBlock();
...
std::shared_ptr<MemcpyInstruction> IRBuilder::MakeMemcpy(const std::string &dst,
...
std::shared_ptr<MemsetInstruction> IRBuilder::MakeMemset(const std::string &dst,
                                                         const std::string &value,
                                                         const std::string &size_name,
                                                         size_t align) {
  auto bb = CurrentBlock();
...
std::shared_ptr<InvokeInstruction> IRBuilder::MakeInvoke(
    const std::string &callee, const std::vector<std::string> &args, const IRType &ret_type,
    BasicBlock *normal_dest, BasicBlock *unwind_dest, const std::string &name) {
  auto bb = CurrentBlock();
...
std::shared_ptr<LandingPadInstruction> IRBuilder::MakeLandingPad(
    bool is_cleanup, const std::vector<IRType> &catch_types, const std::string &name) {
  auto bb = CurrentBlock();
...
std::shared_ptr<Statement> IRBuilder::MakeReturn(const std::string &value_name) {
  ...
  if (!value_name.empty())
    ret->operands.push_back(value_name);
  ret->parent = bb.get();
...
std::shared_ptr<SwitchStatement> IRBuilder::MakeSwitch(
    const std::string &value, const std::vector<SwitchStatement::Case> &cases,
...
std::shared_ptr<PhiInstruction> IRBuilder::MakePhi(
    const IRType &type, const std::vector<std::pair<BasicBlock *, std::string>> &incomings,
    const std::string &name) {
  auto bb = CurrentBlock();
