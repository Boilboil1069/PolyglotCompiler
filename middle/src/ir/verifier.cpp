...
bool IsDefined(const std::string &name, const std::unordered_set<std::string> &defs,
  ...
  if (name.empty())
    return true; // allow empty/literal operands
  if (name == "undef")
...
bool CheckBinary(const BinaryInstruction &bin, const IRType &lhs, const IRType &rhs,
                 std::string *msg) {
  ...
  auto is_generic = [](const IRType &t) {
    return t.kind == IRTypeKind::kI64 || t.kind == IRTypeKind::kInvalid ||
           t.kind == IRTypeKind::kPointer || t.kind == IRTypeKind::kReference || t.is_placeholder;
  };
  ...
  case BinaryInstruction::Op::kCmpNe:
    if (!require_same_scalar())
      return false;
    if (bin.type.kind != IRTypeKind::kI1)
      return Fail("cmp result must be i1", msg);
    return true;
...
bool CheckCast(const CastInstruction &cast, const IRType &src, std::string *msg) {
  ...
  case CastInstruction::CastKind::kSExt:
    if (!src.IsInteger() || !dst.IsInteger())
      return Fail("zext/sext require integer types", msg);
    if (!src.CanLosslesslyConvertTo(dst))
      return Fail("zext/sext must widen", msg);
    return true;
  case CastInstruction::CastKind::kTrunc:
    if (!src.IsInteger() || !dst.IsInteger())
      return Fail("trunc requires integer types", msg);
    if (!dst.CanLosslesslyConvertTo(src))
      return Fail("trunc must narrow", msg);
    return true;
  case CastInstruction::CastKind::kBitcast:
    if (!src.CanBitcastTo(dst))
      return Fail("illegal bitcast", msg);
    return true;
  case CastInstruction::CastKind::kFpExt:
    if (!src.IsFloat() || !dst.IsFloat())
      return Fail("fpext requires float types", msg);
    if (!src.CanLosslesslyConvertTo(dst))
      return Fail("fpext must widen", msg);
    return true;
  case CastInstruction::CastKind::kFpTrunc:
    if (!src.IsFloat() || !dst.IsFloat())
      return Fail("fptrunc requires float types", msg);
    if (!dst.CanLosslesslyConvertTo(src))
  ...
  case CastInstruction::CastKind::kIntToPtr:
    ...
    if (dst.kind != IRTypeKind::kPointer && dst.kind != IRTypeKind::kReference)
      return Fail("inttoptr requires pointer dest", msg);
    return true;
  case CastInstruction::CastKind::kPtrToInt:
    if (src.kind != IRTypeKind::kPointer && src.kind != IRTypeKind::kReference)
      return Fail("ptrtoint requires pointer source", msg);
    if (!dst.IsInteger())
      return Fail("ptrtoint requires integer dest", msg);
    return true;
  }
  return Fail("unknown cast kind", msg);
}

bool CheckGEP(const GetElementPtrInstruction &gep, const IRType &base_ptr, const DataLayout *layout,
              std::string *msg) {
  if (!(base_ptr.kind == IRTypeKind::kPointer || base_ptr.kind == IRTypeKind::kReference)) {
    return Fail("gep base is not a pointer", msg);
  }
  if (base_ptr.subtypes.empty())
    return Fail("gep base missing pointee", msg);
  ...
  for (size_t idx : gep.indices) {
    ...
    case IRTypeKind::kVector:
      ...
      if (idx >= cur.count)
        return Fail("gep index out of bounds", msg);
      if (cur.subtypes.empty())
        return Fail("gep on array/vector with unknown element", msg);
      if (layout) {
        size_t elem_size = layout->SizeOf(cur.subtypes[0]);
        size_t elem_align = layout->AlignOf(cur.subtypes[0]);
        if (elem_size == 0)
          return Fail("gep element has zero size", msg);
  ...
  if (!expect.SameShape(gep.type))
    return Fail("gep result type mismatch", msg);

  // If layout exists, ensure we can compute size/align of the final pointee (not used yet for
  // offset, but rejects unsized types).
  if (layout) {
    size_t align = layout->AlignOf(cur);
    size_t size = layout->SizeOf(cur);
    if (size == 0 && cur.kind != IRTypeKind::kFunction && cur.kind != IRTypeKind::kVoid &&
        cur.kind != IRTypeKind::kInvalid) {
...
static bool VerifyImpl(const Function &func, const DataLayout *layout,
  ...
  if (func.blocks.empty())
    return true;
  if (!func.entry)
    return Fail("function has no entry block", msg);
  if (func.ret_type.kind == IRTypeKind::kInvalid)
    return Fail("function missing return type", msg);
  ...
  for (auto &bb_ptr : func.blocks) {
    ...
    if (!block_names.insert(bb_ptr->name).second)
      return Fail("duplicate block name: " + bb_ptr->name, msg);
  }
  if (block_set.count(func.entry) == 0)
    return Fail("entry block not in function blocks", msg);
  ...
  auto add_edge = [&](const BasicBlock *from, const BasicBlock *to) -> bool {
    if (!to)
      return Fail("edge targets null block", msg);
    if (block_set.count(to) == 0)
      return Fail("terminator targets block outside function", msg);
  ...
  for (auto &bb_ptr : func.blocks) {
    auto *bb = bb_ptr.get();
    if (!bb->terminator)
      return Fail("block missing terminator: " + bb->name, msg);
    auto *term = bb->terminator.get();
    if (auto *br = dynamic_cast<BranchStatement *>(term)) {
      if (!br->target)
        return Fail("branch missing target in block " + bb->name, msg);
      if (!add_edge(bb, br->target))
        return false;
    } else if (auto *cbr = dynamic_cast<CondBranchStatement *>(term)) {
      if (!cbr->true_target || !cbr->false_target)
        return Fail("condbr missing target in block " + bb->name, msg);
      if (!add_edge(bb, cbr->true_target))
        return false;
      if (!add_edge(bb, cbr->false_target))
        return false;
    } else if (auto *sw = dynamic_cast<SwitchStatement *>(term)) {
      for (auto &c : sw->cases) {
        if (!c.target)
          return Fail("switch case missing target in block " + bb->name, msg);
        if (!add_edge(bb, c.target))
          return false;
      }
      if (!sw->default_target)
        return Fail("switch missing default target in block " + bb->name, msg);
      if (!add_edge(bb, sw->default_target))
        return false;
  ...
  // Unreachable blocks are allowed but skipped in dominance checks; still validated for
  // typing/structure below.
  ...
  std::unordered_map<const BasicBlock *, int> rpo_index;
  {
    ...
    while (!stack.empty()) {
      ...
      if (it != succs.end()) {
        for (auto *succ : it->second) {
          if (reachable.count(succ) && !visited.count(succ))
  ...
  auto Intersect = [&](const BasicBlock *b1, const BasicBlock *b2,
  ...
  auto Dominates = [&](const BasicBlock *a, const BasicBlock *b) {
    ...
    while (runner && runner != a) {
      ...
      if (it2 == idom.end())
        return false;
      if (it2->second == runner)
        break; // reached entry without finding a
      runner = it2->second;
    }
    return runner == a;
  };
  ...
  for (auto &bb_ptr : func.blocks) {
    ...
    for (auto &phi : bb->phis) {
      if (phi->HasResult()) {
        if (!defs.insert(phi->name).second)
          return Fail("duplicate SSA name: " + phi->name, msg);
    ...
    for (auto &inst : bb->instructions) {
      if (inst->HasResult()) {
        if (!defs.insert(inst->name).second)
          return Fail("duplicate SSA name: " + inst->name, msg);
  ...
  for (auto &bb_ptr : func.blocks) {
    ...
    for (auto &inst : bb->instructions) {
      if (inst->IsTerminator())
        return Fail("terminator found inside instruction list in block " + bb->name, msg);
      if (seen_terminator)
        return Fail("instruction appears after terminator in block " + bb->name, msg);
    }
    seen_terminator = bb->terminator != nullptr;

    // operand checks for instructions and terminator
    auto check_dom_use = [&](const std::string &op, const BasicBlock *use_block, int use_index,
                             const BasicBlock *incoming_pred) -> bool {
      auto it = def_locs.find(op);
      if (it == def_locs.end())
        return true; // undefined handled elsewhere
      const DefLoc &def = it->second;
      if (def.block == nullptr)
        return true; // params dominate all
      if (!reachable.count(use_block))
        return true; // skip dominance for unreachable uses
      if (!reachable.count(def.block))
        return true; // def unreachable; skip dominance
      if (incoming_pred) {
        ...
        if (def.block == pred)
          return true;
        if (!Dominates(def.block, pred))
          return Fail("phi incoming not dominated by definition: " + op, msg);
        return true;
      }
      if (def.block == use_block) {
        if (def.index >= use_index)
          return Fail("use before def in block " + use_block->name + " for value " + op, msg);
        return true;
      }
      if (!Dominates(def.block, use_block))
        return Fail("use not dominated by definition in block " + use_block->name + " for value " +
                        op,
                    msg);
      return true;
    };
    ...
    auto check_operands = [&](const Instruction &inst, int use_index) -> bool {
      for (const auto &op : inst.operands) {
        if (!IsDefined(op, defs, extra_defs))
          return Fail("use of undefined value: " + op + " in block " + bb->name, msg);
        if (!check_dom_use(op, bb, use_index, nullptr))
          return false;
      }
      return true;
    };
    ...
    for (auto &phi : bb->phis) {
      ...
      for (size_t i = 0; i < phi->incomings.size(); ++i) {
        ...
        if (!inc.first)
          return Fail("phi missing predecessor in block " + bb->name, msg);
        ...
        if (!check_dom_use(inc.second, bb, 0, inc.first))
          return false;
        if (phi->HasResult()) {
    ...
    for (auto &inst : bb->instructions) {
      ...
      if (auto *ld = dynamic_cast<LoadInstruction *>(inst.get())) {
        ...
        if (ptr_ty.subtypes.empty())
          return Fail("load pointer missing pointee type in block " + bb->name, msg);
        // Allow type mismatch when load type or pointee is I64 (generic)
        bool ld_compatible =
            ld->type.SameShape(ptr_ty.subtypes[0]) || ld->type.kind == IRTypeKind::kI64 ||
            ptr_ty.subtypes[0].kind == IRTypeKind::kI64 || ld->type.kind == IRTypeKind::kInvalid;
        if (!ld_compatible)
          return Fail("load type mismatch pointee in block " + bb->name, msg);
        size_t natural = NaturalAlign(ptr_ty.subtypes[0], layout);
        size_t requested = ld->align ? ld->align : natural;
        if (!IsPowerOfTwo(requested))
          return Fail("load alignment not power of two in block " + bb->name, msg);
        if (requested < natural)
          return Fail("load alignment smaller than natural in block " + bb->name, msg);
      }

      if (auto *st = dynamic_cast<StoreInstruction *>(inst.get())) {
        if (st->operands.size() < 2)
          return Fail("store missing operands in block " + bb->name, msg);
        ...
        if (ptr_ty.subtypes.empty())
          return Fail("store pointer missing pointee type in block " + bb->name, msg);
        ...
        if (!st_compatible)
          return Fail("store value type mismatch pointee in block " + bb->name, msg);
        size_t natural = NaturalAlign(ptr_ty.subtypes[0], layout);
        size_t requested = st->align ? st->align : natural;
        if (!IsPowerOfTwo(requested))
          return Fail("store alignment not power of two in block " + bb->name, msg);
        if (requested < natural)
          return Fail("store alignment smaller than natural in block " + bb->name, msg);
      }

      if (auto *cast = dynamic_cast<CastInstruction *>(inst.get())) {
        if (cast->operands.size() < 1)
          return Fail("cast missing operand in block " + bb->name, msg);
        IRType src_ty = LookupType(cast->operands[0], types);
        if (!CheckCast(*cast, src_ty, msg))
          return false;
      }

      if (auto *mc = dynamic_cast<MemcpyInstruction *>(inst.get())) {
        if (mc->operands.size() < 3)
          return Fail("memcpy missing operands in block " + bb->name, msg);
        ...
        if (!(dst_ty.kind == IRTypeKind::kPointer || dst_ty.kind == IRTypeKind::kReference))
          return Fail("memcpy dst not pointer", msg);
        if (!(src_ty.kind == IRTypeKind::kPointer || src_ty.kind == IRTypeKind::kReference))
          return Fail("memcpy src not pointer", msg);
        if (!size_ty.IsInteger())
          return Fail("memcpy size not integer", msg);
        if (!dst_ty.subtypes.empty() && !src_ty.subtypes.empty() &&
            !dst_ty.subtypes[0].SameShape(src_ty.subtypes[0])) {
        ...
        if (!IsPowerOfTwo(requested))
          return Fail("memcpy alignment not power of two in block " + bb->name, msg);
        if (requested < natural)
          return Fail("memcpy alignment smaller than natural in block " + bb->name, msg);
      }

      if (auto *ms = dynamic_cast<MemsetInstruction *>(inst.get())) {
        if (ms->operands.size() < 3)
          return Fail("memset missing operands in block " + bb->name, msg);
        ...
        if (!(dst_ty.kind == IRTypeKind::kPointer || dst_ty.kind == IRTypeKind::kReference))
          return Fail("memset dst not pointer", msg);
        if (!val_ty.IsInteger())
          return Fail("memset value not integer", msg);
        if (!size_ty.IsInteger())
          return Fail("memset size not integer", msg);
        size_t natural = dst_ty.subtypes.empty() ? 1 : NaturalAlign(dst_ty.subtypes[0], layout);
        size_t requested = ms->align ? ms->align : natural;
        if (!IsPowerOfTwo(requested))
          return Fail("memset alignment not power of two in block " + bb->name, msg);
        if (requested < natural)
          return Fail("memset alignment smaller than natural in block " + bb->name, msg);
      }

      if (auto *gep = dynamic_cast<GetElementPtrInstruction *>(inst.get())) {
        if (gep->operands.size() < 1)
          return Fail("gep missing base in block " + bb->name, msg);
        ...
        if (!CheckGEP(*gep, base_ptr, layout, msg))
          return false;
      }
      ...
      if (auto *call = dynamic_cast<CallInstruction *>(inst.get())) {
        ...
        if (fn_ty.kind == IRTypeKind::kInvalid) {
          ...
          if (fn_ty.kind == IRTypeKind::kPointer || fn_ty.kind == IRTypeKind::kReference) {
            if (!fn_ty.subtypes.empty())
              fn_ty = fn_ty.subtypes[0];
          }
          if (fn_ty.kind == IRTypeKind::kInvalid) {
        ...
        if (!skip_call_check) {
          if (fn_ty.kind != IRTypeKind::kFunction)
            return Fail("call callee is not a function", msg);
          const size_t param_count = fn_ty.count;
          if (!call->is_vararg && param_count != call->operands.size())
            return Fail("call arg count mismatch", msg);
          if (call->is_vararg && call->operands.size() < param_count)
            return Fail("call vararg missing fixed args", msg);
          if (!call->type.SameShape(fn_ty.subtypes[0]))
            return Fail("call return type mismatch", msg);
          for (size_t i = 0; i < std::min(call->operands.size(), fn_ty.subtypes.size() - 1); ++i) {
            IRType arg_ty = LookupType(call->operands[i], types);
            if (!arg_ty.SameShape(fn_ty.subtypes[i + 1]))
              return Fail("call arg type mismatch", msg);
          }
        }
      }
      ++inst_use_index;
    ...
    if (bb->terminator) {
      if (!check_operands(*bb->terminator, inst_use_index))
        return false;
      if (auto *ret = dynamic_cast<ReturnStatement *>(bb->terminator.get())) {
        ...
        if (fn_ret.kind == IRTypeKind::kVoid) {
          if (!ret->operands.empty())
            return Fail("return with value in void function in block " + bb->name, msg);
        } else {
          if (ret->operands.size() != 1)
            return Fail("return missing value in non-void function in block " + bb->name, msg);
          ...
          if (!compatible && val_ty.kind == IRTypeKind::kI64) {
            compatible = fn_ret.kind == IRTypeKind::kPointer ||
                         fn_ret.kind == IRTypeKind::kReference || fn_ret.kind == IRTypeKind::kF64 ||
                         fn_ret.IsInteger();
          }
          if (!compatible && fn_ret.kind == IRTypeKind::kI64) {
            compatible = val_ty.kind == IRTypeKind::kPointer ||
                         val_ty.kind == IRTypeKind::kReference || val_ty.kind == IRTypeKind::kF64 ||
                         val_ty.IsInteger();
          }
          ...
          if (!compatible)
            return Fail("return value type mismatch in block " + bb->name, msg);
        }
      }
      ...
      if (auto *cbr = dynamic_cast<CondBranchStatement *>(bb->terminator.get())) {
        if (cbr->operands.size() < 1)
          return Fail("condbr missing condition in block " + bb->name, msg);
      ...
      if (auto *sw = dynamic_cast<SwitchStatement *>(bb->terminator.get())) {
        if (sw->operands.size() < 1)
          return Fail("switch missing operand in block " + bb->name, msg);
        IRType scrut_ty = LookupType(sw->operands[0], types);
        if (!scrut_ty.IsInteger())
          return Fail("switch operand must be integer in block " + bb->name, msg);
        if (!sw->default_target)
          return Fail("switch missing default target in block " + bb->name, msg);
        ...
        for (auto &c : sw->cases) {
          if (!c.target)
            return Fail("switch case missing target in block " + bb->name, msg);
          if (!seen_cases.insert(c.value).second)
            return Fail("duplicate switch case value in block " + bb->name, msg);
        }
      }
    }
  }
    ...
    for (auto &fn : ctx.Functions()) {
      ...
      if (fn->ret_type.kind == IRTypeKind::kInvalid) {
        return Fail("strict: function '" + fn->name +
                        ...
                        "type annotation or LINK with MAP_TYPE",
                    msg);
      }
      ...
      if (fn->ret_type.kind == IRTypeKind::kI64 && fn->ret_type.is_placeholder) {
        return Fail(
            "strict: function '" + fn->name + "' has unresolved placeholder return type I64", msg);
      }
      // Reject Invalid parameter types
      for (std::size_t pi = 0; pi < fn->param_types.size(); ++pi) {
        if (fn->param_types[pi].kind == IRTypeKind::kInvalid) {
          return Fail("strict: function '" + fn->name + "' parameter " + std::to_string(pi) +
                          " has unresolved Invalid type — add an explicit type annotation",
                      msg);
        }
      }
      ...
      for (auto &bb : fn->blocks) {
        for (auto &inst : bb->instructions) {
          ...
          for (const auto &op : inst->operands) {
            if (op == "undef") {
              return Fail("strict: placeholder 'undef' operand in function '" + fn->name +
                              "' block '" + bb->name + "'",
                          msg);
            }
          }
