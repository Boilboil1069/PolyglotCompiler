...
bool AliasInfo::IsLocalStack(const std::string &name) const {
  return ClassOf(name) == AliasClass::kLocalStack;
}

bool AliasInfo::IsAddrTaken(const std::string &name) const {
  return addr_taken.count(name) > 0;
}
...
void ComputeOrders(const ControlFlowGraph &cfg, std::vector<BasicBlock *> &post,
...
std::vector<LoopInfo> ComputeLoops(const ControlFlowGraph &cfg, const DominatorTree &dom) {
  ...
  for (auto *tail : cfg.blocks) {
    for (auto *head : tail->successors) {
      ...
      if (std::find(loop.blocks.begin(), loop.blocks.end(), head) == loop.blocks.end())
...
AliasInfo ComputeAlias(Function &func) {
  ...
  auto mark_addr_taken = [&](const std::string &name) {
    if (ai.ClassOf(name) == AliasClass::kLocalStack)
      ai.addr_taken.insert(name);
  };
...
void AnalysisCache::InvalidateAll() {
  cfg_dirty_ = dom_dirty_ = df_dirty_ = order_dirty_ = loops_dirty_ = liveness_dirty_ =
      alias_dirty_ = true;
}
