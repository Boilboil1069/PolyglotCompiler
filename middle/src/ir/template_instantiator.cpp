...
bool TemplateArgument::operator==(const TemplateArgument &other) const {
  ...
  if (is_type != other.is_type)
    return false;
  if (is_pack != other.is_pack)
    return false;

  // For pack arguments, compare pack contents
  if (is_pack) {
    if (pack_args.size() != other.pack_args.size())
      return false;
    for (size_t i = 0; i < pack_args.size(); ++i) {
      if (pack_args[i] != other.pack_args[i])
        return false;
    }
    return true;
  }
...
void TemplateInstantiator::RegisterClassTemplate(const std::string &name,
...
void TemplateInstantiator::RegisterFunctionTemplate(const std::string &name,
                                                    const std::vector<TemplateParameter> &params,
                                                    void *template_ast,
...
void TemplateInstantiator::RegisterTemplateAlias(const std::string &name,
                                                 const std::vector<TemplateParameter> &params,
...
void TemplateInstantiator::RegisterSpecialization(const std::string &template_name,
                                                  const std::vector<TemplateArgument> &pattern,
                                                  const std::vector<TemplateParameter> &params,
                                                  void *specialized_ast, bool is_partial) {
...
std::string TemplateInstantiator::InstantiateClass(const std::string &template_name,
...
std::string TemplateInstantiator::InstantiateFunction(
    const std::string &template_name, const std::vector<TemplateArgument> &arguments) {
  // Create cache key
  TemplateInstantiationKey key{template_name, arguments};
...
DeductionResult TemplateInstantiator::DeduceTemplateArguments(
    const std::string &template_name, const std::vector<IRType> &argument_types) {
  // Find the function template
  auto tmpl_it = function_templates_.find(template_name);
  if (tmpl_it == function_templates_.end()) {
...
DeductionResult TemplateInstantiator::DeduceFromType(const IRType &pattern, const IRType &argument,
                                                     const std::vector<TemplateParameter> &params) {
...
SubstitutionResult TemplateInstantiator::SubstituteType(
    const IRType &type, const std::unordered_map<std::string, TemplateArgument> &substitutions) {
...
SubstitutionResult TemplateInstantiator::SubstituteAst(
    void *ast, const std::unordered_map<std::string, TemplateArgument> &substitutions,
  ...
  auto non_owning = std::shared_ptr<GenericAstNode>(
      static_cast<GenericAstNode *>(ast), [](GenericAstNode *) { /* intentional no-op deleter */ });
  ...
  if (!result_node) {
    return SubstitutionResult::Failure(
        "AST visitor returned null after substitution for node type '" + non_owning->NodeType() +
        "'");
  }
...
bool TemplateInstantiator::MatchesPattern(
    const std::vector<TemplateArgument> &arguments, const std::vector<TemplateArgument> &pattern,
    std::unordered_map<std::string, TemplateArgument> *bindings) {
...
std::string TemplateInstantiator::GenerateInstanceName(
    const std::string &template_name, const std::vector<TemplateArgument> &arguments) const {
...
int TemplateInstantiator::CalculateSpecificity(const std::vector<TemplateArgument> &pattern) const {
  int specificity = 0;
  ...
  for (const auto &arg : pattern) {
    if (arg.is_pack) {
      ...
      if (arg.type.kind != IRTypeKind::kInvalid && arg.type.kind != IRTypeKind::kStruct) {
...
bool TemplateInstantiator::TypeMatchesPattern(
    const IRType &pattern, const IRType &argument, const std::set<std::string> &param_names,
  ...
  case IRTypeKind::kReference:
    ...
    if (!pattern.subtypes.empty()) {
      return TypeMatchesPattern(pattern.subtypes[0], argument.subtypes[0], param_names, bindings);
    }
    return true;
  ...
  case IRTypeKind::kFunction:
    ...
    for (size_t i = 0; i < pattern.subtypes.size(); ++i) {
      if (!TypeMatchesPattern(pattern.subtypes[i], argument.subtypes[i], param_names, bindings)) {
        return false;
      }
    }
    return true;
...
bool TemplateInstantiator::IsTemplateParameterType(const IRType &type,
...
bool TemplateInstantiator::ValidateArguments(const std::vector<TemplateArgument> &arguments,
                                             const std::vector<TemplateParameter> &params,
  ...
  if (arguments.size() < required_count) {
    if (error_msg) {
      *error_msg = "Too few template arguments: expected at least " +
                   std::to_string(required_count) + ", got " + std::to_string(arguments.size());
    }
    return false;
  }
  ...
  if (!has_pack && arguments.size() > params.size()) {
    if (error_msg) {
      *error_msg = "Too many template arguments: expected at most " +
                   std::to_string(params.size()) + ", got " + std::to_string(arguments.size());
    }
    return false;
  }
