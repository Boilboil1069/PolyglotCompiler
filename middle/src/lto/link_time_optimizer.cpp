  ...
  for (const auto &fn : functions) {
    ...
    for (const auto &block : fn.blocks) {
      ...
      for (const auto &inst : block->instructions) {
        ...
        out << inst_name << " " << static_cast<int>(inst->type.kind) << " "
            << inst->operands.size();
...
bool LTOModule::LoadBitcode(const std::string &filename) {
  ...
  std::string header;
  if (!(in >> header >> module_name))
    return false;

  size_t fn_count = 0, gv_count = 0;
  if (!(in >> fn_count >> gv_count))
    return false;
  std::string line;
          ...
          for (size_t inst_idx = 0; inst_idx < inst_count; ++inst_idx) {
            ...
            if (in >> inst_name >> type_kind >> op_count) {
              ...
              for (size_t op_i = 0; op_i < op_count; ++op_i) {
                std::string operand;
                if (in >> operand) {
                  instruction->operands.push_back((operand == "_") ? "" : operand);
                }
              }
            }
            std::getline(in, line); // consume endline
...
std::vector<CrossModuleInliner::InlineCandidate> CrossModuleInliner::FindInlineCandidates() const {
  ...
  for (const auto &[fn_name, node] : cg.nodes) {
    for (const auto &callee : node.callees) {
      ...
      cand.inline_cost =
          CalculateInlineCost(*caller_fn, *callee_fn, cand.call_site_count, cand.is_recursive);
...
size_t CrossModuleInliner::CalculateInlineCost(const ir::Function &caller,
                                               const ir::Function &callee, size_t call_site_count,
...
bool CrossModuleInliner::ShouldInline(const ir::Function &caller,
                                      const ir::Function &callee) const {
  (void)caller;
...
void CrossModuleInliner::CloneInstructions(ir::BasicBlock &target, const ir::BasicBlock &source,
...
LatticeValue InterproceduralConstantPropagation::EvaluateBinaryOp(ir::BinaryInstruction::Op op,
  ...
  if (!lhs.is_float && !rhs.is_float) {
    ...
    case ir::BinaryInstruction::Op::kLShr:
      return LatticeValue::Constant(static_cast<int64_t>(static_cast<uint64_t>(l) >> r));
    ...
    case ir::BinaryInstruction::Op::kCmpUlt:
      return LatticeValue::Constant(static_cast<uint64_t>(l) < static_cast<uint64_t>(r) ? 1 : 0);
...
bool CrossModuleGVN::IsPure(const ir::Instruction *inst) const {
  ...
  if (dynamic_cast<const ir::CallInstruction *>(inst))
    return false;

  // Stores are not pure
  if (dynamic_cast<const ir::StoreInstruction *>(inst))
    return false;

  // Allocas are not pure (each one creates a new address)
  if (dynamic_cast<const ir::AllocaInstruction *>(inst))
    return false;
  ...
  if (dynamic_cast<const ir::LoadInstruction *>(inst))
    return false;

  // Binary operations, casts, GEPs are pure
  if (dynamic_cast<const ir::BinaryInstruction *>(inst))
    return true;
  if (dynamic_cast<const ir::CastInstruction *>(inst))
    return true;
  if (dynamic_cast<const ir::GetElementPtrInstruction *>(inst))
    return true;
...
size_t CountFunctions(const LTOContext &ctx) {
  size_t total = 0;
  for (const auto &mod : ctx.GetModules())
  ...
  for (auto &summary : summaries_) {
    for (auto &func : summary.functions) {
      ...
      for (const auto &ref : summary.referenced_symbols) {
        if (symbol_to_module.count(ref) && symbol_to_module[ref] != summary.module_id) {
  ...
  for (const auto &summary : summaries_) {
    for (const auto &func : summary.functions) {
      for (const auto &callee : func.callees) {
        ...
        if (function_to_module.count(callee) && function_to_module[callee] != summary.module_id) {
...
bool GenerateObjectFromBitcode(const std::string &bitcode_file, const std::string &output_object) {
  ...
  LTOModule mod;
  if (!mod.LoadBitcode(bitcode_file))
    return false;
...
bool LTOWorkflow::CompilePhase(const std::vector<std::string> &sources,
  ...
  for (const auto &src : sources) {
    std::filesystem::path out =
        std::filesystem::path(output_dir) / (std::filesystem::path(src).stem().string() + ".bc");
    ok &= CompileToBitcode(src, out.string());
  }
  return ok;
}

bool LTOWorkflow::LinkPhase(const std::vector<std::string> &bitcodes,
...
bool LTOWorkflow::FullLTO(const std::vector<std::string> &sources, const std::string &output_exe,
  ...
  for (const auto &src : sources) {
    ...
    if (!CompileToBitcode(src, out.string()))
      return false;
