...
public:
  ...
  // Get all modules
  const std::vector<std::unique_ptr<LTOModule>> &GetModules() const { return modules_; }

  // Mutable access for transformation passes
  std::vector<std::unique_ptr<LTOModule>> &MutableModules() { return modules_; }
...
public:
  explicit CrossModuleInliner(LTOContext &context,
                              size_t threshold = InlineCostModel::kDefaultInlineThreshold) :
...
private:
  ...
  size_t CalculateInlineCost(const ir::Function &caller, const ir::Function &callee,
                             size_t call_site_count, bool is_recursive) const;

  // Calculate benefit from inlining (optimization opportunities)
  size_t CalculateInlineBenefit(const ir::Function &caller, const ir::Function &callee) const;
  ...
  void CloneInstructions(ir::BasicBlock &target, const ir::BasicBlock &source,
                         const std::string &prefix, std::map<std::string, std::string> &name_map);
};
...
private:
  ...
  void MarkReachable(const std::string &symbol, std::set<std::string> &reachable,
...
public:
  explicit InterproceduralConstantPropagation(LTOContext &context) : context_(context) {}
...
private:
  ...
  LatticeValue EvaluateBinaryOp(ir::BinaryInstruction::Op op, const LatticeValue &lhs,
...
private:
  ...
  std::optional<std::string> GetUniqueImplementation(const std::string &class_name,
...
public:
  ...
  static bool CompilePhase(const std::vector<std::string> &sources, const std::string &output_dir);

  // Step 2: link-time optimization
  static bool LinkPhase(const std::vector<std::string> &bitcodes, const std::string &output_exe);

  // Full pipeline
  static bool FullLTO(const std::vector<std::string> &sources, const std::string &output_exe,
                      int opt_level = 2);

  // Thin LTO pipeline
  static bool ThinLTO(const std::vector<std::string> &sources, const std::string &output_exe,
                      int opt_level = 2, size_t num_threads = 4);
};
