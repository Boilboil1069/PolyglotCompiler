...
struct BranchProfile {
  ...
  uint64_t TotalCount() const { return taken_count + not_taken_count; }
};
...
struct CallSiteProfile {
  ...
  bool has_single_target() const { return target_distribution.size() <= 1; }
...
public:
  ...
  bool IsHotFunction(uint64_t threshold = 1000) const { return invocation_count >= threshold; }
};
...
public:
  ...
  void RecordCall(const std::string &caller, size_t call_site_id, const std::string &callee);
...
private:
  ...
  InliningDecision MakeInliningDecision(const std::string &caller, const CallSiteProfile &call_site,
                                        const FunctionProfile &caller_profile) const;

  BranchPredictionHint MakeBranchPrediction(const std::string &function,
...
public:
  ...
  static bool GenerateInstrumentedBinary(const std::string &source_file,
                                         const std::string &output_file);

  // Step 2: Run and collect the profile (user executes)

  // Step 3: Compile using the collected profile
  static bool CompileWithProfile(const std::string &source_file, const std::string &profile_file,
                                 const std::string &output_file);

  // Utility: merge multiple profile files
  static bool MergeProfiles(const std::vector<std::string> &input_files,
                            const std::string &output_file);
};
