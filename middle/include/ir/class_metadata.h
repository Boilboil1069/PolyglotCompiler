...
struct ClassLayout {
  ...
  std::unordered_map<std::string, std::shared_ptr<VTable>>
      base_vtables; // Base class name -> vtable
  ...
  bool IsVirtualBase(const std::string &base_name) const {
    return std::find(virtual_bases.begin(), virtual_bases.end(), base_name) != virtual_bases.end();
  }
};
...
struct TypeInfo {
  ...
  std::string GetTypeInfoName() const { return "__type_info_" + class_name; }
};
