...
struct IRType {
  ...
  static IRType I1() {
    IRType t{IRTypeKind::kI1, "i1"};
    t.is_signed = false;
    return t;
  }
  static IRType I8(bool is_signed = true) {
    IRType t{IRTypeKind::kI8, is_signed ? "i8" : "u8"};
    t.is_signed = is_signed;
    return t;
  }
  static IRType I16(bool is_signed = true) {
    IRType t{IRTypeKind::kI16, is_signed ? "i16" : "u16"};
    t.is_signed = is_signed;
    return t;
  }
  static IRType I32(bool is_signed = true) {
    IRType t{IRTypeKind::kI32, is_signed ? "i32" : "u32"};
    t.is_signed = is_signed;
    return t;
  }
  static IRType I64(bool is_signed = true) {
    IRType t{IRTypeKind::kI64, is_signed ? "i64" : "u64"};
    t.is_signed = is_signed;
    return t;
  }
  ...
  IRType WithSigned(bool sign) const {
    ...
    t.is_signed = sign;
    if (t.IsInteger()) {
      if (t.kind == IRTypeKind::kI8)
        t.name = sign ? "i8" : "u8";
      if (t.kind == IRTypeKind::kI16)
        t.name = sign ? "i16" : "u16";
      if (t.kind == IRTypeKind::kI32)
        t.name = sign ? "i32" : "u32";
      if (t.kind == IRTypeKind::kI64)
        t.name = sign ? "i64" : "u64";
    }
    return t;
  }
  ...
  bool IsInteger() const {
    return kind == IRTypeKind::kI1 || kind == IRTypeKind::kI8 || kind == IRTypeKind::kI16 ||
           kind == IRTypeKind::kI32 || kind == IRTypeKind::kI64;
  }
  ...
  bool SameShape(const IRType &other) const {
    if (kind != other.kind || count != other.count)
      return false;
    if (IsInteger() && is_signed != other.is_signed)
      return false;
    if (kind == IRTypeKind::kStruct && name != other.name)
      return false;
    if (subtypes.size() != other.subtypes.size())
      return false;
    for (size_t i = 0; i < subtypes.size(); ++i) {
      if (!subtypes[i].SameShape(other.subtypes[i]))
        return false;
    }
    return true;
  }
  ...
  bool CanLosslesslyConvertTo(const IRType &dst) const {
    ...
    if (IsInteger() && dst.IsInteger()) {
      ...
      if (is_signed != dst.is_signed && rank_src == rank_dst)
        return false;
    ...
    if ((kind == IRTypeKind::kPointer || kind == IRTypeKind::kReference) &&
      ...
      if (subtypes.empty() || dst.subtypes.empty())
        return false;
  ...
  bool CanBitcastTo(const IRType &dst) const {
    ...
    if (kind == IRTypeKind::kStruct && dst.kind == IRTypeKind::kStruct &&
        subtypes.size() == dst.subtypes.size()) {
  ...
  bool operator==(const IRType &other) const {
    if (kind != other.kind || name != other.name || count != other.count)
      return false;
    if (IsInteger() && is_signed != other.is_signed)
      return false;
    if (subtypes.size() != other.subtypes.size())
