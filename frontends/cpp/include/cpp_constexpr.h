...
public:
  ...
  enum class Type { kInt, kFloat, kBool, kPointer, kNull, kUndefined };

  ConstexprValue() : type_(Type::kUndefined) {}

  explicit ConstexprValue(long long val) : type_(Type::kInt), int_value_(val) {}

  explicit ConstexprValue(double val) : type_(Type::kFloat), float_value_(val) {}

  explicit ConstexprValue(bool val) : type_(Type::kBool), bool_value_(val) {}
...
public:
  ...
  ConstexprValue EvaluateCall(VarDecl *func, const std::vector<ConstexprValue> &args);

  // Evaluate a constexpr function call (FunctionDecl-based)
  ConstexprValue EvaluateFunctionCall(FunctionDecl *func, const std::vector<ConstexprValue> &args);
...
private:
  ...
  ConstexprValue ApplyBinaryOp(const std::string &op, const ConstexprValue &lhs,
