  ...
  while (current_.kind != frontends::TokenKind::kEndOfFile) {
    if (current_.kind == frontends::TokenKind::kKeyword) {
      if (current_.lexeme == "import" || current_.lexeme == "using" ||
          ...
          current_.lexeme == "switch" || current_.lexeme == "try" || current_.lexeme == "throw" ||
          current_.lexeme == "int" || current_.lexeme == "void" || current_.lexeme == "float" ||
          current_.lexeme == "double" || current_.lexeme == "char" || current_.lexeme == "bool" ||
          current_.lexeme == "auto" || current_.lexeme == "class" || current_.lexeme == "struct" ||
      ...
      if (IsSymbol("<")) {
        ...
        if (la.kind == frontends::TokenKind::kSymbol && la.lexeme == ">") {
          ...
          if (la2.kind == frontends::TokenKind::kSymbol &&
              (la2.lexeme == ">" || la2.lexeme == "," || la2.lexeme == "::" || la2.lexeme == "<" ||
...
std::shared_ptr<Expression> CppParser::ParseExpression() {
  return ParseComma();
}
...
std::shared_ptr<Statement> CppParser::ParseFriend() {
  ...
  std::string name = current_.lexeme;
  Consume();
  auto fn = ParseFunctionWithSignature(ret, name, false, false, false, false, false, "", "", true);
  if (fn)
    fn->is_friend = true;
  return fn;
}
...
std::shared_ptr<Statement> CppParser::ParseSwitch() {
  ...
  while (!IsSymbol("}") && current_.kind != frontends::TokenKind::kEndOfFile) {
    ...
    while (!(IsSymbol("}") || (current_.kind == frontends::TokenKind::kKeyword &&
      ...
      if (current_.loc.line == before_line && current_.loc.column == before_col) {
        Consume();
      }
    }
...
std::shared_ptr<Statement> CppParser::ParseRecord(const std::string &kind) {
  ...
  if (MatchSymbol(":")) {
    do {
      BaseSpecifier base;
      if (current_.kind == frontends::TokenKind::kKeyword && current_.lexeme == "virtual") {
  ...
  while (!IsSymbol("}") && current_.kind != frontends::TokenKind::kEndOfFile) {
    ...
    if (IsSymbol("~")) {
      ...
      if (current_.kind == frontends::TokenKind::kIdentifier && current_.lexeme == rec->name) {
        Consume();
        auto fn = ParseFunctionWithSignature(nullptr, "~" + rec->name, is_constexpr, is_consteval,
                                             is_inline, is_static, false, "", current_access, true);
        if (fn) {
    ...
    if (current_.kind == frontends::TokenKind::kIdentifier && current_.lexeme == rec->name &&
      ...
      Consume();
      if (IsSymbol("(")) {
        auto fn = ParseFunctionWithSignature(nullptr, saved.lexeme, is_constexpr, is_consteval,
                                             is_inline, is_static, false, "", current_access, true);
        if (fn) {
    ...
    if (IsSymbol("(")) {
      auto fn = ParseFunctionWithSignature(member_type, name, is_constexpr, is_consteval, is_inline,
                                           is_static, is_operator, op_symbol, current_access, true);
      if (fn) {
        fn->is_virtual = is_virtual;
        fn->attributes = attrs;
        rec->methods.push_back(fn);
...
std::shared_ptr<FunctionDecl> CppParser::ParseFunctionWithSignature(
    std::shared_ptr<TypeNode> ret_type, const std::string &name, bool is_constexpr,
    bool is_consteval, bool is_inline, bool is_static, bool is_operator,
    const std::string &op_symbol, const std::string &access, bool inside_record) {
...
std::shared_ptr<Statement> CppParser::ParseFunction() {
  ...
  std::string name = current_.lexeme;
  Consume();
  return ParseFunctionWithSignature(ret, name, false, false, false, false, false, "", "", false);
}
...
std::shared_ptr<Module> CppParser::TakeModule() {
  return module_;
}
