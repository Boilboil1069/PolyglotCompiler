...
ConstexprValue ConstexprEvaluator::ApplyBinaryOp(const std::string &op, const ConstexprValue &lhs,
                                                 const ConstexprValue &rhs) {
  // Integer operations
  if (lhs.GetType() == ConstexprValue::Type::kInt && rhs.GetType() == ConstexprValue::Type::kInt) {
...
ConstexprValue ConstexprEvaluator::ApplyUnaryOp(const std::string &op, const ConstexprValue &val) {
...
bool ConstexprEvaluator::EvaluateCompoundStmt(CompoundStatement *stmt,
                                              ConstexprValue &return_value) {
  if (!stmt)
    return false;
...
bool ConstexprEvaluator::EvaluateReturnStmt(ReturnStatement *stmt, ConstexprValue &return_value) {
  if (!stmt)
    return false;
...
bool ConstexprChecker::IsConstexprExpr(Expression *expr) const {
  ...
  if (auto *id = dynamic_cast<Identifier *>(expr)) {
    ...
    if (id->name == "true" || id->name == "false" || id->name == "nullptr" || id->name == "NULL") {
      return true;
    }
...
bool ConstexprChecker::CheckConditionalExpr(ConditionalExpression *expr) const {
  ...
  return IsConstexprExpr(expr->condition.get()) && IsConstexprExpr(expr->then_expr.get()) &&
