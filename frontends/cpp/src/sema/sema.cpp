...
private:
  ...
  void AnalyzeDecl(const std::shared_ptr<Statement> &decl) {
    ...
    if (auto tr = std::dynamic_pointer_cast<TryStatement>(decl)) {
      EnterScope(ScopeKind::kBlock);
      for (auto &s : tr->try_body)
        AnalyzeDecl(s);
      ExitScope();
      for (auto &c : tr->catches) {
        EnterScope(ScopeKind::kBlock);
        if (!c.name.empty()) {
          Symbol ex{c.name, MapType(c.exception_type),
                    c.exception_type ? c.exception_type->loc : c.body.front()->loc,
                    SymbolKind::kVariable, "cpp"};
          Syms().Declare(ex);
        }
        for (auto &s : c.body)
          AnalyzeDecl(s);
  ...
  void AnalyzeStmt(const std::shared_ptr<Statement> &stmt) {
    ...
    if (std::dynamic_pointer_cast<FunctionDecl>(stmt) ||
        std::dynamic_pointer_cast<RecordDecl>(stmt) || std::dynamic_pointer_cast<EnumDecl>(stmt) ||
