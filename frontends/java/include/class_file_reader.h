...
class JarReader {
public:
  static std::shared_ptr<JarReader> Open(const std::string &path, frontends::Diagnostics &diags);
...
public:
  explicit ClasspathLoader(std::vector<std::string> classpath, frontends::Diagnostics &diags);
  ...
  const std::vector<const ClassFile *> &ListPackage(const std::string &package_name);
...
private:
  ...
  std::optional<std::vector<std::uint8_t>> LocateBytes(const std::string &qualified_name);
