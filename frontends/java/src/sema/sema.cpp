...
class Analyzer {
public:
  Analyzer(const Module &mod, frontends::SemaContext &ctx, ClasspathLoader *loader) :
...
private:
  ...
  void DeclareImportedClass(const ClassFile &cf, const core::SourceLoc &loc, bool import_static,
    ...
    for (const auto &i : cf.interfaces)
      bases.push_back(i);
    if (!bases.empty())
      Syms().RegisterTypeBases(simple, bases);
    ...
    for (const auto &f : cf.fields) {
      if ((f.access & kAccStatic) == 0)
        continue;
      if ((f.access & kAccPublic) == 0)
        continue;
    ...
    for (const auto &m : cf.methods) {
      if ((m.access & kAccStatic) == 0)
        continue;
      if ((m.access & kAccPublic) == 0)
        continue;
  ...
  void AnalyzeImport(const ImportDecl &imp) {
    ...
    Symbol module_sym{imp.path, Type::Module(imp.path, "java"), imp.loc, SymbolKind::kModule,
                      "java"};
    ...
    bool is_wildcard =
        imp.path.size() >= kWildcard.size() &&
        imp.path.compare(imp.path.size() - kWildcard.size(), kWildcard.size(), kWildcard) == 0;
    if (is_wildcard) {
  ...
  void AnalyzeStmt(const std::shared_ptr<Statement> &stmt) {
    ...
    if (std::dynamic_pointer_cast<ClassDecl>(stmt) ||
        ...
        std::dynamic_pointer_cast<EnumDecl>(stmt) || std::dynamic_pointer_cast<RecordDecl>(stmt) ||
    ...
    if (auto try_stmt = std::dynamic_pointer_cast<TryStatement>(stmt)) {
      ...
      for (auto &c : try_stmt->catches) {
        EnterScope(ScopeKind::kBlock);
        if (!c.var_name.empty()) {
          Type et = c.exception_types.empty() ? Type::Any() : MapType(c.exception_types[0]);
    ...
    if (auto assert_stmt = std::dynamic_pointer_cast<AssertStatement>(stmt)) {
      ...
      if (assert_stmt->message)
        AnalyzeExpr(assert_stmt->message);
      return;
    }
  ...
  Type AnalyzeExpr(const std::shared_ptr<Expression> &expr) {
    ...
    if (auto arr = std::dynamic_pointer_cast<ArrayAccessExpression>(expr)) {
      ...
      if (!arrType.type_args.empty())
        return arrType.type_args[0];
      return Type::Any();
    }
