...
std::vector<frontends::Token> PloyLanguageFrontend::Tokenize(const std::string &source,
...
bool PloyLanguageFrontend::Analyze(const std::string &source, const std::string &filename,
...
frontends::FrontendResult PloyLanguageFrontend::Lower(
  ...
  PloyFrontendResult extended =
      LowerWithDescriptors(source, filename, ir_ctx, diagnostics, options);
...
PloyFrontendResult PloyLanguageFrontend::LowerWithDescriptors(
  ...
  sema_opts.enable_package_discovery = false;
  PloySema sema(diagnostics, sema_opts);
  if (!sema.Analyze(module) && !options.force)
    return result;
