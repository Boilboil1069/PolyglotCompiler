...
public:
  PloyLowering(ir::IRContext &ir_context, frontends::Diagnostics &diagnostics,
               const PloySema &sema) :
      ir_ctx_(ir_context), diagnostics_(diagnostics), sema_(sema), builder_(ir_context) {}
  ...
  const std::vector<CrossLangCallDescriptor> &CallDescriptors() const { return call_descriptors_; }
