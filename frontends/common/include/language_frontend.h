...
public:
  ...
  virtual bool Analyze(const std::string &source, const std::string &filename,
                       Diagnostics &diagnostics, const FrontendOptions &options) const = 0;

  // Run the full frontend pipeline (lex + parse + sema + lower) and
  // populate `ir_ctx` with the lowered IR.  Returns a FrontendResult
  // describing the outcome.
  virtual FrontendResult Lower(const std::string &source, const std::string &filename,
                               ir::IRContext &ir_ctx, Diagnostics &diagnostics,
