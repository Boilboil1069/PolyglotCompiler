...
RbImportResolver::RbImportResolver(std::string project_dir, std::vector<std::string> gem_paths,
                                   frontends::Diagnostics &diagnostics) :
    project_dir_(std::move(project_dir)), gem_paths_(std::move(gem_paths)),
    diagnostics_(diagnostics) {
#if defined(_MSC_VER)
#pragma warning(push)
#pragma warning(disable : 4996)
#endif
  if (const char *rl = std::getenv("RUBYLIB"))
    SplitPathList(rl, rubylib_paths_);
#if defined(_MSC_VER)
#pragma warning(pop)
#endif
}
...
void RbImportResolver::EnsureDefaultLoadPaths() {
  ...
  while (std::fgets(buf, sizeof(buf), pipe)) {
    ...
    while (!line.empty() && (line.back() == '\n' || line.back() == '\r' || line.back() == ' '))
      line.pop_back();
    if (!line.empty() && line != ".")
      default_load_paths_.push_back(line);
  }
#ifdef _WIN32
  _pclose(pipe);
#else
...
std::string RbImportResolver::ResolveBare(const std::string &specifier,
  ...
  auto try_in = [&](const fs::path &root) -> std::string {
    ...
    if (auto p = ProbeRb((root / specifier).string()); !p.empty())
      return p;
  ...
  if (!project_dir_.empty()) {
    if (auto p = try_in(fs::path(project_dir_) / "lib"); !p.empty())
      return p;
    if (auto p = try_in(project_dir_); !p.empty())
      return p;
  }
...
void Harvest(const std::vector<std::shared_ptr<Statement>> &body, const std::string &prefix,
  ...
  for (const auto &stmt : body) {
    if (auto md = std::dynamic_pointer_cast<MethodDecl>(stmt)) {
      ...
      s.name = md->name;
      s.qualified_name =
          prefix.empty() ? md->name : (prefix + (md->is_self ? "." : "#") + md->name);
...
const RbFile *RbImportResolver::Resolve(const std::string &specifier,
                                        const std::string &importer_dir, bool relative) {
  ...
  if (resolved.empty()) {
    diagnostics_.Report(core::SourceLoc{}, "ruby require not resolved: '" + specifier +
                                               "' (relative=" + (relative ? "true" : "false") +
                                               ", gem_paths=" + std::to_string(gem_paths_.size()) +
                                               ")");
    return nullptr;
  }
