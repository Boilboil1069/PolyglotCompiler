...
public:
  RbImportResolver(std::string project_dir, std::vector<std::string> gem_paths,
   ...
   */
  const RbFile *Resolve(const std::string &specifier, const std::string &importer_dir,
                        bool relative);
...
private:
  ...
  std::string ResolveBare(const std::string &specifier, std::string &out_gem_name) const;
  std::unique_ptr<RbFile> LoadFile(const std::string &requested, const std::string &resolved_path,
