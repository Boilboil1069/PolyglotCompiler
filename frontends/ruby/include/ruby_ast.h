...
struct HashLit : Expression {
  struct Pair {
    std::shared_ptr<Expression> key, value;
  };
...
struct AssignExpr : Expression {
  std::string op;
  std::shared_ptr<Expression> target, value;
};
struct TernaryExpr : Expression {
  std::shared_ptr<Expression> cond, then_e, else_e;
};
...
struct BeginStmt : Statement {
  std::shared_ptr<Statement> body;
  struct Rescue {
    std::vector<std::shared_ptr<TypeNode>> classes;
    std::string var;
    std::shared_ptr<Statement> body;
  };
