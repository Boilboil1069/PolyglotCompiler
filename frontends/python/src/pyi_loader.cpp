...
public:
  ...
  std::vector<Token> Tokenize() {
    ...
    while (pos_ < src_.size()) {
      ...
      if ((c == 'r' || c == 'R' || c == 'b' || c == 'B' || c == 'u' || c == 'U' || c == 'f' ||
           c == 'F') &&
...
private:
  void Push(TokKind k, std::string t) { tokens_.push_back({k, std::move(t), line_, column_}); }
...
class StubParser {
public:
  StubParser(std::vector<Token> toks, const std::string &module_name, PyiLoader &owner,
             frontends::Diagnostics &diags) :
      toks_(std::move(toks)), mod_name_(module_name), owner_(owner), diags_(diags) {}
...
private:
  ...
  bool MatchName(const std::string &name) {
    if (Peek().kind == TokKind::kName && Peek().text == name) {
      ++pos_;
      return true;
    }
    return false;
  ...
  void ParseStatement(PyiModule &m) {
    ...
    while (IsOp("@")) {
      ...
      while (Peek().kind != TokKind::kNewline && Peek().kind != TokKind::kEnd)
        ++pos_;
    ...
    // Compound statements
    if (IsName("def") ||
        (IsName("async") && Peek(1).kind == TokKind::kName && Peek(1).text == "def")) {
      ParseDef(m);
      return;
    }
    if (IsName("class")) {
  ...
  void SkipExpression() {
    ...
    while (Peek().kind != TokKind::kEnd) {
      const Token &t = Peek();
      if (t.kind == TokKind::kNewline && depth == 0)
        break;
  ...
  void ParseDef(PyiModule &m) {
    ...
    for (auto &p : params)
      fn.type_args.push_back(std::move(p));
  ...
  void SkipDefault() {
    ...
    while (Peek().kind != TokKind::kEnd) {
      ...
      if (depth == 0 && IsOp(","))
        return;
      if (Peek().kind == TokKind::kNewline && depth == 0)
        return;
      ++pos_;
    }
  }
  ...
  core::Type ParseTypeAtom() {
    ...
    if (Peek().kind != TokKind::kName) {
      ...
      if (Peek().kind != TokKind::kNewline && Peek().kind != TokKind::kEnd)
        ++pos_;
      return core::Type::Any();
    }
  ...
  static core::Type MapNamedType(const std::string &qname) {
    ...
    if (n == "int" || n == "long" || n == "Int")
      return core::Type::Int();
    if (n == "float" || n == "double")
      return core::Type::Float();
    if (n == "str" || n == "unicode" || n == "Str")
      return core::Type::String();
    if (n == "bool")
      return core::Type::Bool();
    if (n == "None" || n == "NoneType")
      return core::Type::Void();
    if (n == "bytes" || n == "bytearray" || n == "memoryview" || n == "complex" || n == "object" ||
        n == "Any" || n == "AnyStr" || n == "Never" || n == "NoReturn") {
      return core::Type::Any();
    }
    if (n == "list" || n == "List")
      return MakeGeneric("list", {core::Type::Any()});
    if (n == "dict" || n == "Dict")
      return MakeGeneric("dict", {core::Type::Any(), core::Type::Any()});
    if (n == "set" || n == "Set" || n == "frozenset" || n == "FrozenSet")
      return MakeGeneric("set", {core::Type::Any()});
    if (n == "tuple" || n == "Tuple")
      return MakeTuple({});
    if (n == "Callable")
      return MakeFunction(core::Type::Any(), {});
  ...
  static core::Type ApplySubscript(core::Type base, std::vector<core::Type> args) {
    ...
    if (n == "list" || n == "List")
      return MakeGeneric("list", args.empty() ? std::vector<core::Type>{core::Type::Any()}
                                              : std::move(args));
    if (n == "dict" || n == "Dict") {
      std::vector<core::Type> a = std::move(args);
      while (a.size() < 2)
        a.push_back(core::Type::Any());
      return MakeGeneric("dict", std::move(a));
    }
    if (n == "set" || n == "Set" || n == "frozenset" || n == "FrozenSet") {
      return MakeGeneric("set", args.empty() ? std::vector<core::Type>{core::Type::Any()}
                                             : std::move(args));
    }
    if (n == "tuple" || n == "Tuple")
      return MakeTuple(std::move(args));
    if (n == "Callable") {
    ...
    if (n == "Type" || n == "type" || n == "ClassVar" || n == "Final" || n == "Annotated") {
  ...
  static core::Type MakeFunction(core::Type ret, std::vector<core::Type> params) {
    ...
    t.type_args.push_back(std::move(ret));
    for (auto &p : params)
      t.type_args.push_back(std::move(p));
    return t;
  }
...
// PyiLoader
// ============================================================================
PyiLoader::PyiLoader(std::vector<std::string> stub_paths, frontends::Diagnostics &diags) :
    search_paths_(std::move(stub_paths)), diags_(diags) {
...
std::optional<std::string> PyiLoader::LocateStubFile(const std::string &module_name) const {
  ...
  for (const auto &dir : search_paths_) {
    ...
    std::string top =
        (first_dot == std::string::npos) ? module_name : module_name.substr(0, first_dot);
    std::string sub =
        (first_dot == std::string::npos) ? std::string{} : module_name.substr(first_dot + 1);
...
std::unique_ptr<PyiModule> PyiLoader::ParseStubFile(const std::string &path,
