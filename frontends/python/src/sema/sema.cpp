...
class Analyzer {
public:
  Analyzer(const Module &mod, frontends::SemaContext &ctx, PyiLoader *loader = nullptr) :
...
private:
  ...
  void DeclareBuiltins() {
    ...
    for (const auto &exc : {"Exception", "ValueError", "TypeError", "RuntimeError", "KeyError",
                            "IndexError", "AttributeError", "StopIteration", "OSError", "IOError",
                            "FileNotFoundError", "ZeroDivisionError", "NotImplementedError",
                            "OverflowError", "ImportError", "NameError", "AssertionError"}) {
    ...
    auto declare_fn = [&](const std::string &name, const Type &ret,
      ...
      Symbol sym{name, Types().FunctionType(name, ret, params), builtin_loc, SymbolKind::kFunction,
                 "python"};
      Syms().Declare(sym);
    };
  ...
  std::unordered_map<std::string, std::unordered_map<std::string, Type>> BuiltinModuleExports() {
    return {
        ...
        {"asyncio",
         {
             {"sleep",
              Types().FunctionType("sleep", Types().Generic("coroutine", {Type::Void()}, "python"),
                                   {Type::Float()})},
        ...
        {"re",
         {
             {"match",
              Types().FunctionType("match", Type::Any(), {Type::String(), Type::String()})},
             {"search",
              Types().FunctionType("search", Type::Any(), {Type::String(), Type::String()})},
  ...
  bool IsKnownModule(const std::string &modname) {
    if (imported_modules_.count(modname))
      return true;
    if (loader_ && loader_->Resolve(modname) != nullptr)
      return true;
  ...
  void AnalyzeImportStatement(const ImportStatement &imp, const core::SourceLoc &loc) {
    ...
    if (!imp.is_from) {
      ...
      for (const auto &al : imp.names) {
        ...
        DeclareSimple(bind_name, SymbolKind::kModule, Type{core::TypeKind::kModule, modname}, loc);
  ...
  Type AnalyzeExpr(const std::shared_ptr<Expression> &expr) {
    ...
    if (auto await = std::dynamic_pointer_cast<AwaitExpression>(expr)) {
      ...
      if (awaited.kind == core::TypeKind::kGenericInstance && awaited.name == "coroutine" &&
          !awaited.type_args.empty()) {
    ...
    if (auto attr = std::dynamic_pointer_cast<AttributeExpression>(expr)) {
      ...
      if (obj_t.kind == core::TypeKind::kAny || obj_t.kind == core::TypeKind::kInvalid) {
        return Type::Any();
      }
      if (!obj_t.name.empty()) {
        // Builtin container protocols
        if (auto bt = ResolveBuiltinMember(obj_t, attr->attribute);
            bt.kind != core::TypeKind::kInvalid) {
          return bt;
        }
    ...
    if (auto idx = std::dynamic_pointer_cast<IndexExpression>(expr)) {
      ...
      if (obj_t.kind == core::TypeKind::kGenericInstance && obj_t.name == "list" &&
          !obj_t.type_args.empty()) {
        return obj_t.type_args.front();
      }
      if (obj_t.kind == core::TypeKind::kGenericInstance && obj_t.name == "dict" &&
          obj_t.type_args.size() >= 2) {
    ...
    if (auto bin = std::dynamic_pointer_cast<BinaryExpression>(expr)) {
      ...
      if (lhs.kind == core::TypeKind::kInvalid || rhs.kind == core::TypeKind::kInvalid) {
        return Type::Any();
      }
  ...
  Type ResolveBuiltinMember(const Type &obj_t, const std::string &attr) {
    ...
    if (obj_t.name == "list" && obj_t.kind == core::TypeKind::kGenericInstance) {
      if (attr == "append")
        return Types().FunctionType("append", Type::Void(),
                                    {obj_t.type_args.empty() ? Type::Any() : obj_t.type_args[0]});
      if (attr == "pop")
        return Types().FunctionType("pop",
                                    obj_t.type_args.empty() ? Type::Any() : obj_t.type_args[0], {});
      if (attr == "__len__")
        return Type::Int();
    }
    if (obj_t.name == "dict" && obj_t.kind == core::TypeKind::kGenericInstance) {
      ...
      if (attr == "get")
        return Types().FunctionType("get", v, {k});
      if (attr == "keys" || attr == "values")
        return Types().Generic("list", {attr == "keys" ? k : v}, "python");
      if (attr == "__len__")
        return Type::Int();
    }
    if (obj_t.name == "set" && obj_t.kind == core::TypeKind::kGenericInstance) {
      if (attr == "add")
        return Types().FunctionType("add", Type::Void(),
                                    {obj_t.type_args.empty() ? Type::Any() : obj_t.type_args[0]});
      if (attr == "__len__")
        return Type::Int();
    }
    ...
    if (obj_t.name == "coroutine" && obj_t.kind == core::TypeKind::kGenericInstance) {
      ...
      if (attr == "__await__")
        return Types().FunctionType("__await__", Types().Generic("iterator", {ret}, "python"), {});
      if (attr == "send")
        return Types().FunctionType("send", ret, {Type::Any()});
      if (attr == "throw")
        return Types().FunctionType("throw", ret, {Type::Any()});
      if (attr == "close")
        return Types().FunctionType("close", Type::Void(), {});
    }
    return Type::Invalid();
  }
