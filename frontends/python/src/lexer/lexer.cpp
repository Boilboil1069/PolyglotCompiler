...
bool PythonLexer::ParseFormatExpression(core::SourceLoc brace_loc) {
  ...
  while (!Eof() && brace_depth > 0) {
    ...
    if (in_format_spec) {
      ...
      if (!spec_text.empty()) {
        pending_.push_back(frontends::Token{frontends::TokenKind::kString, spec_text, loc});
      }
      continue;
    }
    ...
    if (c == '=' && brace_depth == 1) {
      ...
      if (ahead < source_.size() && (source_[ahead] == '}' || source_[ahead] == ':')) {
        Get();
        pending_.push_back(frontends::Token{frontends::TokenKind::kString, "=", loc});
        in_format_spec = true;
        continue;
      }
    }
...
frontends::Token PythonLexer::LexStringInternal(bool allow_formatting) {
  ...
  while (!Eof()) {
    ...
    if (formatted && c == '}') {
      ...
      flush_segment();
      pending_.push_back(
          frontends::Token{frontends::TokenKind::kUnknown, "unmatched } in f-string", ch_loc});
