...
bool PythonLexer::ParseFormatExpression(core::SourceLoc brace_loc) {
  int brace_depth = 1;
  pending_.push_back(
      frontends::Token{frontends::TokenKind::kSymbol, "{", brace_loc});
  ...
  pending_.push_back(frontends::Token{
      frontends::TokenKind::kUnknown, "unterminated f-string expression", brace_loc});
...
frontends::Token PythonLexer::LexStringInternal(bool allow_formatting) {
  ...
  while (!Eof()) {
    ...
    if (formatted && c == '}') {
      ...
      flush_segment();
      pending_.push_back(frontends::Token{
          frontends::TokenKind::kUnknown, "unmatched } in f-string", ch_loc});
...
void PythonLexer::HandleIndentation() {
  if (!at_line_start_ || paren_level_ > 0) return;
