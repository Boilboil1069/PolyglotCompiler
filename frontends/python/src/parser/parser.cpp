  ...
  while (current_.kind != frontends::TokenKind::kEndOfFile &&
    ...
    if (current_.kind == frontends::TokenKind::kKeyword) {
      if (current_.lexeme == "def" || current_.lexeme == "if" || current_.lexeme == "while" ||
          current_.lexeme == "for" || current_.lexeme == "return" || current_.lexeme == "import" ||
          current_.lexeme == "from" || current_.lexeme == "class" || current_.lexeme == "try" ||
          current_.lexeme == "with" || current_.lexeme == "match" || current_.lexeme == "async" ||
          current_.lexeme == "raise" || current_.lexeme == "pass" || current_.lexeme == "break" ||
...
std::shared_ptr<Expression> PythonParser::ParseAtom() {
  if (current_.kind == frontends::TokenKind::kKeyword) {
    if (current_.lexeme == "lambda")
      return ParseLambda();
    if (current_.lexeme == "True" || current_.lexeme == "False" || current_.lexeme == "None") {
  ...
  while (true) {
    ...
    if (IsSymbol("==") || IsSymbol("!=") || IsSymbol("<") || IsSymbol(">") || IsSymbol("<=") ||
      ...
      Consume();
               ...
               (current_.lexeme == "is" || current_.lexeme == "in" || current_.lexeme == "not")) {
      // When parsing for-loop / comprehension targets, stop at 'in'
      if (suppress_in_ && (current_.lexeme == "in" || (current_.lexeme == "not"))) {
        break;
      }
      if (current_.lexeme == "is") {
        ...
        Consume();
        if (current_.kind == frontends::TokenKind::kKeyword && current_.lexeme == "in") {
          Consume();
          op = "not in";
        } else {
...
std::vector<std::shared_ptr<Expression>> PythonParser::ParseExpressionList() {
  ...
  while (MatchSymbol(",")) {
    if (current_.kind == frontends::TokenKind::kNewline || IsSymbol(")") || IsSymbol("]") ||
        IsSymbol("}")) {
      break;
    }
    elems.push_back(ParseExpression());
...
std::shared_ptr<Expression> PythonParser::ParseComprehensionTail(
    std::shared_ptr<Expression> first, bool is_dict, bool is_set, bool is_generator,
...
std::shared_ptr<Statement> PythonParser::ParseStatement() {
  if (IsSymbol("@")) {
    ...
    } else if (current_.kind == frontends::TokenKind::kKeyword && current_.lexeme == "def") {
      stmt = ParseFunction();
      if (auto fn = std::dynamic_pointer_cast<FunctionDef>(stmt)) {
        fn->decorators = std::move(decorators);
      }
...
std::shared_ptr<Module> PythonParser::TakeModule() {
  return module_;
}
