...
std::vector<frontends::Token> PythonLanguageFrontend::Tokenize(const std::string &source,
...
bool PythonLanguageFrontend::Analyze(const std::string &source, const std::string &filename,
...
core::Type PyAnnotationToCore(const std::shared_ptr<Expression> &expr) {
  ...
  if (auto id = std::dynamic_pointer_cast<Identifier>(expr)) {
    ...
    if (n == "bytes")
      return core::Type{core::TypeKind::kSlice, "bytes", "python"};
    if (n == "object")
      return core::Type::Any();
  ...
  if (auto idx = std::dynamic_pointer_cast<IndexExpression>(expr)) {
    ...
    if (auto base = std::dynamic_pointer_cast<Identifier>(idx->object)) {
      if (base->name == "Optional")
        return PyAnnotationToCore(idx->index);
...
void ExtractFromFunctionDef(const FunctionDef &fn, const std::string &module_name,
                            const std::string &class_name,
                            std::vector<frontends::ForeignFunctionSignature> &out) {
  frontends::ForeignFunctionSignature sig;
  sig.name = fn.name;
  if (!class_name.empty()) {
    sig.qualified_name = module_name.empty() ? class_name + "::" + fn.name
