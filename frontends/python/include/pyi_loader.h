...
class PyiLoader {
public:
  PyiLoader(std::vector<std::string> stub_paths, frontends::Diagnostics &diags);
...
private:
  ...
  std::unique_ptr<PyiModule> ParseStubFile(const std::string &path, const std::string &module_name);
