...
// Diagnostics helper
// ============================================================================
core::SourceLoc LocOf(const std::string &p) {
  return core::SourceLoc{p, 0, 0};
}
...
public:
  ...
  std::uint32_t u32() {
    ...
    std::uint32_t v = std::uint32_t(base_[pos_]) | (std::uint32_t(base_[pos_ + 1]) << 8) |
...
bool ReadPE(const std::vector<std::uint8_t> &bytes, PEInfo &out, const std::string &path,
            frontends::Diagnostics &diags) {
  LEReader r(bytes.data(), bytes.size(), 0);
  if (bytes.size() < 0x40) {
    diags.ReportError(LocOf(path), frontends::ErrorCode::kUnknown, "PE: file too small: " + path);
    return false;
  }
...
struct MetadataInfo {
  ...
  std::vector<std::uint8_t> ReadBlob(std::uint32_t idx) const {
    ...
    if ((b0 & 0x80) == 0) {
      ...
      if (avail < 2)
        return {};
      length = (std::uint32_t(b0 & 0x3F) << 8) | p[1];
      hdr = 2;
    } else if ((b0 & 0xE0) == 0xC0) {
      if (avail < 4)
        return {};
      length = (std::uint32_t(b0 & 0x1F) << 24) | (std::uint32_t(p[1]) << 16) |
...
const CodedIndexInfo &CI_TypeDefOrRef() {
  static const CodedIndexInfo v{{kTabTypeDef, kTabTypeRef, kTabTypeSpec}, 2};
  return v;
}
...
const CodedIndexInfo &CI_HasCustomAttribute() {
  static const CodedIndexInfo v{{kTabMethodDef,   kTabField,        kTabTypeRef,
                                 kTabTypeDef,     kTabParam,        kTabInterfaceImpl,
                                 kTabMemberRef,   kTabModule,       kTabDeclSecurity,
                                 kTabProperty,    kTabEvent,        kTabStandAloneSig,
                                 kTabModuleRef,   kTabTypeSpec,     kTabAssembly,
                                 kTabAssemblyRef, kTabFile,         kTabExportedType,
                                 kTabManifestRes, kTabGenericParam, kTabGenericParamConstraint,
                                 kTabMethodSpec},
                                5};
  return v;
}
...
const CodedIndexInfo &CI_CustomAttributeType() {
  ...
  static const CodedIndexInfo v{{0xFF, 0xFF, kTabMethodDef, kTabMemberRef, 0xFF}, 3};
  return v;
}
const CodedIndexInfo &CI_ResolutionScope() {
  static const CodedIndexInfo v{{kTabModule, kTabModuleRef, kTabAssemblyRef, kTabTypeRef}, 2};
  return v;
}
...
// ============================================================================
bool LocateStreams(const std::uint8_t *base, std::size_t size, std::size_t md_off, MetadataInfo &md,
...
ColumnSizes ComputeColumnSizes(const MetadataInfo &md) {
  ...
  for (int i = 0; i < 64; ++i)
    c.simple[i] = SimpleIndexBytes(md, std::uint8_t(i));
...
std::size_t RowSize(std::uint8_t tab, const ColumnSizes &c) {
  switch (tab) {
  case kTabModule:
    return 2 + c.string_idx + 3 * c.guid_idx;
  case kTabTypeRef:
    return c.ResolutionScope + 2 * c.string_idx;
  case kTabTypeDef:
    return 4 + 2 * c.string_idx + c.TypeDefOrRef + c.simple[kTabField] + c.simple[kTabMethodDef];
  case kTabField:
    return 2 + c.string_idx + c.blob_idx;
  case kTabMethodDef:
    return 4 + 2 + 2 + c.string_idx + c.blob_idx + c.simple[kTabParam];
  case kTabParam:
    return 2 + 2 + c.string_idx;
  case kTabInterfaceImpl:
    return c.simple[kTabTypeDef] + c.TypeDefOrRef;
  case kTabMemberRef:
    return c.MemberRefParent + c.string_idx + c.blob_idx;
  case kTabConstant:
    return 2 + c.HasConstant + c.blob_idx;
  case kTabCustomAttr:
    return c.HasCustomAttribute + c.CustomAttributeType + c.blob_idx;
  case kTabFieldMarshal:
    return c.HasFieldMarshal + c.blob_idx;
  case kTabDeclSecurity:
    return 2 + c.HasDeclSecurity + c.blob_idx;
  case kTabClassLayout:
    return 2 + 4 + c.simple[kTabTypeDef];
  case kTabFieldLayout:
    return 4 + c.simple[kTabField];
  case kTabStandAloneSig:
    return c.blob_idx;
  case kTabEventMap:
    return c.simple[kTabTypeDef] + c.simple[kTabEvent];
  case kTabEvent:
    return 2 + c.string_idx + c.TypeDefOrRef;
  case kTabPropertyMap:
    return c.simple[kTabTypeDef] + c.simple[kTabProperty];
  case kTabProperty:
    return 2 + c.string_idx + c.blob_idx;
  case kTabMethodSemantics:
    return 2 + c.simple[kTabMethodDef] + c.HasSemantics;
  case kTabMethodImpl:
    return c.simple[kTabTypeDef] + 2 * c.MethodDefOrRef;
  case kTabModuleRef:
    return c.string_idx;
  case kTabTypeSpec:
    return c.blob_idx;
  case kTabImplMap:
    return 2 + c.MemberForwarded + c.string_idx + c.simple[kTabModuleRef];
  case kTabFieldRVA:
    return 4 + c.simple[kTabField];
  case kTabAssembly:
    return 4 + 4 * 2 + 4 + c.blob_idx + 2 * c.string_idx;
  case kTabAssemblyProc:
    return 4;
  case kTabAssemblyOS:
    return 12;
  case kTabAssemblyRef:
    return 4 * 2 + 4 + 2 * c.blob_idx + 2 * c.string_idx;
  case kTabAssemblyRefProc:
    return 4 + c.simple[kTabAssemblyRef];
  case kTabAssemblyRefOS:
    return 12 + c.simple[kTabAssemblyRef];
  case kTabFile:
    return 4 + c.string_idx + c.blob_idx;
  case kTabExportedType:
    return 4 + 4 + 2 * c.string_idx + c.Implementation;
  case kTabManifestRes:
    return 4 + 4 + c.string_idx + c.Implementation;
  case kTabNestedClass:
    return 2 * c.simple[kTabTypeDef];
  case kTabGenericParam:
    return 2 + 2 + c.TypeOrMethodDef + c.string_idx;
  case kTabMethodSpec:
    return c.MethodDefOrRef + c.blob_idx;
  case kTabGenericParamConstraint:
    return c.simple[kTabGenericParam] + c.TypeDefOrRef;
  default:
    return 0;
  }
}
...
std::string ReadTypeDefName(const MetadataInfo &md, std::uint32_t row, const ColumnSizes &c,
                            const std::array<std::size_t, 64> &row_off) {
  if (row == 0 || row > md.rows[kTabTypeDef])
    return {};
...
std::string ReadTypeRefName(const MetadataInfo &md, std::uint32_t row, const ColumnSizes &c,
                            const std::array<std::size_t, 64> &row_off) {
  if (row == 0 || row > md.rows[kTabTypeRef])
    return {};
...
std::string ResolveTypeRef(const MetadataInfo &md, std::uint8_t tab, std::uint32_t row,
  ...
  if (row == 0)
    return {};
  if (tab == kTabTypeDef)
    return ReadTypeDefName(md, row, c, row_off);
  if (tab == kTabTypeRef)
    return ReadTypeRefName(md, row, c, row_off);
...
public:
  ...
  std::uint32_t cu() {
    ...
    if ((b & 0xE0) == 0xC0) {
      ...
      std::uint32_t v = (std::uint32_t(b & 0x1F) << 24) | (std::uint32_t(p_[0]) << 16) |
...
void SkipCMods(SigReader &sr) {
  while (!sr.eof() && !sr.fail()) {
    ...
    if (b != ET_CMOD_REQD && b != ET_CMOD_OPT)
      break;
    sr.u8();
...
core::Type ParseSigType(SigReader &sr, const MetadataInfo &md, const ColumnSizes &c,
  ...
  case ET_ARRAY: {
    ...
    for (std::uint32_t i = 0; i < num_sizes; ++i)
      sr.cu();
    std::uint32_t num_lo = sr.cu();
    for (std::uint32_t i = 0; i < num_lo; ++i)
      sr.cu();
    return core::Type::Array(std::move(elem));
  }
  case ET_PTR: {
...
MethodSig ParseMethodSig(const std::vector<std::uint8_t> &blob, const MetadataInfo &md,
                         const ColumnSizes &c, const std::array<std::size_t, 64> &row_off) {
  MethodSig s;
...
core::Type ParseFieldSig(const std::vector<std::uint8_t> &blob, const MetadataInfo &md,
                         const ColumnSizes &c, const std::array<std::size_t, 64> &row_off) {
  SigReader sr(blob.data(), blob.size());
  if (sr.eof())
    return core::Type::Any();
...
// ============================================================================
bool ParseAssembly(const std::vector<std::uint8_t> &bytes, const PEInfo &pe, std::size_t md_off,
                   CliAssembly &out, const std::string &path, frontends::Diagnostics &diags) {
  MetadataInfo md;
  if (!LocateStreams(bytes.data(), bytes.size(), md_off, md, path, diags))
    return false;
  ...
  auto read_typedef = [&](std::uint32_t i, std::uint32_t &flags, std::string &ns, std::string &name,
  ...
  for (std::uint32_t i = 0; i < typedef_count; ++i) {
    ...
    for (std::uint32_t fi = field_list; fi < next_field_list && fi >= 1 && fi <= field_total;
         ++fi) {
      CliFieldMeta f;
      read_field_row(fi - 1, f);
      t.fields.push_back(std::move(f));
    }
    for (std::uint32_t mi = method_list; mi < next_method_list && mi >= 1 && mi <= method_total;
         ++mi) {
      CliMethodMeta m;
...
std::optional<CliAssembly> ReadAssemblyMetadata(const std::string &path,
                                                frontends::Diagnostics &diags) {
  std::vector<std::uint8_t> bytes;
  if (!LoadFile(path, bytes)) {
    diags.ReportError(LocOf(path), frontends::ErrorCode::kUnknown, "cannot open assembly: " + path);
    return std::nullopt;
  }
  PEInfo pe;
  if (!ReadPE(bytes, pe, path, diags))
    return std::nullopt;
  ...
  CliAssembly out;
  out.path = path;
  if (!ParseAssembly(bytes, pe, *md_off, out, path, diags))
    return std::nullopt;
  return out;
}
...
AssemblyLoader::AssemblyLoader(const std::vector<std::string> &references,
  ...
  for (const auto &p : references) {
    ...
    if (a)
      assemblies_.push_back(std::move(*a));
  }
}

std::vector<const CliTypeMeta *> AssemblyLoader::ListNamespace(
    const std::string &namespace_name) const {
  std::vector<const CliTypeMeta *> out;
  for (const auto &asm_ : assemblies_) {
    auto it = asm_.ns_index.find(namespace_name);
    if (it == asm_.ns_index.end())
      continue;
...
const CliTypeMeta *AssemblyLoader::ResolveType(const std::string &full_name) const {
  for (const auto &asm_ : assemblies_) {
