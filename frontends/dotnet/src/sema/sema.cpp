...
class Analyzer {
public:
  Analyzer(const Module &mod, frontends::SemaContext &ctx, AssemblyLoader *loader) :
...
private:
  ...
  void DeclareUsingType(const CliTypeMeta &t, const core::SourceLoc &loc, bool import_static) {
    ...
    for (const auto &f : t.fields) {
      if ((f.flags & kFieldStatic) == 0)
        continue;
      if ((f.flags & 0x0007) != kFieldPublic)
        continue;
      Symbol fs{f.name, f.type, loc, SymbolKind::kVariable, "csharp"};
      Syms().Declare(fs);
    }
    for (const auto &m : t.methods) {
      if ((m.flags & kMethodStatic) == 0)
        continue;
      if ((m.flags & 0x0007) != kMethodPublic)
        continue;
  ...
  void AnalyzeUsing(const UsingDirective &u) {
    ...
    Symbol module_sym{u.alias.empty() ? u.ns : u.alias, Type::Module(u.ns, "csharp"), u.loc,
    ...
    if (u.is_static) {
      ...
      if (t)
        DeclareUsingType(*t, u.loc, /*import_static=*/true);
      return;
    }
    ...
    for (const CliTypeMeta *t : loader_->ListNamespace(u.ns)) {
      if (t)
        DeclareUsingType(*t, u.loc, /*import_static=*/false);
    }
  }
  /** @} */
  ...
  void AnalyzeStmt(const std::shared_ptr<Statement> &stmt) {
    ...
    if (std::dynamic_pointer_cast<ClassDecl>(stmt) || std::dynamic_pointer_cast<StructDecl>(stmt) ||
    ...
    if (auto for_stmt = std::dynamic_pointer_cast<ForStatement>(stmt)) {
      ...
      if (for_stmt->condition)
        AnalyzeExpr(for_stmt->condition);
      if (for_stmt->update)
        AnalyzeExpr(for_stmt->update);
    ...
    if (auto foreach_stmt = std::dynamic_pointer_cast<ForEachStatement>(stmt)) {
      ...
      Symbol sym{foreach_stmt->var_name, elem, foreach_stmt->loc, SymbolKind::kVariable, "csharp"};
      Syms().Declare(sym);
    ...
    if (auto try_stmt = std::dynamic_pointer_cast<TryStatement>(stmt)) {
      ...
      for (auto &c : try_stmt->catches) {
        ...
        if (!c.var_name.empty()) {
          ...
          Symbol ex{c.var_name, et, c.body ? c.body->loc : try_stmt->loc, SymbolKind::kVariable,
                    "csharp"};
          Syms().Declare(ex);
        }
        if (c.filter)
          AnalyzeExpr(c.filter);
        AnalyzeStmt(c.body);
        ExitScope();
      }
    ...
    if (auto using_stmt = std::dynamic_pointer_cast<UsingStatement>(stmt)) {
      if (using_stmt->declaration) {
        ...
        if (using_stmt->declaration->init)
          AnalyzeExpr(using_stmt->declaration->init);
        Symbol sym{using_stmt->declaration->name, t, using_stmt->loc, SymbolKind::kVariable,
                   "csharp"};
        Syms().Declare(sym);
      }
      if (using_stmt->expression)
        AnalyzeExpr(using_stmt->expression);
      if (using_stmt->body)
        AnalyzeStmt(using_stmt->body);
      return;
    }
  ...
  Type AnalyzeExpr(const std::shared_ptr<Expression> &expr) {
    ...
    if (auto new_expr = std::dynamic_pointer_cast<NewExpression>(expr)) {
      for (auto &arg : new_expr->args)
        AnalyzeExpr(arg);
      for (auto &i : new_expr->initializer)
        AnalyzeExpr(i);
    ...
    if (auto is_expr = std::dynamic_pointer_cast<IsExpression>(expr)) {
      ...
      if (!is_expr->pattern_var.empty()) {
        ...
        Symbol ps{is_expr->pattern_var, pt, is_expr->loc, SymbolKind::kVariable, "csharp"};
        Syms().Declare(ps);
      }
      return Type::Bool();
    }
    ...
    if (auto index = std::dynamic_pointer_cast<IndexExpression>(expr)) {
      ...
      if (!arr_type.type_args.empty())
        return arr_type.type_args[0];
      return Type::Any();
    }
