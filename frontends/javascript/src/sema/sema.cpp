...
private:
  ...
  bool DeclareLexical(const std::string &name, const std::string &kind,
    ...
    if (it != top.bindings.end()) {
      if (kind == "let" || kind == "const" || it->second == "let" || it->second == "const") {
        ctx_.Diags().Report(loc, "redeclaration of '" + name + "' (previously declared as " +
                                     it->second + ")");
        return false;
      }
    }
  ...
  void HoistStatement(const std::shared_ptr<Statement> &s) {
    ...
    if (auto fio = std::dynamic_pointer_cast<ForInOfStatement>(s)) {
      if (fio->var_kind == "var")
        DeclareVar(fio->var_name, fio->loc);
  ...
  void AnalyzeStatement(const std::shared_ptr<Statement> &s) {
    ...
    if (auto vd = std::dynamic_pointer_cast<VariableDecl>(s)) {
      for (auto &d : vd->decls) {
        ...
        if (d.init)
          AnalyzeExpression(d.init);
      }
      return;
    }
    if (auto fd = std::dynamic_pointer_cast<FunctionDecl>(s)) {
      if (!fd->name.empty())
        DeclareLexical(fd->name, "function", fd->loc);
      AnalyzeFunctionBody(fd->params, fd->body);
      return;
    }
    if (auto cd = std::dynamic_pointer_cast<ClassDecl>(s)) {
      if (!cd->name.empty())
        DeclareLexical(cd->name, "class", cd->loc);
      if (cd->superclass)
        AnalyzeExpression(cd->superclass);
    ...
    if (auto fs = std::dynamic_pointer_cast<ForStatement>(s)) {
      ...
      if (fs->condition)
        AnalyzeExpression(fs->condition);
      if (fs->update)
        AnalyzeExpression(fs->update);
      ++loop_depth_;
    ...
    if (auto fio = std::dynamic_pointer_cast<ForInOfStatement>(s)) {
      ...
      if (!fio->var_kind.empty()) {
        if (fio->var_kind == "var")
          ...
          DeclareLexical(fio->var_name, fio->var_kind, fio->loc);
      }
    ...
    if (auto tr = std::dynamic_pointer_cast<TryStatement>(s)) {
      ...
      if (tr->handler) {
        EnterBlock();
        if (!tr->catch_var.empty())
          DeclareLexical(tr->catch_var, "let", tr->loc);
    ...
    if (auto imp = std::dynamic_pointer_cast<ImportDecl>(s)) {
      for (auto &spec : imp->specifiers) {
        if (!spec.local.empty())
          DeclareLexical(spec.local, "const", imp->loc);
      }
      return;
    }
    if (auto exp = std::dynamic_pointer_cast<ExportDecl>(s)) {
      if (exp->declaration)
        AnalyzeStatement(exp->declaration);
      if (exp->default_expr)
        AnalyzeExpression(exp->default_expr);
      return;
    }
  }
  ...
  void AnalyzeFunctionBody(const std::vector<ArrowFunction::Param> &params,
    ...
    for (auto &p : params) {
      ...
      if (p.default_value)
        AnalyzeExpression(p.default_value);
    }
  ...
  void AnalyzeExpression(const std::shared_ptr<Expression> &e) {
    ...
    if (auto id = std::dynamic_pointer_cast<Identifier>(e)) {
      ...
      if (!LookupIdent(id->name) && !options_strict_) {
        ...
        ctx_.Diags().Report(id->loc, "use of undeclared identifier '" + id->name + "'");
      }
      return;
    }
    if (std::dynamic_pointer_cast<Literal>(e))
      return;
    if (auto arr = std::dynamic_pointer_cast<ArrayExpr>(e)) {
      for (auto &el : arr->elements)
        AnalyzeExpression(el);
      return;
    }
    if (auto obj = std::dynamic_pointer_cast<ObjectExpr>(e)) {
      for (auto &p : obj->properties)
        AnalyzeExpression(p.value);
      return;
    }
