...
JsImportResolver::JsImportResolver(std::string project_dir,
    ...
    project_dir_(std::move(project_dir)), extra_node_modules_(std::move(extra_node_modules)),
...
std::string JsImportResolver::ResolveRelative(const std::string &specifier,
  ...
  if (auto with_ext = ProbeExtensions(candidate.string()); !with_ext.empty())
    return with_ext;
  if (DirExists(candidate)) {
    // Try package.json in the directory, then index.*
    if (auto entry = ResolvePackageEntry(candidate.string()); !entry.empty())
      return entry;
    if (auto idx = ProbeExtensions((candidate / "index").string()); !idx.empty())
      return idx;
  }
  return {};
}
...
std::string JsImportResolver::ResolvePackageEntry(const std::string &pkg_dir) const {
  ...
  for (const char *field : {"types", "typings", "module", "main"}) {
    ...
    if (FileExists(entry))
      return entry.string();
    if (auto with_ext = ProbeExtensions(entry.string()); !with_ext.empty())
      return with_ext;
    if (DirExists(entry)) {
      if (auto idx = ProbeExtensions((entry / "index").string()); !idx.empty())
        return idx;
    }
  }
  if (auto idx = ProbeExtensions((fs::path(pkg_dir) / "index").string()); !idx.empty())
    return idx;
  return {};
}
...
std::string JsImportResolver::ResolveBare(const std::string &specifier,
  ...
  if (!specifier.empty() && specifier[0] == '@') {
    ...
    if (first == std::string::npos) {
      ...
      if (second == std::string::npos)
        ...
        pkg_name = specifier.substr(0, second);
        remainder = specifier.substr(second + 1);
      }
    }
  } else {
    auto first = specifier.find('/');
    if (first == std::string::npos)
      ...
      pkg_name = specifier.substr(0, first);
      remainder = specifier.substr(first + 1);
    }
  }
    ...
    for (fs::path cur = start;; cur = cur.parent_path()) {
      ...
      if (DirExists(nm)) {
        ...
        if (!remainder.empty()) {
          ...
          if (FileExists(full))
            return full.string();
          if (auto we = ProbeExtensions(full.string()); !we.empty())
            return we;
          if (DirExists(full)) {
            if (auto idx = ProbeExtensions((full / "index").string()); !idx.empty())
              return idx;
          }
          return {};
        }
  ...
  if (!importer_dir.empty()) {
    if (auto r = walk_node_modules(importer_dir); !r.empty())
      return r;
  }
  if (!project_dir_.empty()) {
    if (auto r = walk_node_modules(project_dir_); !r.empty())
      return r;
  }
...
std::unique_ptr<JsModule> JsImportResolver::LoadModule(const std::string &specifier,
  ...
  auto record_function = [&](const std::string &name,
                             ...
                             const std::shared_ptr<TypeNode> &ret_t, bool is_default) {
    JsSymbol s;
    s.name = name;
    ...
    if (auto e = std::dynamic_pointer_cast<ExportDecl>(stmt)) {
      if (e->declaration) {
        if (auto fd = std::dynamic_pointer_cast<FunctionDecl>(e->declaration)) {
          ...
          for (const auto &d : vd->decls)
            record_const(d.name, d.type);
        }
      }
...
const JsModule *JsImportResolver::Resolve(const std::string &specifier,
  ...
  if (resolved.empty()) {
    diagnostics_.Report(
        core::SourceLoc{},
        "javascript module not found: '" + specifier + "' (importer_dir='" + importer_dir +
