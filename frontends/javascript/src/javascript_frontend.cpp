...
std::vector<frontends::Token> JsLanguageFrontend::Tokenize(const std::string &source,
...
bool JsLanguageFrontend::Analyze(const std::string &source, const std::string &filename,
  ...
  JsImportResolver resolver(options.js_project_dir, options.node_modules_paths, diagnostics);
  for (const auto &stmt : module->body) {
    ...
    for (const auto &spec : imp->specifiers) {
      ...
      s.kind =
          (it->second.kind == JsSymbolKind::kFunction) ? core::SymbolKind::kFunction
          : (it->second.kind == JsSymbolKind::kClass || it->second.kind == JsSymbolKind::kTypeAlias)
...
void ExtractFromFunction(const std::string &fn_name,
  ...
  sig.name = fn_name;
  if (!class_name.empty()) {
    sig.qualified_name = module_name.empty() ? class_name + "." + fn_name
                                             : module_name + "." + class_name + "." + fn_name;
    sig.is_method = true;
    sig.class_name = class_name;
  } else {
    sig.qualified_name = module_name.empty() ? fn_name : module_name + "." + fn_name;
  }
...
std::vector<frontends::ForeignFunctionSignature> JsLanguageFrontend::ExtractSignatures(
  ...
  auto walk = [&](auto &&self, const std::shared_ptr<Statement> &s) -> void {
    if (auto fd = std::dynamic_pointer_cast<FunctionDecl>(s)) {
      if (!fd->name.empty()) {
        ExtractFromFunction(fd->name, fd->params, fd->return_type, module_name, "", result);
      }
      ...
      for (auto &m : cd->members) {
        if (auto md = std::dynamic_pointer_cast<MethodDecl>(m)) {
          if (md->kind == MethodDecl::Kind::kConstructor) {
            ExtractFromFunction("constructor", md->params, md->return_type, module_name, cd->name,
                                result);
          } else if (!md->name.empty() && !md->is_private) {
            ExtractFromFunction(md->name, md->params, md->return_type, module_name, cd->name,
                                result);
          }
        }
      }
