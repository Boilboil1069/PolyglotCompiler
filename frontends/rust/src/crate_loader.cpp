...
core::Type RustTypeToCore(const std::shared_ptr<TypeNode> &tn) {
  ...
  if (auto tp = std::dynamic_pointer_cast<TypePath>(tn)) {
    ...
    if ((name == "Option" || name == "Result" || name == "Box") && !tp->generic_args.empty() &&
        !tp->generic_args[0].empty()) {
  ...
  if (auto rt = std::dynamic_pointer_cast<ReferenceType>(tn))
    return RustTypeToCore(rt->inner);
...
class CrateWalker {
public:
  CrateWalker(CrateInfo &info, frontends::Diagnostics &diags) : info_(info), diags_(diags) {}
...
private:
  void WalkFile(const fs::path &file, const std::string &mod_path, const fs::path &mod_dir) {
    if (visited_.count(fs::weakly_canonical(file).string()))
      return;
  ...
  std::string Join(const std::string &mod_path, const std::string &name) const {
    if (mod_path.empty())
      return info_.name + "::" + name;
    return mod_path + "::" + name;
  }

  void Insert(CrateItemKind kind, const std::string &short_name, const std::string &mod_path,
              core::Type type, std::vector<core::Type> params = {},
  ...
  void WalkItems(const std::vector<std::shared_ptr<Statement>> &items, const std::string &mod_path,
                 const fs::path &mod_dir) {
    for (const auto &item : items) {
      if (auto fn = std::dynamic_pointer_cast<FunctionItem>(item)) {
        if (!IsPublic(fn->visibility))
          continue;
        ...
        core::Type ret = fn->return_type ? RustTypeToCore(fn->return_type) : core::Type::Void();
        core::Type fnt{core::TypeKind::kFunction, fn->name};
        fnt.language = "rust";
        fnt.type_args.reserve(params.size() + 1);
        fnt.type_args.push_back(ret);
        for (auto &p : params)
          fnt.type_args.push_back(p);
        Insert(CrateItemKind::kFunction, fn->name, mod_path, std::move(fnt), std::move(params),
               ret);
        continue;
      }
      if (auto st = std::dynamic_pointer_cast<StructItem>(item)) {
        if (!IsPublic(st->visibility))
          continue;
...
void CrateLoader::LoadFromPath(const std::string &explicit_name, const std::string &path_in,
                               frontends::Diagnostics &diags) {
  fs::path p(path_in);
  std::error_code ec;
  if (!fs::exists(p, ec)) {
    diags.Report({"<rust-extern>", 0, 0}, "Rust crate path does not exist: " + path_in);
    return;
  }
  ...
  if (fs::is_directory(p, ec)) {
    ...
    if (info->name.empty())
      info->name = explicit_name.empty() ? p.filename().string() : explicit_name;

    fs::path entry;
    if (fs::exists(lib_rs))
      entry = lib_rs;
    else if (fs::exists(main_rs))
      entry = main_rs;
    else {
      // Fallback: any .rs file directly inside the directory.
      for (const auto &de : fs::directory_iterator(p, ec)) {
        if (de.is_regular_file() && de.path().extension() == ".rs") {
          entry = de.path();
          break;
        }
      }
    ...
    diags.Report({"<rust-extern>", 0, 0}, "Unsupported Rust artefact: " + p.string());
    return;
  }

  if (info->name.empty()) {
    diags.Report({"<rust-extern>", 0, 0}, "Could not determine crate name for: " + path_in);
    return;
  }
