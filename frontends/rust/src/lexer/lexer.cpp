...
frontends::Token LexDocLine(const std::string &source, const std::string &file, size_t &position,
    ...
    return frontends::Token{frontends::TokenKind::kComment, text,
                            core::SourceLoc{file, start_line, start_col}, true};
}

frontends::Token LexDocBlock(const std::string &source, const std::string &file, size_t &position,
                             size_t &line, size_t &column) {
    size_t start_pos = position;
    size_t start_col = column;
    size_t start_line = line;
...
frontends::Token RustLexer::NextToken() {
    ...
    if (c == '\'') {
        if (std::isalpha(static_cast<unsigned char>(PeekNext())) && PeekNext() != '\0' &&
            source_.size() > position_ + 2 && source_[position_ + 2] != '\'') {
