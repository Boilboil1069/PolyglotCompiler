...
class Analyzer {
public:
  Analyzer(const Module &mod, frontends::SemaContext &ctx, CrateLoader *loader = nullptr) :
...
private:
  ...
  bool ValidateReturnLifetime(const Type &ret_type, const std::vector<std::string> &fn_lifetimes,
    ...
    if (ret_type.kind != core::TypeKind::kPointer)
      return true;
  ...
  bool ValidateBorrowLifetime(const Type &arg_type, const std::string &param_lifetime,
                              const core::SourceLoc &loc) {
    if (arg_type.kind != core::TypeKind::kPointer)
      return true;
    ...
    if (!param_lifetime.empty() && param_lifetime != "anon") {
      ...
      if (!LifetimeOutlives(arg_type.lifetime, param_lifetime)) {
        Diags().Report(loc, "lifetime '" + arg_type.lifetime + "' does not outlive '" +
                                param_lifetime + "'");
        return false;
      }
    }
  ...
  void AnalyzeItem(const std::shared_ptr<Statement> &item) {
    ...
    if (auto ruse = std::dynamic_pointer_cast<UseDeclaration>(item)) {
      ...
      Symbol us{ruse->path, Type::Module(ruse->path, "rust"), ruse->loc, SymbolKind::kModule,
                "rust"};
      Syms().Declare(us);
      ...
      if (loader_) {
        ...
        if (as_pos != std::string::npos) {
          ...
          // Strip whitespace
          while (!alias_name.empty() &&
                 std::isspace(static_cast<unsigned char>(alias_name.front())))
            alias_name.erase(0, 1);
          while (!alias_name.empty() && std::isspace(static_cast<unsigned char>(alias_name.back())))
            alias_name.pop_back();
          path.erase(as_pos);
        }
        ...
        if (const auto *item = loader_->ResolvePath(path)) {
          if (!bind.empty()) {
            ...
            case CrateItemKind::kMacro:
              kind = SymbolKind::kFunction;
              break;
            case CrateItemKind::kStruct:
            case CrateItemKind::kEnum:
            case CrateItemKind::kTrait:
            case CrateItemKind::kTypeAlias:
              kind = SymbolKind::kTypeName;
              break;
            case CrateItemKind::kConst:
            case CrateItemKind::kStatic:
              kind = SymbolKind::kVariable;
              break;
            case CrateItemKind::kModule:
              kind = SymbolKind::kModule;
              break;
            }
    ...
    if (current_return_type_.kind == core::TypeKind::kPointer) {
      if (current_return_type_.lifetime.empty() || current_return_type_.lifetime == "anon") {
        ...
        if (!elision_applies && current_fn_lifetimes_.empty()) {
          Diags().Report(fn.loc,
                         "return type contains reference but function has no lifetime parameters");
        }
      } else {
  ...
  void AnalyzeStmt(const std::shared_ptr<Statement> &stmt) {
    ...
    if (auto let = std::dynamic_pointer_cast<LetStatement>(stmt)) {
      ...
      if (t.kind == core::TypeKind::kAny && let->init)
        t = AnalyzeExpr(let->init);
      ...
      if (t.kind == core::TypeKind::kPointer) {
        if (t.lifetime.empty() || t.lifetime == "anon") {
          ...
          if (!can_infer) {
            Diags().Report(let->loc, "reference binding requires lifetime annotation or "
                                     "initialization from local reference");
          }
        }
      }
  ...
  void DeclarePattern(const std::shared_ptr<Pattern> &pat, const Type &type,
    ...
    if (auto bind = std::dynamic_pointer_cast<BindingPattern>(pat)) {
      ...
      Syms().Declare(sym);
      if (!borrow_stack_.empty())
        borrow_stack_.back()[bind->name] = {};
      if (bind->pattern)
        DeclarePattern(bind->pattern, type, loc);
      return;
    }
    ...
    if (auto orp = std::dynamic_pointer_cast<OrPattern>(pat)) {
      for (auto &p : orp->patterns)
        DeclarePattern(p, type, loc);
      return;
    }
  }
  ...
  Type AnalyzeExpr(const std::shared_ptr<Expression> &expr) {
    ...
    if (auto assign = std::dynamic_pointer_cast<AssignmentExpression>(expr)) {
      ...
      if (auto lhs_id = std::dynamic_pointer_cast<Identifier>(assign->left)) {
        ...
        if (active.mut > 0 || active.imm > 0) {
          Diags().Report(assign->loc,
                         "cannot assign to `" + lhs_id->name + "` while it is borrowed");
        }
      }
    ...
    if (auto un = std::dynamic_pointer_cast<UnaryExpression>(expr)) {
      if (un->op == "&" || un->op == "&mut") {
        ...
        std::string name;
        if (auto id = std::dynamic_pointer_cast<Identifier>(un->operand))
          name = id->name;
        if (!name.empty()) {
  ...
  bool IsWildcardPattern(const std::shared_ptr<Pattern> &pat) const {
    if (!pat)
      return false;
    if (std::dynamic_pointer_cast<WildcardPattern>(pat))
      return true;
    if (auto id = std::dynamic_pointer_cast<IdentifierPattern>(pat))
      return id->name == "_";
    if (auto orp = std::dynamic_pointer_cast<OrPattern>(pat)) {
