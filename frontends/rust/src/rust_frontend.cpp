...
std::vector<frontends::Token> RustLanguageFrontend::Tokenize(const std::string &source,
...
bool RustLanguageFrontend::Analyze(const std::string &source, const std::string &filename,
...
core::Type RustTypeToCore(const std::shared_ptr<TypeNode> &tn) {
  if (!tn)
    return core::Type::Void(); // no return type →()
  ...
  if (auto tt = std::dynamic_pointer_cast<TupleType>(tn)) {
    if (tt->elements.empty())
      return core::Type::Void(); // () →void
    return core::Type{core::TypeKind::kStruct, "tuple", "rust"};
  }
  return core::Type::Any();
}

void ExtractFromFunction(const FunctionItem &fn, const std::string &module_name,
  ...
  if (!impl_type.empty()) {
    sig.qualified_name = module_name.empty() ? impl_type + "::" + fn.name
  ...
  sig.return_type = fn.return_type ? RustTypeToCore(fn.return_type) : core::Type::Void();
