...
bool IsTerminatorKind(TokenKind k, const std::string &lex) {
  if (k == TokenKind::kIdentifier)
    return true;
  if (k == TokenKind::kNumber || k == TokenKind::kNumber || k == TokenKind::kString ||
      k == TokenKind::kChar)
    return true;
  if (k == TokenKind::kKeyword) {
    return lex == "break" || lex == "continue" || lex == "fallthrough" || lex == "return" ||
           lex == "true" || lex == "false" || lex == "nil" || lex == "iota";
  }
