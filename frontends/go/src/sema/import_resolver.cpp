...
core::Type ToCoreType(const std::shared_ptr<TypeNode> &t) {
  ...
  case TypeKind::kSlice:
    return core::Type::Slice(ToCoreType(t->elem));
  case TypeKind::kArray:
    return core::Type::Array(ToCoreType(t->elem));
  case TypeKind::kMap:
...
std::string DetectGoroot() {
  if (const char *env = std::getenv("GOROOT"); env && *env)
    return env;
#ifdef _WIN32
  const char *defaults[] = {"C:/Program Files/Go", "C:/Go"};
#elif defined(__APPLE__)
  const char *defaults[] = {"/usr/local/go", "/opt/homebrew/Cellar/go",
...
std::string DetectGopath() {
  if (const char *env = std::getenv("GOPATH"); env && *env)
    return env;
...
GoImportResolver::GoImportResolver(std::string project_dir,
    ...
    project_dir_(std::move(project_dir)), module_paths_(std::move(extra_module_paths)),
  ...
  if (std::string gopath = DetectGopath(); !gopath.empty()) {
    ...
    std::error_code ec;
    if (fs::exists(mod_cache, ec))
      module_paths_.push_back(mod_cache);
  }
  if (std::string goroot = DetectGoroot(); !goroot.empty()) {
    std::string std_src = goroot + "/src";
    std::error_code ec;
    if (fs::exists(std_src, ec))
      module_paths_.push_back(std_src);
  }
...
void GoImportResolver::LoadGoMod() {
  ...
  while (std::getline(iss, line)) {
    ...
    if (t.rfind("module ", 0) == 0) {
      ...
      if (manifest_.module_path.size() >= 2 && manifest_.module_path.front() == '"' &&
          manifest_.module_path.back() == '"') {
    ...
    if (in_require) {
      parse_require_line(t);
      continue;
    }
    if (in_replace) {
      parse_replace_line(t);
      continue;
    }
    if (t.rfind("require ", 0) == 0)
      parse_require_line(t.substr(8));
    else if (t.rfind("replace ", 0) == 0)
      parse_replace_line(t.substr(8));
  }
}
...
std::string GoImportResolver::LocatePackageDir(const std::string &import_path) const {
  ...
  if (auto it = manifest_.replaces.find(import_path); it != manifest_.replaces.end()) {
    ...
    if (repl.is_relative() && !project_dir_.empty())
      repl = fs::path(project_dir_) / repl;
    if (exists_dir(repl))
      return repl.string();
  }
...
// ---------------------------------------------------------------------------
std::unique_ptr<GoPackage> GoImportResolver::LoadPackage(const std::string &import_path,
                                                         const std::string &dir,
                                                         const std::string &version, bool is_local,
                                                         bool is_stdlib) {
  ...
  for (auto &ent : fs::directory_iterator(d, ec)) {
    ...
    if (fname.size() >= 8 && fname.compare(fname.size() - 8, 8, "_test.go") == 0)
      continue;
    ...
    for (const auto &fn : file->funcs) {
      if (!fn)
        continue;
      if (!StartsWithExportedName(fn->name))
        continue;
      GoSymbol s;
      s.name = fn->name;
      ...
      // index key.
      std::string key = s.receiver_type.empty() ? s.name : (s.receiver_type + "." + s.name);
...
const GoPackage *GoImportResolver::Resolve(const std::string &import_path) {
  if (auto it = by_path_.find(import_path); it != by_path_.end())
    return it->second;

  std::string dir = LocatePackageDir(import_path);
  if (dir.empty()) {
    diagnostics_.Report(core::SourceLoc{},
                        "go import not found: " + import_path + " (configured project_dir='" +
                            project_dir_ +
  ...
  bool is_local =
      !manifest_.module_path.empty() && import_path.rfind(manifest_.module_path, 0) == 0;
