...
void GoParser::ParseFile() {
  ...
  while (current_.kind != frontends::TokenKind::kEndOfFile) {
    if (IsKeyword("import")) {
      ...
      if (fn)
        file_->funcs.push_back(fn);
    } else {
      diagnostics_.Report(current_.loc, "unexpected token at top level: '" + current_.lexeme + "'");
      Advance();
    }
    SkipSemis();
  }
...
void GoParser::ParsePackage() {
  if (!ExpectKeyword("package", "expected 'package' clause"))
    return;
...
GenDecl GoParser::ParseGenDecl(const std::string &keyword) {
  ...
  auto parse_one = [&]() {
    if (keyword == "type")
      gd.types.push_back(ParseTypeSpec());
    else
      gd.values.push_back(ParseValueSpec());
  };
  if (MatchSymbol("(")) {
    SkipSemis();
    while (!IsSymbol(")") && current_.kind != frontends::TokenKind::kEndOfFile) {
...
std::vector<TypeNode::Field> GoParser::ParseStructFields() {
  ...
  while (!IsSymbol("}") && current_.kind != frontends::TokenKind::kEndOfFile) {
    ...
    if (current_.kind == frontends::TokenKind::kIdentifier) {
      ...
      bool has_type =
          !(IsSymbol(";") || IsSymbol("}") || current_.kind == frontends::TokenKind::kString);
      if (has_type) {
        f.names = names;
        f.type = ParseType();
      } else {
  ...
  while (!IsSymbol("}") && current_.kind != frontends::TokenKind::kEndOfFile) {
    ...
    if (current_.kind == frontends::TokenKind::kIdentifier) {
      ...
      if (IsSymbol("(")) {
        ...
        for (auto &p : ps)
          fty->params.push_back(p.second);
        for (auto &r : rs)
          fty->results.push_back(r.second);
        f.type = fty;
      } else {
        // embedded interface
...
std::shared_ptr<TypeNode> GoParser::ParseFuncType() {
  ...
  ParseSignature(ps, rs, variadic);
  for (auto &p : ps)
    t->params.push_back(p.second);
  for (auto &r : rs)
    t->results.push_back(r.second);
  return t;
}
...
std::vector<std::pair<std::string, std::shared_ptr<TypeNode>>> GoParser::ParseParamGroup(
    bool *is_variadic) {
    ...
    if (current_.kind == frontends::TokenKind::kIdentifier) {
      ...
      bool type_follows =
          (IsSymbol("...") || IsSymbol("*") || IsSymbol("[") || IsSymbol("(") || IsSymbol(".") ||
           IsKeyword("func") || IsKeyword("map") || IsKeyword("chan") || IsKeyword("struct") ||
           IsKeyword("interface") || current_.kind == frontends::TokenKind::kIdentifier);
...
void GoParser::ParseSignature(
    ...
    std::vector<std::pair<std::string, std::shared_ptr<TypeNode>>> &results, bool &is_variadic) {
  bool v = false;
...
std::shared_ptr<Statement> GoParser::ParseStatement() {
  ...
  if (IsKeyword("break") || IsKeyword("continue") || IsKeyword("goto") ||
      IsKeyword("fallthrough")) {
    std::string kw = current_.lexeme;
...
std::shared_ptr<Statement> GoParser::ParseIfStmt() {
  ...
  if (MatchSymbol(";")) {
    ...
    if (auto e = std::dynamic_pointer_cast<ExprStmt>(first))
      s->cond = e->expr;
    else {
      (void)saved;
      (void)saved_cur;
  ...
  if (MatchKeyword("else")) {
    if (IsKeyword("if"))
      s->else_branch = ParseIfStmt();
    else
      s->else_branch = ParseBlock();
  }
  return s;
}
...
std::shared_ptr<Statement> GoParser::ParseForStmt() {
  ...
  if (auto as = std::dynamic_pointer_cast<AssignStmt>(first)) {
    ...
    if (as->rhs.size() == 1) {
      if (auto un = std::dynamic_pointer_cast<UnaryExpr>(as->rhs.front());
          un && un->op == "range") {
        s->is_range = true;
        s->range_lhs = as->lhs;
...
std::shared_ptr<Expression> GoParser::ParsePrimaryExpr() {
  ...
  while (true) {
    if (IsSymbol(".")) {
      ...
      if (std::dynamic_pointer_cast<Identifier>(e) || std::dynamic_pointer_cast<SelectorExpr>(e)) {
        auto t = std::make_shared<TypeNode>();
        t->kind = TypeKind::kNamed;
...
std::shared_ptr<Expression> GoParser::ParseOperand() {
  ...
  if (current_.kind == frontends::TokenKind::kNumber) {
    ...
    if (has_i)
      lit->kind = BasicLit::Kind::kImag;
    else if (has_dot || has_e)
      lit->kind = BasicLit::Kind::kFloat;
    else
      lit->kind = BasicLit::Kind::kInt;
    Advance();
    return lit;
  }
  if (current_.kind == frontends::TokenKind::kString) {
