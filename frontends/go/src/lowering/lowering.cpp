...
ir::IRType ToIRType(const std::shared_ptr<TypeNode> &t) {
  ...
  if (t->kind == TypeKind::kPointer)
    return ir::IRType::Pointer(ToIRType(t->elem));
  if (t->kind == TypeKind::kSlice || t->kind == TypeKind::kArray || t->kind == TypeKind::kMap ||
      t->kind == TypeKind::kChan || t->kind == TypeKind::kFunc || t->kind == TypeKind::kInterface ||
  ...
  if (n == "bool")
    return ir::IRType::I1();
  if (n == "int8" || n == "byte" || n == "uint8")
    return ir::IRType::I8();
  if (n == "int16" || n == "uint16")
    return ir::IRType::I16();
  if (n == "int32" || n == "rune" || n == "uint32")
    return ir::IRType::I32();
  if (n == "int" || n == "int64" || n == "uint" || n == "uint64" || n == "uintptr")
...
private:
  void LowerFunc(const FuncDecl &f) {
    ...
    if (f.results.size() == 1)
      ret = ToIRType(f.results.front().second);
  ...
  std::string LowerExpr(const std::shared_ptr<Expression> &e, const ir::IRType &want) {
    ...
    if (auto c = std::dynamic_pointer_cast<CallExpr>(e)) {
      ...
      for (auto &a : c->args)
        args.push_back(LowerExpr(a, ir::IRType::I64()));
      std::string callee;
